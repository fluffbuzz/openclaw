# FluffBuzz ACP Bridge

This document describes how the FluffBuzz ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`fluffbuzz acp` exposes an ACP agent over stdio and forwards prompts to a running
FluffBuzz Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `fluffbuzz acp` and speaks ACP messages over stdio.
...
`fluffbuzz acp` resolves the Gateway URL and auth from CLI flags or config:
