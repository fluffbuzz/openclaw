...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "flufflog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
flufflog - FluffBuzz Logging Utility

USAGE:
    flufflog [OPTIONS]

DESCRIPTION:
    View FluffBuzz logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    FluffBuzz logs flow through the macOS unified log (subsystem: com.fluffbuzz).
...
QUICK START:
    flufflog -n 100             Show last 100 lines from all components
    flufflog -f                 Follow logs in real-time
    flufflog -e                 Show only errors
    flufflog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    flufflog                   Show last 50 lines from past 5 minutes (default)
    flufflog -f                Stream logs continuously
    flufflog -n 100            Show last 100 lines
    flufflog -e                Show only recent errors
    flufflog -l 30m -n 200     Show last 200 lines from past 30 minutes
    flufflog -c ServerManager  Show recent ServerManager logs
    flufflog -s "fail"         Search for "fail" in recent logs
    flufflog --server -e       Show recent server errors
    flufflog -f -d             Stream debug logs continuously
