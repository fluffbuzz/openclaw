...
IMAGE_NAME="fluffbuzz-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    FLUFFBUZZ_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    FLUFFBUZZ_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export FLUFFBUZZ_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/fluffbuzz-bin:$PATH"
  mkdir -p /tmp/fluffbuzz-bin
  cat > /tmp/fluffbuzz-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/fluffbuzz-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$FLUFFBUZZ_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_fluffbuzz_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/fluffbuzz-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/fluffbuzz-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$FLUFFBUZZ_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/fluffbuzz-e2e-$1.XXXXXX"
  }

  set_isolated_fluffbuzz_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export FLUFFBUZZ_HOME="$home_dir"
    export FLUFFBUZZ_STATE_DIR="$home_dir/.fluffbuzz"
    export FLUFFBUZZ_CONFIG_PATH="$FLUFFBUZZ_STATE_DIR/fluffbuzz.json"
    mkdir -p "$FLUFFBUZZ_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$FLUFFBUZZ_STATE_DIR/workspace"
    config_path="$FLUFFBUZZ_CONFIG_PATH"
    sessions_dir="$FLUFFBUZZ_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_fluffbuzz_env "$home_dir"
	    ...
	    node "$FLUFFBUZZ_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$FLUFFBUZZ_CONFIG_PATH" <<'"'"'JSON'"'"'
{
  "meta": {},
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$FLUFFBUZZ_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$FLUFFBUZZ_ENTRY\" configure --section skills" send_skills_flow
