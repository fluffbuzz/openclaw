...
IMAGE_NAME="fluffbuzz-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="fluffbuzz-net-e2e-$$"
GW_NAME="fluffbuzz-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "FLUFFBUZZ_GATEWAY_TOKEN=$TOKEN" \
  -e "FLUFFBUZZ_SKIP_CHANNELS=1" \
  -e "FLUFFBUZZ_SKIP_GMAIL_WATCHER=1" \
  -e "FLUFFBUZZ_SKIP_CRON=1" \
  -e "FLUFFBUZZ_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
