...
INSTALL_URL="${FLUFFBUZZ_INSTALL_URL:-https://fluffbuzz.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${FLUFFBUZZ_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FLUFFBUZZ_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="fluffbuzz"
PACKAGE_NAME="${FLUFFBUZZ_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FLUFFBUZZ_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FLUFFBUZZ_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FLUFFBUZZ_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
