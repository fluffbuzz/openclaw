# FluffBuzz Installer for Windows (PowerShell)
# Usage: iwr -useb https://fluffbuzz.com/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://fluffbuzz.com/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\fluffbuzz",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🐾 FluffBuzz Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one FluffBuzz.$NC" -Level info
    Write-Host ""
}
...
function Install-FluffBuzzNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing FluffBuzz (fluffbuzz@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g fluffbuzz@$Version --no-fund --no-audit 2>&1
        Write-Host "FluffBuzz installed" -Level success
        return $true
    } catch {
...
function Install-FluffBuzzGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing FluffBuzz from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/fluffbuzz/fluffbuzz.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\fluffbuzz\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\fluffbuzz.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install FluffBuzz from git to $GitDir" -Level info
        } else {
            Install-FluffBuzzGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install FluffBuzz via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-FluffBuzzNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'fluffbuzz onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🐾 FluffBuzz installed successfully!" -Level success
}
