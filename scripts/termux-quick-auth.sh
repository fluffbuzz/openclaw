...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/fluffbuzz/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FluffBuzz Re-Auth" -c "After getting key, run: ssh $SERVER '~/fluffbuzz/scripts/mobile-reauth.sh'" --id fluffbuzz-auth
        ;;
    *)
