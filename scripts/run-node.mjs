...
const shouldBuild = (deps) => {
  if (deps.env.FLUFFBUZZ_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.FLUFFBUZZ_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[fluffbuzz] ${message}\n`);
};

const runFluffBuzz = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["fluffbuzz.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runFluffBuzz(deps);
  }
