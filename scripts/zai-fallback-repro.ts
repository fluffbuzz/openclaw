...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "fluffbuzz.json");
  ...
  const sessionId =
    process.env.FLUFFBUZZ_ZAI_FALLBACK_SESSION_ID ??
    process.env.FLUFFBOT_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const baseEnv: NodeJS.ProcessEnv = {
    ...
    FLUFFBOT_CONFIG_PATH: configPath,
    FLUFFBOT_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  };
  ...
  const run1 = await runCommand(
    "run1",
    ["fluffbuzz", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["fluffbuzz", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
