...
#
# Suggested cron: */30 * * * * /home/admin/fluffbuzz/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FluffBuzz notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., fluffbuzz-alerts)
...
STATE_FILE="$HOME/.fluffbuzz/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FluffBuzz if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use fluffbuzz
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FluffBuzz to $NOTIFY_PHONE..."
            fluffbuzz send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FluffBuzz is down. Run: ssh l36 '~/fluffbuzz/scripts/mobile-reauth.sh'" "urgent"
    exit 1
