#!/usr/bin/env bash
# ClawDock - Docker helpers for FluffBuzz
# Inspired by Simon Willison's "Running FluffBuzz in Docker"
# https://til.simonwillison.net/llms/fluffbuzz-docker
#
# Installation:
...
# Common paths to check for FluffBuzz
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/fluffbuzz"
  "${HOME}/workspace/fluffbuzz"
  "${HOME}/projects/fluffbuzz"
  "${HOME}/dev/fluffbuzz"
  "${HOME}/code/fluffbuzz"
  "${HOME}/src/fluffbuzz"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🐾 Found FluffBuzz at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/fluffbuzz"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ FluffBuzz not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/fluffbuzz/fluffbuzz.git ~/fluffbuzz"
    echo "  cd ~/fluffbuzz && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^FLUFFBUZZ_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d fluffbuzz-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart fluffbuzz-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f fluffbuzz-gateway
}
...
clawdock-workspace() {
  cd ~/.fluffbuzz/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec fluffbuzz-gateway \
    bash -c 'echo "alias fluffbuzz=\"./fluffbuzz.mjs\"" > /tmp/.bashrc_fluffbuzz && bash --rcfile /tmp/.bashrc_fluffbuzz'
}
...
clawdock-cli() {
  _clawdock_compose run --rm fluffbuzz-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build fluffbuzz-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "FLUFFBUZZ_GATEWAY_TOKEN=$token" fluffbuzz-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" fluffbuzz-gateway \
    bash -c './fluffbuzz.mjs config set gateway.remote.token "$TOKEN" && ./fluffbuzz.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec fluffbuzz-gateway \
    bash -c "./fluffbuzz.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart fluffbuzz-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm fluffbuzz-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec fluffbuzz-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'fluffbuzz config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🐾 ClawDock - Docker Helpers for FluffBuzz${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (fluffbuzz alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to fluffbuzz project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.fluffbuzz)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'fluffbuzz channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'fluffbuzz status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.fluffbuzz.com${_CLR_RESET}"
  echo ""
}
