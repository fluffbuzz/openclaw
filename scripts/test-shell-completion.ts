 ...
 * `fluffbuzz update`. Use it to verify the completion installation flow
...
\`fluffbuzz update\`, \`fluffbuzz doctor\`, and \`fluffbuzz onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FluffBuzz Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`fluffbuzz completion --install\` later to enable.`));
    return;
  }
