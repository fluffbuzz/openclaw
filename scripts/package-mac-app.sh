...
# Build and bundle FluffBuzz into a minimal .app we can open.
# Outputs to dist/FluffBuzz.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FluffBuzz.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FluffBuzz"
BUNDLE_ID="${BUNDLE_ID:-com.fluffbuzz.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/fluffbuzz/fluffbuzz/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FluffBuzz/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FluffBuzzBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FluffBuzzGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FluffBuzz"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FluffBuzz"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FluffBuzz"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FluffBuzz" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FluffBuzz/Resources/FluffBuzz.icns" "$APP_ROOT/Contents/Resources/FluffBuzz.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FluffBuzz/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FluffBuzzKit resources"
FLUFFBUZZKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FluffBuzzKit_FluffBuzzKit.bundle"
if [ -d "$FLUFFBUZZKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FluffBuzzKit_FluffBuzzKit.bundle"
  cp -R "$FLUFFBUZZKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FluffBuzzKit_FluffBuzzKit.bundle"
else
  echo "WARN: FluffBuzzKit resource bundle not found at $FLUFFBUZZKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FluffBuzz"
killall -q FluffBuzz 2>/dev/null || true
