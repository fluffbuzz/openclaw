...
BASE_IMAGE="${BASE_IMAGE:-fluffbuzz-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-fluffbuzz-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=fluffbuzz.sandbox=1)
NOTE
