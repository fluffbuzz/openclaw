...
# FluffBuzz Auth Widget for Termux
...
# Check auth status
termux-toast "Checking FluffBuzz auth..."

STATUS=$(ssh "$SERVER" '$HOME/fluffbuzz/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/fluffbuzz/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|FLUFFBUZZ_EXPIRING|FLUFFBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/fluffbuzz/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|FLUFFBUZZ_EXPIRED|FLUFFBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: FluffBuzz won't work until re-auth"
                ;;
        esac
        ;;
