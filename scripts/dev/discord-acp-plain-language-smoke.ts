...
type DriverMode = "token" | "webhook" | "fluffbuzz";
...
function resolveStateDir(): string {
  const override = process.env.FLUFFBUZZ_STATE_DIR?.trim() || process.env.FLUFFBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.FLUFFBUZZ_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".fluffbuzz");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver fluffbuzz] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) FluffBuzz spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|fluffbuzz> Driver transport mode (default: token)\n" +
    ...
    "  --fluffbuzz-bin <path>        FluffBuzz CLI binary for driver=fluffbuzz (default: fluffbuzz)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_DRIVER\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_AGENT\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_POLL_MS\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  FLUFFBUZZ_DISCORD_SMOKE_FLUFFBUZZ_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.FLUFFBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_DRIVER ||
    process.env.FLUFFBOT_DISCORD_SMOKE_DRIVER ||
    "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "fluffbuzz"
        ? "fluffbuzz"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.FLUFFBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.FLUFFBUZZ_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.FLUFFBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_AGENT ||
    process.env.FLUFFBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.FLUFFBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.FLUFFBUZZ_DISCORD_SMOKE_INSTRUCTION ||
    process.env.FLUFFBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.FLUFFBUZZ_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.FLUFFBUZZ_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.FLUFFBUZZ_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const fluffbuzzBin =
    resolveArg("--fluffbuzz-bin") || process.env.FLUFFBUZZ_DISCORD_SMOKE_FLUFFBUZZ_BIN || "fluffbuzz";
...
async function fluffbuzzCliJson<T>(params: { fluffbuzzBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.fluffbuzzBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`fluffbuzz ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await fluffbuzzCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    fluffbuzzBin: params.fluffbuzzBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "fluffbuzz") {
    return await readMessagesWithOpenclaw({
      fluffbuzzBin: params.args.fluffbuzzBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `fluffbuzz-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await fluffbuzzCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        fluffbuzzBin: args.fluffbuzzBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("fluffbuzz message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from FluffBuzz.`,
        diagnostics: {
          bindingCandidates: [
            {
