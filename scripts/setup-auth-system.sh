#!/bin/bash
# Setup FluffBuzz Auth Management System
...
echo "=== FluffBuzz Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'fluffbuzz-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FluffBuzz message: Send warning via FluffBuzz itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/fluffbuzz-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/fluffbuzz-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/fluffbuzz-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now fluffbuzz-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status fluffbuzz-auth-monitor.timer"
echo ""
echo "Quick commands:"
