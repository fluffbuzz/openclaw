...
process.env.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { FluffBuzzConfig } from "../src/config/config.js";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.js";
import { withIsolatedTestHome } from "./test-env.js";

// Set HOME/state isolation before importing any runtime FluffBuzz modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: FluffBuzzConfig) => {
    ...
    resolveAccount: (cfg: FluffBuzzConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: FluffBuzzConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
