...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `fluffbuzz-e2e-${name}-`));
  const configDir = path.join(homeDir, ".fluffbuzz");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          FLUFFBUZZ_SKIP_PROVIDERS: "1",
          FLUFFBUZZ_SKIP_GMAIL_WATCHER: "1",
          FLUFFBUZZ_SKIP_CRON: "1",
          FLUFFBUZZ_SKIP_BROWSER_CONTROL_SERVER: "1",
          FLUFFBUZZ_SKIP_CANVAS_HOST: "1",
          FLUFFBUZZ_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
