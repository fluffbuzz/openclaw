...
type EnvSnapshot = {
  ...
  fluffbuzzHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    fluffbuzzHome: process.env.FLUFFBUZZ_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("FLUFFBUZZ_HOME", snapshot.fluffbuzzHome);
  restoreKey("FLUFFBUZZ_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked FLUFFBUZZ_HOME.
  delete process.env.FLUFFBUZZ_HOME;
  process.env.FLUFFBUZZ_STATE_DIR = path.join(base, ".fluffbuzz");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "fluffbuzz-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".fluffbuzz", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
