...
export async function handleWhatsAppStart(host: FluffBuzzApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: FluffBuzzApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: FluffBuzzApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: FluffBuzzApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: FluffBuzzApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: FluffBuzzApp): string {
...
function resolveGatewayHttpAuthHeader(host: FluffBuzzApp): string | null {
...
function buildGatewayHttpHeaders(host: FluffBuzzApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: FluffBuzzApp) {
...
export function handleNostrProfileToggleAdvanced(host: FluffBuzzApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: FluffBuzzApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: FluffBuzzApp) {
  const state = host.nostrProfileFormState;
