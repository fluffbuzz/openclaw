...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    clientName: "fluffbuzz-control-ui",
    mode: "webchat",
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as FluffBuzzApp);
      void loadAgents(host as unknown as FluffBuzzApp);
      void loadToolsCatalog(host as unknown as FluffBuzzApp);
      void loadNodes(host as unknown as FluffBuzzApp, { quiet: true });
      void loadDevices(host as unknown as FluffBuzzApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (state === "final") {
    void loadSessions(host as unknown as FluffBuzzApp, {
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as FluffBuzzApp, payload);
  handleTerminalChatEvent(host, payload, state);
  if (state === "final" && shouldReloadHistoryForFinalEvent(payload)) {
    void loadChatHistory(host as unknown as FluffBuzzApp);
  }
}
