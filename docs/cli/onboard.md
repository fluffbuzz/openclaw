---
summary: "CLI reference for `fluffbuzz onboard` (interactive onboarding wizard)"
read_when:
...
```bash
fluffbuzz onboard
fluffbuzz onboard --flow quickstart
fluffbuzz onboard --flow manual
fluffbuzz onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`FLUFFBUZZ_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `fluffbuzz dashboard` (Control UI, no channel setup).
