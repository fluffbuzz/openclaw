---
summary: "CLI reference for `fluffbuzz qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
fluffbuzz qr
fluffbuzz qr --setup-code-only
fluffbuzz qr --json
fluffbuzz qr --remote
fluffbuzz qr --url wss://gateway.example/ws --token '<token>'
```
