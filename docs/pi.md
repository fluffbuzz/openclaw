...
summary: "Architecture of FluffBuzz's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in FluffBuzz
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how FluffBuzz integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

FluffBuzz uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, FluffBuzz directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in FluffBuzz's local TUI mode)                                             |
...
├── pi-tools.ts                    # createFluffBuzzCodingTools()
...
├── fluffbuzz-tools.ts              # FluffBuzz-specific tools
...
Image injection is prompt-local: FluffBuzz loads image refs from the current prompt and
...
2. **Custom Replacements**: FluffBuzz replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **FluffBuzz Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
FluffBuzz wraps this with `guardSessionManager()` for tool result safety.
...
FluffBuzz maintains an auth profile store with multiple API keys per provider:
...
FluffBuzz loads custom pi extensions for specialized behavior:
...
FluffBuzz also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | FluffBuzz Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom FluffBuzz tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.fluffbuzz/agents/<agentId>/sessions/` (or `$FLUFFBUZZ_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `FLUFFBUZZ_LIVE_TEST=1`)
