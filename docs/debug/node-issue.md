...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FluffBuzz
...
Running FluffBuzz via Node with `tsx` fails at startup with:

```
[fluffbuzz] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `fluffbuzz status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch fluffbuzz.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node fluffbuzz.mjs status` works on Node 25.
