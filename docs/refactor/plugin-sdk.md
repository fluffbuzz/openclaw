...
- Publish as `fluffbuzz/plugin-sdk` (or export from core under `fluffbuzz/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `FluffBuzzPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: FluffBuzzConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: FluffBuzzConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: FluffBuzzConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: FluffBuzzConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: FluffBuzzConfig): string;
  };
};
```
...
- Introduce `fluffbuzz/plugin-sdk`.
- Add `api.runtime` to `FluffBuzzPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `fluffbuzzRuntime: ">=2026.2.0"`).
