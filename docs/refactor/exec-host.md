...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.fluffbuzz/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.fluffbuzz/node.json` (node id + pairing token).
...
Path: `~/.fluffbuzz/exec-approvals.json`
...
- Unix socket at `~/.fluffbuzz/exec-approvals.sock` (0600).
