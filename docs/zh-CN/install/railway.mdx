...
通过一键模板在 Railway 上部署 FluffBuzz，并在浏览器中完成设置。
...
<a href="https://railway.com/deploy/fluffbot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/fluffbuzz` — 控制面板 UI

## 你将获得

- 托管的 FluffBuzz Gateway网关 + 控制面板 UI
- `/setup` 网页设置向导（无需终端命令）
...
- `FLUFFBUZZ_STATE_DIR=/data/.fluffbuzz`（推荐）
- `FLUFFBUZZ_WORKSPACE_DIR=/data/workspace`（推荐）
- `FLUFFBUZZ_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 FluffBuzz 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
