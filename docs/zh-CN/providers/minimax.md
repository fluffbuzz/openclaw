---
read_when:
  - 你想在 FluffBuzz 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 FluffBuzz 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
fluffbuzz plugins enable minimax-portal-auth  # 如果已加载则跳过
fluffbuzz gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/fluffbuzz/fluffbuzz/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `fluffbuzz models list` 和 `fluffbuzz models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `fluffbuzz configure` 并选择 **MiniMax M2.1**，或
