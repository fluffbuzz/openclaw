...
你可以在 `~/.fluffbuzz/fluffbuzz.json` 中覆盖此设置：
...
所有日志配置都在 `~/.fluffbuzz/fluffbuzz.json` 的 `logging` 下。
...
- FluffBuzz 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `fluffbuzz plugins enable diagnostics-otel` 启用插件。
...
- `fluffbuzz.tokens`（计数器，属性：`fluffbuzz.token`、`fluffbuzz.channel`、`fluffbuzz.provider`、`fluffbuzz.model`）
- `fluffbuzz.cost.usd`（计数器，属性：`fluffbuzz.channel`、`fluffbuzz.provider`、`fluffbuzz.model`）
- `fluffbuzz.run.duration_ms`（直方图，属性：`fluffbuzz.channel`、`fluffbuzz.provider`、`fluffbuzz.model`）
- `fluffbuzz.context.tokens`（直方图，属性：`fluffbuzz.context`、`fluffbuzz.channel`、`fluffbuzz.provider`、`fluffbuzz.model`）

消息流：

- `fluffbuzz.webhook.received`（计数器，属性：`fluffbuzz.channel`、`fluffbuzz.webhook`）
- `fluffbuzz.webhook.error`（计数器，属性：`fluffbuzz.channel`、`fluffbuzz.webhook`）
- `fluffbuzz.webhook.duration_ms`（直方图，属性：`fluffbuzz.channel`、`fluffbuzz.webhook`）
- `fluffbuzz.message.queued`（计数器，属性：`fluffbuzz.channel`、`fluffbuzz.source`）
- `fluffbuzz.message.processed`（计数器，属性：`fluffbuzz.channel`、`fluffbuzz.outcome`）
- `fluffbuzz.message.duration_ms`（直方图，属性：`fluffbuzz.channel`、`fluffbuzz.outcome`）

队列 + 会话：

- `fluffbuzz.queue.lane.enqueue`（计数器，属性：`fluffbuzz.lane`）
- `fluffbuzz.queue.lane.dequeue`（计数器，属性：`fluffbuzz.lane`）
- `fluffbuzz.queue.depth`（直方图，属性：`fluffbuzz.lane` 或 `fluffbuzz.channel=heartbeat`）
- `fluffbuzz.queue.wait_ms`（直方图，属性：`fluffbuzz.lane`）
- `fluffbuzz.session.state`（计数器，属性：`fluffbuzz.state`、`fluffbuzz.reason`）
- `fluffbuzz.session.stuck`（计数器，属性：`fluffbuzz.state`）
- `fluffbuzz.session.stuck_age_ms`（直方图，属性：`fluffbuzz.state`）
- `fluffbuzz.run.attempt`（计数器，属性：`fluffbuzz.attempt`）
...
- `fluffbuzz.webhook.error`
  - `fluffbuzz.channel`、`fluffbuzz.webhook`、`fluffbuzz.chatId`、`fluffbuzz.error`
- `fluffbuzz.message.processed`
  - `fluffbuzz.channel`、`fluffbuzz.outcome`、`fluffbuzz.chatId`、`fluffbuzz.messageId`、`fluffbuzz.sessionKey`、`fluffbuzz.sessionId`、`fluffbuzz.reason`
- `fluffbuzz.session.stuck`
  - `fluffbuzz.state`、`fluffbuzz.ageMs`、`fluffbuzz.queueDepth`、`fluffbuzz.sessionKey`、`fluffbuzz.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `fluffbuzz doctor`。
