...
当你需要切换不常用的设置而不编辑 `fluffbuzz.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.fluffbuzz-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
```bash
pnpm gateway:dev
FLUFFBUZZ_PROFILE=dev fluffbuzz tui
```

如果你还没有全局安装，请通过 `pnpm fluffbuzz ...` 运行 CLI。
...
1. **配置文件隔离**（全局 `--dev`）
   ...
   - `FLUFFBUZZ_GATEWAY_PORT=19001`（浏览器/画布相应移动）
...
2. **Dev 引导**（`gateway --dev`）
   ...
   - 默认身份：**C3‑PO**（礼仪机器人）。
   - 在 dev 模式下跳过渠道提供商（`FLUFFBUZZ_SKIP_CHANNELS=1`）。
...
```bash
FLUFFBUZZ_PROFILE=dev fluffbuzz gateway --dev --reset
```
...
FluffBuzz 可以在任何过滤/格式化之前记录**原始助手流**。
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.fluffbuzz/logs/raw-stream.jsonl
```

等效环境变量：

```bash
FLUFFBUZZ_RAW_STREAM=1
FLUFFBUZZ_RAW_STREAM_PATH=~/.fluffbuzz/logs/raw-stream.jsonl
```

默认文件：

`~/.fluffbuzz/logs/raw-stream.jsonl`
