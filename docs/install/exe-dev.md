---
summary: "Run FluffBuzz Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: FluffBuzz Gateway running on an exe.dev VM, reachable from your laptop via: `https://<vm-name>.exe.xyz`
...
Shelley, [exe.dev](https://exe.dev)'s agent, can install FluffBuzz instantly with our
...
Tip: keep this VM **stateful**. FluffBuzz stores state under `~/.fluffbuzz/` and `~/.fluffbuzz/workspace/`.
...
token from `gateway.auth.token` on the VM (retrieve with `fluffbuzz config get gateway.auth.token`, or generate one
with `fluffbuzz doctor --generate-gateway-token`). Approve devices with `fluffbuzz devices list` and
`fluffbuzz devices approve <requestId>`. When in doubt, use Shelley from your browser!
