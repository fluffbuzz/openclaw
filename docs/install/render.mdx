...
Deploy FluffBuzz on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/fluffbuzz/fluffbuzz)
...
repository configures everything needed to run FluffBuzz:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: FLUFFBUZZ_STATE_DIR
        value: /data/.fluffbuzz
      - key: FLUFFBUZZ_WORKSPACE_DIR
        value: /data/workspace
      - key: FLUFFBUZZ_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/fluffbuzz`.
...
If you use the original FluffBuzz repository, Render will not auto-deploy your FluffBuzz. To update it, run a manual Blueprint sync from the dashboard.
...
For FluffBuzz, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any FluffBuzz host.
