...
read_when:
  - Setting up Zalo Personal for FluffBuzz
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside FluffBuzz.
...
- Install via CLI: `fluffbuzz plugins install @fluffbuzz/zalouser`
- Or from a source checkout: `fluffbuzz plugins install ./extensions/zalouser`
...
- `fluffbuzz pairing list zalouser`
- `fluffbuzz pairing approve zalouser <code>`
...
- On startup, FluffBuzz resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in FluffBuzz state. Example:
...
- FluffBuzz sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, FluffBuzz sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `fluffbuzz channels logout --channel zalouser && fluffbuzz channels login --channel zalouser`
...
- The channel now runs fully in FluffBuzz without external CLI binaries.
