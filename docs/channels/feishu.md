...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects FluffBuzz to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
fluffbuzz plugins install @fluffbuzz/feishu
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/feishu
```
...
If you just installed FluffBuzz, run the wizard:
...
2. Configuring app credentials in FluffBuzz
...
1. You already ran `fluffbuzz channels add` for Feishu
2. The gateway is running (`fluffbuzz gateway status`)
...
```bash
fluffbuzz pairing approve feishu <CODE>
```
...
2. Run `fluffbuzz logs --follow` and look for `chat_id`
...
2. Run `fluffbuzz logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `fluffbuzz gateway status`  | Show gateway status           |
| `fluffbuzz gateway install` | Install/start gateway service |
| `fluffbuzz gateway stop`    | Stop gateway service          |
| `fluffbuzz gateway restart` | Restart gateway service       |
| `fluffbuzz logs --follow`   | Tail gateway logs             |
...
4. Check logs: `fluffbuzz logs --follow`
...
5. Ensure the gateway is running: `fluffbuzz gateway status`
6. Check logs: `fluffbuzz logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.fluffbuzz/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.fluffbuzz/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
