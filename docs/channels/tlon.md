...
Tlon is a decentralized messenger built on Urbit. FluffBuzz connects to your Urbit ship and can
...
```bash
fluffbuzz plugins install @fluffbuzz/tlon
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/tlon
```
...
By default, FluffBuzz blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `fluffbuzz message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FluffBuzz replies in-thread.
