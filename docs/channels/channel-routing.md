...
FluffBuzz routes replies **back to the channel where a message came from**. The
...
FluffBuzz infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, FluffBuzz still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when FluffBuzz would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.fluffbuzz/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.fluffbuzz`):

- `~/.fluffbuzz/agents/<agentId>/sessions/sessions.json`
