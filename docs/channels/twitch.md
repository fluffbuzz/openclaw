...
read_when:
  - Setting up Twitch chat integration for FluffBuzz
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. FluffBuzz connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
fluffbuzz plugins install @fluffbuzz/twitch
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FLUFFBUZZ_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "fluffbuzz", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FLUFFBUZZ_TWITCH_ACCESS_TOKEN env var)
...
```bash
FLUFFBUZZ_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
