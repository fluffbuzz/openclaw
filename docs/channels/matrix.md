...
Matrix is an open, decentralized messaging protocol. FluffBuzz connects as a Matrix **user**
...
```bash
fluffbuzz plugins install @fluffbuzz/matrix
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `fluffbuzz plugins install @fluffbuzz/matrix`
   - From a local checkout: `fluffbuzz plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FluffBuzz calls the same
     login endpoint, stores the access token in `~/.fluffbuzz/credentials/matrix/credentials.json`,
...
- On first connection, FluffBuzz requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FluffBuzz logs a warning.
...
`~/.fluffbuzz/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `fluffbuzz pairing list matrix`
  - `fluffbuzz pairing approve matrix <CODE>`
...
- On startup, FluffBuzz resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
