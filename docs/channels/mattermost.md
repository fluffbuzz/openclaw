---
summary: "Mattermost bot setup and FluffBuzz config"
read_when:
...
```bash
fluffbuzz plugins install @fluffbuzz/mattermost
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/mattermost
```
...
4. Configure FluffBuzz and start the gateway.
...
- Approve via:
  - `fluffbuzz pairing list mattermost`
  - `fluffbuzz pairing approve mattermost <CODE>`
...
Use these target formats with `fluffbuzz message send` or cron/webhooks:
