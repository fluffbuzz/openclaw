...
```
    ...
    Telegram does **not** use `fluffbuzz channels login telegram`; configure token in config/env, then start gateway.
...
fluffbuzz pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `fluffbuzz doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `fluffbuzz doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `fluffbuzz logs --follow`
...
- DM messages can carry `message_thread_id`; FluffBuzz routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (native drafts + message edits)">
    FluffBuzz can stream partial replies in real time:
    ...
    - DM: FluffBuzz updates the draft in place (no extra preview message)
    - group/topic: FluffBuzz keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), FluffBuzz falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, FluffBuzz skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, FluffBuzz automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, FluffBuzz retries as plain text.
...
```
    ...
    - `~/.fluffbuzz/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, FluffBuzz enqueues system events like:
...
```bash
fluffbuzz message send --channel telegram --target 123456789 --message "hi"
fluffbuzz message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `fluffbuzz channels status` warns when config expects unmentioned group messages.
    - `fluffbuzz channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `fluffbuzz logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, FluffBuzz now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `FLUFFBUZZ_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `FLUFFBUZZ_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `FLUFFBUZZ_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `fluffbuzz doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `fluffbuzz doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
