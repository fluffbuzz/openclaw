...
```bash
fluffbuzz plugins install @fluffbuzz/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FluffBuzz" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `fluffbuzz pairing list nextcloud-talk`
  - `fluffbuzz pairing approve nextcloud-talk <CODE>`
