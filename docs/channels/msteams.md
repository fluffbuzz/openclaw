...
```bash
fluffbuzz plugins install @fluffbuzz/msteams
```

Local checkout (when running from a git repo):

```bash
fluffbuzz plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
FluffBuzz will offer the local install path automatically.
...
3. Configure FluffBuzz with those credentials.
...
- Talk to FluffBuzz via Teams DMs, group chats, or channels.
...
- On startup, FluffBuzz resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.fluffbuzz/fluffbuzz.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring FluffBuzz, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `fluffbuzz-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `fluffbuzz plugins install @fluffbuzz/msteams`
   - From a local checkout: `fluffbuzz plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "FluffBuzz in Teams", "full": "FluffBuzz in Teams" },
...
FluffBuzz handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, FluffBuzz only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/FluffBuzzShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

FluffBuzz sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `fluffbuzz message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.fluffbuzz/msteams-polls.json`.
...
```bash
fluffbuzz message send --channel msteams \
...
fluffbuzz message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
fluffbuzz message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
fluffbuzz message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
fluffbuzz message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
