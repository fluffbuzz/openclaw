...
```bash
fluffbuzz tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `fluffbuzz tui --deliver`
...
- Local shell commands receive `FLUFFBUZZ_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `fluffbuzz logs --follow`.
- Confirm the agent can run: `fluffbuzz status` and `fluffbuzz models status`.
...
- No agents in picker: check `fluffbuzz agents list` and your routing config.
