...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/fluffbuzz`)
...
If the page fails to load, start the Gateway first: `fluffbuzz gateway`.
...
you revoke it with `fluffbuzz devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.fluffbuzz/fluffbuzz.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop fluffbuzz`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. FluffBuzz
...
FluffBuzz **blocks** Control UI connections without device identity.
