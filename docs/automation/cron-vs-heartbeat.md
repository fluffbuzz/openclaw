...
fluffbuzz cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
fluffbuzz cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
fluffbuzz cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Puppy: Deterministic workflows with approvals

Puppy is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
...
- **Puppy** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn that calls Puppy.
For ad-hoc workflows, call Puppy directly.

### Operational notes (from the code)

- Puppy runs as a **local subprocess** (`puppy` CLI) in tool mode and returns a **JSON envelope**.
- If the tool returns `needs_approval`, you resume with a `resumeToken` and `approve` flag.
- The tool is an **optional plugin**; enable it additively via `tools.alsoAllow: ["puppy"]` (recommended).
- Puppy expects the `puppy` CLI to be available on `PATH`.

See [Puppy](/tools/puppy) for full usage and examples.
