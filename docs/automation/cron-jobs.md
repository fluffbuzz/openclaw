...
- Jobs persist under `~/.fluffbuzz/cron/` so restarts don’t lose schedules.
...
fluffbuzz cron list
fluffbuzz cron run <job-id>
fluffbuzz cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.fluffbuzz/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `fluffbuzz cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, FluffBuzz applies a
...
If `delivery` is omitted for isolated jobs, FluffBuzz defaults to `announce`.
...
- Job store: `~/.fluffbuzz/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.fluffbuzz/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, FluffBuzz classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.fluffbuzz/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, FluffBuzz archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `fluffbuzz cron runs` and adjust retention before logs become large
...
fluffbuzz cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
fluffbuzz cron edit <jobId> --agent ops
fluffbuzz cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
fluffbuzz cron run <jobId>
fluffbuzz cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
fluffbuzz cron edit <jobId> \
...
```bash
fluffbuzz cron edit <jobId> --exact
```
...
```bash
fluffbuzz system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`fluffbuzz system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `FLUFFBUZZ_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- FluffBuzz applies exponential retry backoff for recurring jobs after consecutive errors:
