...
read_when:
  ...
  - Wiring external systems into FluffBuzz
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your FluffBuzz config directory (typically `~/.fluffbuzz/hooks/transforms`).
...
- `fluffbuzz webhooks gmail setup` writes `hooks.gmail` config for `fluffbuzz webhooks gmail run`.
