...
- With Anthropic API-key auth profiles, FluffBuzz seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, FluffBuzz injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
FluffBuzz exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.fluffbuzz/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$FLUFFBUZZ_STATE_DIR/logs/cache-trace.jsonl`
...
- `FLUFFBUZZ_CACHE_TRACE=1` enables cache tracing.
- `FLUFFBUZZ_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `FLUFFBUZZ_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `FLUFFBUZZ_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `FLUFFBUZZ_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
