...
FluffBuzz integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- FluffBuzz owns lifecycle when `channels.signal.autoStart=true`.
...
- FluffBuzz spawns `imsg rpc` as a child process (legacy iMessage integration).
