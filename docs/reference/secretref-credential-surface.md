...
- In scope: strictly user-supplied credentials that FluffBuzz does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `fluffbuzz.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
