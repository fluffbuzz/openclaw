...
FluffBuzz features that can generate provider usage or paid API calls.
...
- `fluffbuzz status --usage` and `fluffbuzz channels list` show provider **usage windows**
...
FluffBuzz can pick up credentials from:
...
- `fluffbuzz status --usage`
- `fluffbuzz models status --json`
...
`fluffbuzz models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
