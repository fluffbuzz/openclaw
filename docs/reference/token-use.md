---
summary: "How FluffBuzz builds prompt context and reports token usage + costs"
read_when:
...
FluffBuzz tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

FluffBuzz assembles its own system prompt on every run. It includes:
...
For images, FluffBuzz downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from FluffBuzz session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `fluffbuzz status --usage` and `fluffbuzz channels list` show
...
`cacheWrite`. If pricing is missing, FluffBuzz shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. FluffBuzz can
...
Anthropic's 1M context window is currently beta-gated. FluffBuzz can inject the
...
FluffBuzz skips the `context-1m-*` beta header because Anthropic currently
