...
read_when:
  - Installing FluffBuzz on Windows
...
FluffBuzz on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
```bash
systemctl --user is-enabled fluffbuzz-gateway
systemctl --user status fluffbuzz-gateway --no-pager
```
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `fluffbuzz status --all` to confirm.
