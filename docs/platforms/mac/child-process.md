...
service via the external `fluffbuzz` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `com.fluffbuzz.gateway`
  (or `com.fluffbuzz.<profile>` when using `--profile`/`FLUFFBUZZ_PROFILE`; legacy `com.fluffbuzz.*` is supported).
