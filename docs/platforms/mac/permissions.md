...
- Same path: run the app from a fixed location (for FluffBuzz, `dist/FluffBuzz.app`).
...
```bash
sudo tccutil reset Accessibility com.fluffbuzz.mac
sudo tccutil reset ScreenCapture com.fluffbuzz.mac
...
Workaround: move files into the FluffBuzz workspace (`~/.fluffbuzz/workspace`) if you want to avoid per-folder grants.
