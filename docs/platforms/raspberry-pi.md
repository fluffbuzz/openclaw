---
summary: "FluffBuzz on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up FluffBuzz on a Raspberry Pi
  - Running FluffBuzz on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# FluffBuzz on Raspberry Pi

## Goal

Run a persistent, always-on FluffBuzz Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
fluffbuzz config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/fluffbuzz-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/fluffbuzz-compile-cache
mkdir -p /var/tmp/fluffbuzz-compile-cache
export FLUFFBUZZ_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `FLUFFBUZZ_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running FluffBuzz, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit fluffbuzz
```

```ini
[Service]
Environment=FLUFFBUZZ_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/fluffbuzz-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep FluffBuzz state/cache on SSD-backed storage to avoid SD-card
...
Most FluffBuzz features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled fluffbuzz

# Enable if not
sudo systemctl enable fluffbuzz

# Start on boot
sudo systemctl start fluffbuzz
```
...
```bash
# Check logs
journalctl -u fluffbuzz --no-pager -n 100

# Common fix: rebuild
cd ~/fluffbuzz  # if using hackable install
npm run build
sudo systemctl restart fluffbuzz
```
