---
summary: "FluffBuzz on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up FluffBuzz on DigitalOcean
  - Looking for cheap VPS hosting for FluffBuzz
title: "DigitalOcean"
---

# FluffBuzz on DigitalOcean

## Goal

Run a persistent FluffBuzz Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status fluffbuzz-gateway.service

# View logs
journalctl --user -u fluffbuzz-gateway.service -f
```
...
```bash
fluffbuzz channels login whatsapp
# Scan QR code
```
...
- `~/.fluffbuzz/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u fluffbuzz --no-pager -n 50
```
