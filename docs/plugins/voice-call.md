...
read_when:
  - You want to place an outbound voice call from FluffBuzz
...
Voice calls for FluffBuzz via a plugin. Supports outbound notifications and
...
- Use `fluffbuzz voicecall ...` or the `voice_call` tool
...
```bash
fluffbuzz plugins install ./extensions/voice-call
