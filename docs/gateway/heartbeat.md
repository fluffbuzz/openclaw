...
- During heartbeat runs, FluffBuzz treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `fluffbuzz sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FluffBuzz skips the heartbeat run entirely (no model call).
...
```bash
fluffbuzz system event --text "Check for urgent follow-ups" --mode now
```
