...
read_when:
  - Running FluffBuzz behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of FluffBuzz
...
- You run FluffBuzz behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. FluffBuzz checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. FluffBuzz extracts the user identity from the configured header
...
- FluffBuzz can stay on loopback HTTP behind the proxy.
...
If FluffBuzz itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://fluffbuzz.example.com
    to: http://fluffbuzz-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
fluffbuzz.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy fluffbuzz:18789 {
...
location / {
    ...
    proxy_pass http://fluffbuzz:18789;
...
`fluffbuzz security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update FluffBuzz config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `fluffbuzz security audit` and review findings
