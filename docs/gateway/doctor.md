...
`fluffbuzz doctor` is the repair + migration tool for FluffBuzz. It fixes stale
...
- Extra workspace dir detection (`~/fluffbuzz`).
...
you to run `fluffbuzz doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.fluffbuzz/fluffbuzz.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.fluffbuzz/sessions/` to `~/.fluffbuzz/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.fluffbuzz/agent/` to `~/.fluffbuzz/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.fluffbuzz/credentials/*.json` (except `oauth.json`)
  - to `~/.fluffbuzz/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `fluffbuzz doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.fluffbuzz` folders exist across
  home directories or when `FLUFFBUZZ_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.fluffbuzz/fluffbuzz.json` is
...
offers to remove them and install the FluffBuzz service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named FluffBuzz gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `fluffbuzz doctor --generate-gateway-token` to force token
...
- `fluffbuzz doctor` prompts before rewriting supervisor config.
- `fluffbuzz doctor --yes` accepts the default repair prompts.
- `fluffbuzz doctor --repair` applies recommended fixes without prompts.
- `fluffbuzz doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `fluffbuzz gateway install --force`.
