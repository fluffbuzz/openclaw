...
> FluffBuzz is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
FluffBuzz security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `fluffbuzz security audit`
...
```bash
fluffbuzz security audit
fluffbuzz security audit --deep
fluffbuzz security audit --fix
fluffbuzz security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

FluffBuzz is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
FluffBuzz assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.fluffbuzz`, including `fluffbuzz.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- FluffBuzz can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want FluffBuzz, use one VPS/host per user.
...
FluffBuzz is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/fluffbuzz/fluffbuzz/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, FluffBuzz also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.fluffbuzz/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.fluffbuzz/credentials/<channel>-allowFrom.json` (default account)
  - `~/.fluffbuzz/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.fluffbuzz/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.fluffbuzz/secrets.json`
- **Legacy OAuth import**: `~/.fluffbuzz/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full FluffBuzz state                                 | filesystem perms on `~/.fluffbuzz`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.fluffbuzz/fluffbuzz.json`                                                   | yes      |
...
`fluffbuzz security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`fluffbuzz security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in FluffBuzz config
schema:
...
gateway:
  ...
  auth:
    mode: password
    password: ${FLUFFBUZZ_GATEWAY_PASSWORD}
```
...
- FluffBuzz gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from FluffBuzz responses.
...
FluffBuzz stores session transcripts on disk under `~/.fluffbuzz/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.fluffbuzz` (see the audit section below). If you need
...
FluffBuzz can refresh the skills list mid-session:
...
- If you install plugins from npm (`fluffbuzz plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.fluffbuzz/extensions/<pluginId>/` (or `$FLUFFBUZZ_STATE_DIR/extensions/<pluginId>/`).
  - FluffBuzz uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
fluffbuzz pairing list <channel>
fluffbuzz pairing approve <channel> <code>
```
...
By default, FluffBuzz routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
FluffBuzz has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.fluffbuzz/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.fluffbuzz or your logs.”

## Unsafe external content bypass flags

FluffBuzz includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.fluffbuzz/fluffbuzz.json`: `600` (user read/write only)
- `~/.fluffbuzz`: `700` (user only)

`fluffbuzz doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `FLUFFBUZZ_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__fluffbuzz__/canvas/` and `/__fluffbuzz__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_fluffbuzz-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `FLUFFBUZZ_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `fluffbuzz doctor --generate-gateway-token`.
...
paths, set `FLUFFBUZZ_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `FLUFFBUZZ_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `FLUFFBUZZ_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), FluffBuzz
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. FluffBuzz verifies the identity by resolving the
...
- FluffBuzz will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.fluffbuzz/` (or `$FLUFFBUZZ_STATE_DIR/`) may contain secrets or private data:

- `fluffbuzz.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `fluffbuzz status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@fluffbuzz", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.fluffbuzz`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.fluffbuzz/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `fluffbuzz` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only FluffBuzz clients can connect.
- Disable browser proxy routing when you don’t need it (`gateway.nodes.browser.mode="off"`).
- Chrome extension relay mode is **not** “safer”; it can take over your existing Chrome tabs. Assume it can act as you in whatever that tab/profile can reach.

### Browser SSRF policy (trusted-network default)

FluffBuzz’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default FluffBuzz limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `fluffbuzz gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `FLUFFBUZZ_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/fluffbuzz/fluffbuzz-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.fluffbuzz/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `fluffbuzz security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + FluffBuzz version
...
Found a vulnerability in FluffBuzz? Please report responsibly:

1. Email: [security@fluffbuzz.com](mailto:security@fluffbuzz.com)
