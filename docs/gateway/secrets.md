...
FluffBuzz supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, FluffBuzz runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). FluffBuzz validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/FluffBuzz QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/fluffbuzz"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `fluffbuzz.json` refs)
...
When reload-time activation fails after a healthy state, FluffBuzz enters degraded secrets state.
...
Credential-sensitive command paths that opt in (for example `fluffbuzz memory` remote-memory paths and `fluffbuzz qr --remote`) can resolve supported SecretRefs via gateway snapshot RPC.
...
- Snapshot refresh after backend secret rotation is handled by `fluffbuzz secrets reload`.
...
- plaintext values at rest (`fluffbuzz.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `fluffbuzz.json` refs)
...
- lets you select supported secret-bearing fields in `fluffbuzz.json` plus `auth-profiles.json` for one agent scope
...
- `fluffbuzz secrets configure --agent <id>`
...
FluffBuzz intentionally does not write rollback backups containing historical plaintext secret values.
