...
FluffBuzz supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, FluffBuzz performs a fast preflight check before saving:
...
- lets you select secret-bearing fields in `fluffbuzz.json`
...
FluffBuzz intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, FluffBuzz runtime no longer depends on plaintext `auth.json`.
