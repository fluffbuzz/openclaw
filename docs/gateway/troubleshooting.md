...
- `fluffbuzz gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `fluffbuzz doctor` reports no blocking config/service issues.
- `fluffbuzz channels status --probe` shows connected/ready channels.
...
fluffbuzz config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `fluffbuzz configure`). If you are running FluffBuzz via Podman using the dedicated `fluffbuzz` user, the config lives at `~fluffbuzz/.fluffbuzz/fluffbuzz.json`.
...
fluffbuzz system heartbeat last
fluffbuzz logs --follow
```
...
fluffbuzz approvals get --node <idOrNameOrIp>
fluffbuzz logs --follow
fluffbuzz status
```
...
fluffbuzz browser start --browser-profile fluffbuzz
...
```bash
fluffbuzz gateway status
fluffbuzz config get gateway.mode
fluffbuzz config get gateway.remote.url
fluffbuzz config get gateway.auth.mode
```
...
```bash
fluffbuzz config get gateway.bind
