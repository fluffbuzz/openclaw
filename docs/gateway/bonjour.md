...
FluffBuzz uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
- `transport=gateway`
- `cliPath=<path>` (optional; absolute path to a runnable `fluffbuzz` entrypoint)
...
- `FLUFFBUZZ_SSH_PORT` overrides the SSH port advertised in TXT (legacy: `FLUFFBUZZ_SSH_PORT`).
