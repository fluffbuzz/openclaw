...
read_when:
  - Generating or reviewing `fluffbuzz secrets apply` plan files
...
This page defines the strict contract enforced by `fluffbuzz secrets apply`.
...
If apply fails with an invalid target path message, regenerate the plan with `fluffbuzz secrets configure` or fix the target path to one of the allowed shapes above.
