...
FluffBuzz can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere FluffBuzz can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, FluffBuzz defaults
...
TTS config lives under `messages.tts` in `fluffbuzz.json`.
...
- If `provider` is **unset**, FluffBuzz prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.fluffbuzz/settings/tts.json`, override with `FLUFFBUZZ_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, FluffBuzz retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, FluffBuzz:
...
Discord note: `/tts` is a built-in Discord command, so FluffBuzz registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from FluffBuzz
```
