...
This plan defines how FluffBuzz should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- FluffBuzz core owns ACP control plane concerns
...
FluffBuzz should not reimplement ACP transport internals in core.
FluffBuzz should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in FluffBuzz, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under FluffBuzz state dir
...
Core (must be in FluffBuzz core):
...
- ACP event stream is normalized to FluffBuzz reply chunks
...
For the first production backend (`extensions/acpx`), FluffBuzz and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses FluffBuzz `sessionKey`)
...
- FluffBuzz consumes ndjson events from `acpx --format json --json-strict`
