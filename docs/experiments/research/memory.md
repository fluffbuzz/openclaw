...
read_when:
  - Designing workspace memory (~/.fluffbuzz/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep FluffBuzz integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.fluffbuzz/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.fluffbuzz/workspace` as canonical human-readable memory.
...
```
~/.fluffbuzz/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in FluffBuzz**, but keep a separable core library.

### Why integrate into FluffBuzz?

- FluffBuzz already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `fluffbuzz memory recall "…" --k 25 --since 30d`
  - `fluffbuzz memory reflect --since 7d`
...
Pragmatic take for `~/.fluffbuzz/workspace`:
