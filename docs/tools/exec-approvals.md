...
- **gateway host** → `fluffbuzz` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.fluffbuzz/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.fluffbuzz/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `fluffbuzz approvals allowlist ...`).
- `fluffbuzz security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `fluffbuzz doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.fluffbuzz/exec-approvals.json` directly.

CLI: `fluffbuzz approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
