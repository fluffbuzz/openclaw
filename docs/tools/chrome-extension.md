---
summary: "Chrome extension: let FluffBuzz drive your existing Chrome tab"
read_when:
...
The FluffBuzz Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate fluffbuzz-managed Chrome profile.
...
FluffBuzz then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the FluffBuzz release (npm package) as static files. There is no separate “build” step.

After upgrading FluffBuzz:

- Re-run `fluffbuzz browser extension install` to refresh the installed files under your FluffBuzz state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

FluffBuzz ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `FLUFFBUZZ_GATEWAY_TOKEN`)
...
- Open the tab you want FluffBuzz to control.
...
- `ON`: attached; FluffBuzz can drive that tab.
...
Debugging: `fluffbuzz sandbox explain`
...
`fluffbuzz browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `fluffbuzz browser extension install` first to copy the extension to a stable location under your FluffBuzz state directory.
...
- **This is not isolated** like the dedicated fluffbuzz-managed profile.
