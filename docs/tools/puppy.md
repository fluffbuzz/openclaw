---
title: Puppy
summary: "Typed workflow runtime for FluffBuzz with resumable approval gates."
...
FluffBuzz launches the local `puppy` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Install the Puppy CLI on the **same host** that runs the FluffBuzz Gateway (see the [Puppy repo](https://github.com/fluffbuzz/puppy)), and ensure `puppy` is on `PATH`.
...
- **Hardened** — fixed executable name (`puppy`) on `PATH`; timeouts and output caps enforced.

## Troubleshooting

- **`puppy subprocess timed out`** → increase `timeoutMs`, or split a long pipeline.
...
- **`puppy failed (code …)`** → run the same pipeline in a terminal to inspect stderr.
