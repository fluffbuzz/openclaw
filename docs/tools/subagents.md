...
- For manual spawns, delivery is resilient:
  - FluffBuzz tries direct `agent` delivery first with a stable idempotency key.
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, FluffBuzz uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. FluffBuzz creates or binds a thread to that session target in the active channel.
