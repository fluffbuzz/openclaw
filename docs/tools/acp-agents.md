...
```bash
fluffbuzz plugins install @fluffbuzz/acpx
...
`@fluffbuzz/acpx` now enforces a strict plugin-local pinning model:
...
- FluffBuzz startup stays non-blocking while acpx ensure runs.
