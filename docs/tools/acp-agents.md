...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let FluffBuzz run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask FluffBuzz in plain language to "run this in Codex" or "start Claude Code in a thread", FluffBuzz should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want FluffBuzz-native delegated runs.
...
- FluffBuzz binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, FluffBuzz returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, FluffBuzz uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, FluffBuzz may default to persistent behavior per runtime path
...
ACP sessions currently run on the host runtime, not inside the FluffBuzz sandbox.
...
If no target resolves, FluffBuzz returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, FluffBuzz returns a clear unsupported-control error.
...
When FluffBuzz uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
```bash
fluffbuzz plugins install acpx
fluffbuzz config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
fluffbuzz plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@fluffbuzz/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from FluffBuzz workspace directory.
...
- FluffBuzz startup remains non-blocking while the backend health check runs.
...
```bash
fluffbuzz config set plugins.entries.acpx.config.permissionMode approve-all
fluffbuzz config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** FluffBuzz currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
