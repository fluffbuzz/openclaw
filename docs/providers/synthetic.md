---
summary: "Use Synthetic's Anthropic-compatible API in FluffBuzz"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. FluffBuzz registers it as the
...
Note: FluffBuzz's Anthropic client appends `/v1` to the base URL, so use
