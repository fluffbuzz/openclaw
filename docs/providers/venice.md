---
summary: "Use Venice AI privacy-focused models in FluffBuzz"
read_when:
  - You want privacy-focused inference in FluffBuzz
...
```bash
fluffbuzz onboard --auth-choice venice-api-key
```
...
```bash
fluffbuzz agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```
...
```bash
fluffbuzz models set venice/claude-opus-45
fluffbuzz models set venice/llama-3.3-70b
```

List all available models:

```bash
fluffbuzz models list | grep venice
```

## Configure via `fluffbuzz configure`

1. Run `fluffbuzz configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FluffBuzz automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
fluffbuzz agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
fluffbuzz agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
fluffbuzz agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
fluffbuzz agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
fluffbuzz agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
