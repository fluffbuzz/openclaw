---
summary: "Use MiniMax M2.5 in FluffBuzz"
read_when:
  - You want MiniMax models in FluffBuzz
...
- **Compatibility:** FluffBuzz still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
fluffbuzz plugins enable minimax-portal-auth  # skip if already loaded.
fluffbuzz gateway restart  # restart if gateway is already running
fluffbuzz onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/fluffbuzz/fluffbuzz/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `fluffbuzz.json`:
...
- Use `fluffbuzz models list` and `fluffbuzz models set minimax/MiniMax-M2.5` to switch.
...
- Running `fluffbuzz configure` and selecting **MiniMax M2.5**, or
