...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like FluffBuzz.
...
FluffBuzz uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), FluffBuzz also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. FluffBuzz enables it by default for
...
`baseUrl` on `api.openai.com`), FluffBuzz now auto-enables OpenAI server-side
