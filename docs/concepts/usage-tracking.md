...
- `/usage cost` in chats: local cost summary aggregated from FluffBuzz session logs.
- CLI: `fluffbuzz status --usage` prints a full per-provider breakdown.
- CLI: `fluffbuzz channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
