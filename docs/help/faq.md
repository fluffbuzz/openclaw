---
summary: "Frequently asked questions about FluffBuzz setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up FluffBuzz?](#whats-the-recommended-way-to-install-and-set-up-fluffbuzz)
  ...
  - [I can't access docs.fluffbuzz.com (SSL error). What now?](#i-cant-access-docsfluffbuzzai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or fluffbuzz not recognized](#windows-install-says-git-not-found-or-fluffbuzz-not-recognized)
  ...
  - [How do I install FluffBuzz on Linux?](#how-do-i-install-fluffbuzz-on-linux)
  - [How do I install FluffBuzz on a VPS?](#how-do-i-install-fluffbuzz-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask FluffBuzz to update itself?](#can-i-ask-fluffbuzz-to-update-itself)
  ...
  - [If I buy a Mac mini to run FluffBuzz, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-fluffbuzz-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different FluffBuzz instances?](#can-multiple-people-use-one-whatsapp-number-with-different-fluffbuzz-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run FluffBuzz on a dedicated machine?](#how-important-is-it-to-run-fluffbuzz-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run FluffBuzz in a VM and what are the requirements](#can-i-run-fluffbuzz-in-a-vm-and-what-are-the-requirements)
- [What is FluffBuzz?](#what-is-fluffbuzz)
  - [What is FluffBuzz, in one paragraph?](#what-is-fluffbuzz-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for FluffBuzz](#what-are-the-top-five-everyday-use-cases-for-fluffbuzz)
  - [Can FluffBuzz help with lead gen outreach ads and blogs for a SaaS](#can-fluffbuzz-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can FluffBuzz run tasks on a schedule or continuously in the background?](#can-fluffbuzz-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with FluffBuzz saved locally?](#is-all-data-used-with-fluffbuzz-saved-locally)
  - [Where does FluffBuzz store its data?](#where-does-fluffbuzz-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall FluffBuzz?](#how-do-i-completely-uninstall-fluffbuzz)
...
- [Config basics](#config-basics)
  ...
  - [Can the FluffBuzz browser run headless?](#can-the-fluffbuzz-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two FluffBuzz instances talk to each other (local + VPS)?](#can-two-fluffbuzz-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does FluffBuzz load environment variables?](#how-does-fluffbuzz-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of FluffBuzz instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-fluffbuzz-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset FluffBuzz but keep it installed?](#how-do-i-completely-reset-fluffbuzz-but-keep-it-installed)
  ...
  - [Why doesn't FluffBuzz reply in a group?](#why-doesnt-fluffbuzz-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do FluffBuzz, Flawd, and Krill use for models?](#what-do-fluffbuzz-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `fluffbuzz gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-fluffbuzz-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `fluffbuzz gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-fluffbuzz-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run FluffBuzz in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-fluffbuzz-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart FluffBuzz?](#i-closed-my-terminal-on-windows-how-do-i-restart-fluffbuzz)
  ...
  - [ELI5: `fluffbuzz gateway restart` vs `fluffbuzz gateway`](#eli5-fluffbuzz-gateway-restart-vs-fluffbuzz-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose FluffBuzz to inbound DMs?](#is-it-safe-to-expose-fluffbuzz-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/fluffbuzz/fluffbuzz-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   fluffbuzz health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://fluffbuzz.com/install.sh | bash -s -- --install-method git
```

This installs FluffBuzz **from a git checkout**, so the agent can read the code + docs and
...
[https://github.com/fluffbuzz/fluffbuzz/issues](https://github.com/fluffbuzz/fluffbuzz/issues)
[https://github.com/fluffbuzz/fluffbuzz/pulls](https://github.com/fluffbuzz/fluffbuzz/pulls)

Start with these commands (share outputs when asking for help):

```bash
fluffbuzz status
fluffbuzz models status
fluffbuzz doctor
```

What they do:

- `fluffbuzz status`: quick snapshot of gateway/agent health + basic config.
- `fluffbuzz models status`: checks provider auth + model availability.
- `fluffbuzz doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `fluffbuzz status --all`, `fluffbuzz logs --follow`,
`fluffbuzz gateway status`, `fluffbuzz health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up FluffBuzz
...
If you don't have a global install yet, run it via `pnpm fluffbuzz onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `FLUFFBUZZ_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `fluffbuzz config get gateway.auth.token` (or generate one: `fluffbuzz doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `fluffbuzz gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `fluffbuzz gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install FluffBuzz on the new machine.
2. Copy `$FLUFFBUZZ_STATE_DIR` (default: `~/.fluffbuzz`) from the old machine.
3. Copy your workspace (default: `~/.fluffbuzz/workspace`).
4. Run `fluffbuzz doctor` and restart the Gateway service.
...
under `~/.fluffbuzz/` (for example `~/.fluffbuzz/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-fluffbuzz-store-its-data),
...
[https://github.com/fluffbuzz/fluffbuzz/blob/main/CHANGELOG.md](https://github.com/fluffbuzz/fluffbuzz/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I can't access docs.fluffbuzz.com SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.fluffbuzz.com` via Xfinity
Advanced Security. Disable it or allowlist `docs.fluffbuzz.com`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsfluffbuzzai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/fluffbuzz/fluffbuzz/tree/main/docs](https://github.com/fluffbuzz/fluffbuzz/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://fluffbuzz.com/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://fluffbuzz.com/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://fluffbuzz.com/install.ps1](https://fluffbuzz.com/install.ps1)
...
```bash
curl -fsSL https://fluffbuzz.com/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://fluffbuzz.com/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://fluffbuzz.com/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://fluffbuzz.com/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or fluffbuzz not recognized
...
**2) fluffbuzz is not recognized after install**
...
If you still reproduce this on latest FluffBuzz, track/report it in:

- [Issue #30640](https://github.com/fluffbuzz/fluffbuzz/issues/30640)
...
### How do I install FluffBuzz on Linux
...
### How do I install FluffBuzz on a VPS
...
### Can I ask FluffBuzz to update itself
...
```bash
fluffbuzz update
fluffbuzz update status
fluffbuzz update --channel stable|beta|dev
fluffbuzz update --tag <dist-tag|version>
fluffbuzz update --no-restart
```

If you must automate from an agent:

```bash
fluffbuzz update --yes --no-restart
fluffbuzz gateway restart
```
...
`fluffbuzz onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run FluffBuzz with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like FluffBuzz.
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `fluffbuzz models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `fluffbuzz models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `fluffbuzz models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. FluffBuzz no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so FluffBuzz can keep replying while a provider is rate-limited.
...
FluffBuzz supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. FluffBuzz fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like FluffBuzz. The onboarding wizard can run the OAuth flow for you.

See [OAuth](/concepts/oauth), [Model providers](/concepts/model-providers), and [Wizard](/start/wizard).

### How do I set up Gemini CLI OAuth

Gemini CLI uses a **plugin auth flow**, not a client id or secret in `fluffbuzz.json`.

Steps:

1. Enable the plugin: `fluffbuzz plugins enable google-gemini-cli-auth`
2. Login: `fluffbuzz models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. FluffBuzz needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. FluffBuzz runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run FluffBuzz can I connect it to my MacBook Pro
...
- Use `fluffbuzz nodes status` / `fluffbuzz nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but FluffBuzz authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `fluffbuzz logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different FluffBuzz instances
...
If you run FluffBuzz via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the FluffBuzz code install. Your state
(`~/.fluffbuzz`) and workspace (`~/.fluffbuzz/workspace`) stay untouched.
...
**FluffBuzz-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run FluffBuzz on a dedicated machine
...
FluffBuzz is lightweight. For a basic Gateway + one chat channel:
...
### Can I run FluffBuzz in a VM and what are the requirements
...
### What is FluffBuzz in one paragraph

FluffBuzz is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

FluffBuzz is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for FluffBuzz
...
### Can FluffBuzz help with lead gen outreach ads and blogs for a SaaS
...
FluffBuzz draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

FluffBuzz is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use FluffBuzz when you
...
Showcase: [https://fluffbuzz.com/showcase](https://fluffbuzz.com/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.fluffbuzz/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.fluffbuzz/fluffbuzz.json`). Precedence is `<workspace>/skills` > `~/.fluffbuzz/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.fluffbuzz/fluffbuzz.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.fluffbuzz/skills` → bundled → `skills.load.extraDirs`. `buzzhub` installs into `./skills` by default, which FluffBuzz treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `FLUFFBUZZ_SKIP_CRON` is not set.
...
```bash
fluffbuzz cron run <jobId> --force
fluffbuzz cron runs --id <jobId> --limit 50
```
...
Browse skills at [https://buzzhub.com](https://buzzhub.com).
...
### Can FluffBuzz run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.fluffbuzz.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-fluffbuzz-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. FluffBuzz can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.fluffbuzz/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "fluffbuzz": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
```bash
buzzhub install <skill-slug>
buzzhub update --all
```

ClawHub installs into `./skills` under your current directory (or falls back to your configured FluffBuzz workspace); FluffBuzz treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.fluffbuzz/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/buzzhub).
...
- Persist `/home/node` with `FLUFFBUZZ_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `FLUFFBUZZ_DOCKER_APT_PACKAGES`.
...
FluffBuzz memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

FluffBuzz also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, FluffBuzz auto-selects a provider when it
...
configured and present, FluffBuzz
...
### Is all data used with FluffBuzz saved locally

No - **FluffBuzz's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.fluffbuzz` + your workspace directory).
...
### Where does FluffBuzz store its data

Everything lives under `$FLUFFBUZZ_STATE_DIR` (default: `~/.fluffbuzz`):

| Path                                                            | Purpose                                                            |
| --------------------------------------------------------------- | ------------------------------------------------------------------ |
| `$FLUFFBUZZ_STATE_DIR/fluffbuzz.json`                             | Main config (JSON5)                                                |
| `$FLUFFBUZZ_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
| `$FLUFFBUZZ_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
| `$FLUFFBUZZ_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
| `$FLUFFBUZZ_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
| `$FLUFFBUZZ_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
| `$FLUFFBUZZ_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
| `$FLUFFBUZZ_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
| `$FLUFFBUZZ_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

Legacy single-agent path: `~/.fluffbuzz/agent/*` (migrated by `fluffbuzz doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.fluffbuzz/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.fluffbuzz`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.fluffbuzz`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.fluffbuzz/skills`).

Default workspace is `~/.fluffbuzz/workspace`, configurable via:
...
Do **not** commit anything under `~/.fluffbuzz` (credentials, sessions, tokens, or encrypted secrets payloads).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall FluffBuzz
...
`workspace` to the repo root. The FluffBuzz repo is just source code; keep the
...
FluffBuzz reads an optional **JSON5** config from `$FLUFFBUZZ_CONFIG_PATH` (default: `~/.fluffbuzz/fluffbuzz.json`):

```
$FLUFFBUZZ_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.fluffbuzz/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `FLUFFBUZZ_GATEWAY_TOKEN`).
...
FluffBuzz enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `fluffbuzz doctor --generate-gateway-token`.
...
- `default`: uses `All your chats, one FluffBuzz.` every time.
- `random`: rotating funny/seasonal taglines (default behavior).
- If you want no banner at all, set env `FLUFFBUZZ_HIDE_BANNER=1`.

### How do I enable web search and web fetch

`web_fetch` works without an API key. `web_search` requires a Brave Search API
key. **Recommended:** run `fluffbuzz configure --section web` to store it in
...
- Daemons read env vars from `~/.fluffbuzz/.env` (or the service environment).
...
### Can the FluffBuzz browser run headless
...
- Gateway is running: `fluffbuzz gateway status`
- Gateway health: `fluffbuzz status`
- Channel health: `fluffbuzz channels status`
...
### Can two FluffBuzz instances talk to each other local VPS
...
`fluffbuzz agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
fluffbuzz agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.fluffbuzz/fluffbuzz.json`).
- If you have no backup, re-run `fluffbuzz doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `fluffbuzz config set` for small changes.
- Use `fluffbuzz configure` for interactive edits.
...
```bash
fluffbuzz gateway --tailscale serve
```
...
### How does FluffBuzz load environment variables

FluffBuzz reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.fluffbuzz/.env` (aka `$FLUFFBUZZ_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.fluffbuzz/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`FLUFFBUZZ_LOAD_SHELL_ENV=1`, `FLUFFBUZZ_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`fluffbuzz models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means FluffBuzz won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.fluffbuzz/.env`:
...
### Is there a way to make a team of FluffBuzz instances one CEO and many agents
...
### How do I completely reset FluffBuzz but keep it installed
...
- If you used profiles (`--profile` / `FLUFFBUZZ_PROFILE`), reset each state dir (defaults are `~/.fluffbuzz-<profile>`).
- Dev reset: `fluffbuzz gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), FluffBuzz skips the heartbeat run to save API calls.
...
No. FluffBuzz runs on **your own account**, so if you're in the group, FluffBuzz can see it.
...
```bash
fluffbuzz directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesn't FluffBuzz reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.fluffbuzz/agents/<agentId>/sessions/`.
...
- Use `fluffbuzz doctor` to spot stray workspaces and profile mismatches.
...
FluffBuzz's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, FluffBuzz currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `fluffbuzz models set ...` (updates just model config)
- `fluffbuzz configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.fluffbuzz/fluffbuzz.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `fluffbuzz doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do FluffBuzz, Flawd, and Krill use for models

- These deployments can differ and may change over time; there is no fixed provider recommendation.
- Check the current runtime setting on each gateway with `fluffbuzz models status`.
...
Yes. FluffBuzz ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.fluffbuzz/agents/<agentId>/agent/auth-profiles.json
```

Fix options:

- Run `fluffbuzz agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so FluffBuzz can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.fluffbuzz/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.fluffbuzz/agent/*` (migrated by `fluffbuzz doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.fluffbuzz/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `fluffbuzz models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `fluffbuzz models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `fluffbuzz models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.fluffbuzz/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:

    ```bash
    fluffbuzz models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), FluffBuzz will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: FluffBuzz now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
FluffBuzz uses provider-prefixed IDs like:
...
FluffBuzz may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `fluffbuzz models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
fluffbuzz models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
fluffbuzz models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
fluffbuzz models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
fluffbuzz models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key what's the difference

FluffBuzz supports both:
...
```
--port > FLUFFBUZZ_GATEWAY_PORT > gateway.port > default 18789
```

### Why does fluffbuzz gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `fluffbuzz gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does fluffbuzz gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `FLUFFBUZZ_STATE_DIR` mismatch).

Fix:

```bash
fluffbuzz gateway install --force
```
...
FluffBuzz enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `fluffbuzz gateway --port <port>`.

### How do I run FluffBuzz in remote mode client connects to a Gateway elsewhere
...
- `fluffbuzz gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `fluffbuzz.control.settings.v1`.

Fix:

- Fastest: `fluffbuzz dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `fluffbuzz doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `FLUFFBUZZ_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- Still stuck? Run `fluffbuzz status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `FLUFFBUZZ_CONFIG_PATH` (per-instance config)
- `FLUFFBUZZ_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `fluffbuzz --profile <name> …` per instance (auto-creates `~/.fluffbuzz-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `fluffbuzz --profile <name> gateway install`.

Profiles also suffix service names (`com.fluffbuzz.<profile>`; legacy `com.fluffbuzz.*`, `fluffbuzz-gateway-<profile>.service`, `FluffBuzz Gateway (<profile>)`).
...
```
fluffbuzz tui --url ws://<host>:18789 --token <token>
```
...
- macOS: `$FLUFFBUZZ_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.fluffbuzz/logs/...`; profiles use `~/.fluffbuzz-<profile>/logs/...`)
- Linux: `journalctl --user -u fluffbuzz-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "FluffBuzz Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `fluffbuzz gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart FluffBuzz
...
1. Is the Gateway running? `fluffbuzz gateway status`
2. Is the Gateway healthy? `fluffbuzz status`
3. Does the UI have the right token? `fluffbuzz dashboard`
...
```bash
fluffbuzz channels status
fluffbuzz channels logs --channel telegram
```
...
### ELI5 fluffbuzz gateway restart vs fluffbuzz gateway

- `fluffbuzz gateway restart`: restarts the **background service** (launchd/systemd).
- `fluffbuzz gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `fluffbuzz gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [FluffBuzz assistant setup](/start/fluffbuzz) and [Agent send](/tools/agent-send).

CLI sending:

```bash
fluffbuzz message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose FluffBuzz to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `fluffbuzz pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `fluffbuzz pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `fluffbuzz doctor` to surface risky DM policies.
...
```bash
fluffbuzz pairing list telegram
```
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. FluffBuzz only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
fluffbuzz pairing approve whatsapp <code>
```

List pending requests:

```bash
fluffbuzz pairing list whatsapp
```
...
FluffBuzz blocks **cross-provider** messaging by default. If a tool call is bound
