...
Yes. FluffBuzz fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Usually no. FluffBuzz needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
5. Approve the node on the Gateway:
   ...
   ```bash
   fluffbuzz nodes pending
   fluffbuzz nodes approve <requestId>
   ```
...
- **FluffBuzz + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
