...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    expect(log).toContain("run --rm fluffbuzz-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm fluffbuzz-cli config set gateway.mode local");
    expect(log).toContain("run --rm fluffbuzz-cli config set gateway.bind lan");
  });
  ...
  it("treats FLUFFBUZZ_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("FLUFFBUZZ_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg FLUFFBUZZ_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg FLUFFBUZZ_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      FLUFFBUZZ_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker fluffbuzz-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        FLUFFBUZZ_SANDBOX: "1",
        FLUFFBUZZ_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("fluffbuzz-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps fluffbuzz-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate fluffbuzz-gateway"));
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:fluffbuzz-gateway"');
