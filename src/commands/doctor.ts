...
function resolveMode(cfg: FluffBuzzConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("FluffBuzz doctor");

  const root = await resolveFluffBuzzPackageRoot({
  ...
  let cfg: FluffBuzzConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("fluffbuzz configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("fluffbuzz config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("fluffbuzz setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("fluffbuzz doctor --fix")}" to apply changes.`);
  }
