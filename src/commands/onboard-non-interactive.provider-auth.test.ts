...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "fluffbuzz.json");
  ...
  try {
    await withEnvAsync(
      {
        HOME: tempHome,
        FLUFFBUZZ_STATE_DIR: tempHome,
        FLUFFBUZZ_CONFIG_PATH: configPath,
        FLUFFBUZZ_SKIP_CHANNELS: "1",
        FLUFFBUZZ_SKIP_GMAIL_WATCHER: "1",
        FLUFFBUZZ_SKIP_CRON: "1",
        FLUFFBUZZ_SKIP_CANVAS_HOST: "1",
        FLUFFBUZZ_GATEWAY_TOKEN: undefined,
        FLUFFBUZZ_GATEWAY_PASSWORD: undefined,
        CUSTOM_API_KEY: undefined,
        FLUFFBUZZ_DISABLE_CONFIG_CACHE: "1",
      },
      async () => {
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("fluffbuzz-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("fluffbuzz-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("fluffbuzz-onboard-zai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("fluffbuzz-onboard-zai-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-xai-", async (env) => {
  ...
  it("infers Mistral auth choice from --mistral-api-key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-mistral-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Volcano Engine API key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-volcengine-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers BytePlus auth choice from --byteplus-api-key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-byteplus-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("fluffbuzz-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      name: "anthropic",
      prefix: "fluffbuzz-onboard-ref-flag-anthropic-",
      authChoice: "apiKey",
    ...
    {
      name: "openai",
      prefix: "fluffbuzz-onboard-ref-flag-openai-",
      authChoice: "openai-api-key",
    ...
    {
      name: "openrouter",
      prefix: "fluffbuzz-onboard-ref-flag-openrouter-",
    ...
    {
      name: "xai",
      prefix: "fluffbuzz-onboard-ref-flag-xai-",
      authChoice: "xai-api-key",
      optionKey: "xaiApiKey",
    ...
    {
      name: "volcengine",
      prefix: "fluffbuzz-onboard-ref-flag-volcengine-",
      authChoice: "volcengine-api-key",
    ...
    {
      name: "byteplus",
      prefix: "fluffbuzz-onboard-ref-flag-byteplus-",
  ...
  it("stores the detected env alias as keyRef for opencode ref mode", async () => {
    await withOnboardEnv("fluffbuzz-onboard-ref-opencode-alias-", async ({ runtime }) => {
      await withEnvAsync(
        {
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("fluffbuzz-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-litellm-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      ...
      prefix: "fluffbuzz-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "fluffbuzz-onboard-cf-gateway-infer-",
      options: {},
    },
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("fluffbuzz-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("fluffbuzz-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveOnboardingWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "fluffbuzz-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "fluffbuzz-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("stores CUSTOM_API_KEY env ref for non-interactive custom provider auth in ref mode", async () => {
    await withOnboardEnv(
      "fluffbuzz-onboard-custom-provider-env-ref-",
      async ({ configPath, runtime }) => {
        process.env.CUSTOM_API_KEY = "custom-env-key";
        await runCustomLocalNonInteractive(runtime, {
          secretInputMode: "ref",
  ...
  it("fails fast for custom provider ref mode when --custom-api-key is set but CUSTOM_API_KEY env is missing", async () => {
    await withOnboardEnv("fluffbuzz-onboard-custom-provider-ref-flag-", async ({ runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "fluffbuzz-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
        upsertAuthProfile({
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "fluffbuzz-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            authChoice: "custom-api-key",
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("fluffbuzz-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "fluffbuzz-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractiveOnboardingWithDefaults(runtime, {
            customApiKey: "custom-test-key",
