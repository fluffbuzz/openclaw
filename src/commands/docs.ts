...
const SEARCH_TOOL = "https://docs.fluffbuzz.com/mcp.SearchFluffBuzz";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.fluffbuzz.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('fluffbuzz docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.fluffbuzz.com/");
      runtime.log(`Search: ${formatCliCommand('fluffbuzz docs "your query"')}`);
    }
    return;
  }
