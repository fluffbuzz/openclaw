...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("fluffbuzz pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`fluffbuzz pairing approve ${policy.channel} <code>`)}`,
    ...
    if (!getChannelPlugin(channel)) {
      ...
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("fluffbuzz plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("fluffbuzz channels add")}\``,
        },
      ],
