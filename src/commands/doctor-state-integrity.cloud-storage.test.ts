...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "FluffBuzz", ".fluffbuzz");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/fluffbuzz-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".fluffbuzz");
  ...
  it("anchors cloud detection to OS homedir when FLUFFBUZZ_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".fluffbuzz");
    const originalFluffBuzzHome = process.env.FLUFFBUZZ_HOME;
    process.env.FLUFFBUZZ_HOME = "/tmp/fluffbuzz-home-override";
    ...
    try {
      ...
      if (originalFluffBuzzHome === undefined) {
        delete process.env.FLUFFBUZZ_HOME;
      } else {
        process.env.FLUFFBUZZ_HOME = originalFluffBuzzHome;
      }
    }
  });
