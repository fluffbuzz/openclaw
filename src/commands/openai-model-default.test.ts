...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: FluffBuzzConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: FluffBuzzConfig) => { changed: boolean; next: FluffBuzzConfig };
  defaultModel: string;
  overrideConfig: FluffBuzzConfig;
  alreadyDefaultConfig: FluffBuzzConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: FluffBuzzConfig) => config,
      applyDefaultConfig: (config: FluffBuzzConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: FluffBuzzConfig = { agents: { defaults: {} } };
