...
function prependInternalEventContext(
  ...
  if (body.includes("FluffBuzz runtime context (internal):")) {
    return body;
  }
...
async function agentCommandInternal(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("fluffbuzz agents list")}" to see configured agents.`,
      );
    }
  }
