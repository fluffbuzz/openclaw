...
export async function statusCommand(
  ...
  runtime.log(theme.heading("FluffBuzz status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`fluffbuzz devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("fluffbuzz devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("fluffbuzz devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("fluffbuzz security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("fluffbuzz security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.fluffbuzz.com/faq");
  runtime.log("Troubleshooting: https://docs.fluffbuzz.com/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("fluffbuzz status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("fluffbuzz logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("fluffbuzz status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("fluffbuzz gateway probe")}`);
  }
}
