...
export function normalizeCompatibilityConfigValues(cfg: FluffBuzzConfig): {
  config: FluffBuzzConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: FluffBuzzConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as FluffBuzzConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as FluffBuzzConfig["browser"],
    };
    changes.push(
