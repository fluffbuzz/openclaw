...
beforeAll(() => {
  envSnapshot = captureEnv(["FLUFFBUZZ_PROFILE"]);
  process.env.FLUFFBUZZ_PROFILE = "isolated";
});
...
vi.mock("../infra/fluffbuzz-root.js", () => ({
  resolveFluffBuzzPackageRoot: vi.fn().mockResolvedValue("/tmp/fluffbuzz"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/fluffbuzz/pnpm-lock.yaml",
      markerPath: "/tmp/fluffbuzz/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.fluffbuzz.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.fluffbuzz.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("FluffBuzz status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("fluffbuzz status --all") ||
          l.includes("fluffbuzz --profile isolated status --all") ||
          l.includes("fluffbuzz status --all") ||
          l.includes("fluffbuzz --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = "abcd1234";
    try {
