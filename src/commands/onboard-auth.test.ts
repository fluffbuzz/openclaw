...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FLUFFBUZZ_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("fluffbuzz-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-oauth-sync-"));
    ...
    process.env.FLUFFBUZZ_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-oauth-nosync-"));
    process.env.FLUFFBUZZ_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside FLUFFBUZZ_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-oauth-external-"));
    process.env.FLUFFBUZZ_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* FLUFFBUZZ_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FLUFFBUZZ_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("fluffbuzz-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
