...
beforeEach(() => {
  envSnapshot = captureEnv(["FLUFFBUZZ_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-auth-"));
...
describe("maybeRemoveDeprecatedCliAuthProfiles", () => {
  it("removes deprecated CLI auth profiles from store + config", async () => {
    ...
    const next = await maybeRemoveDeprecatedCliAuthProfiles(
      cfg as unknown as FluffBuzzConfig,
      makePrompter(true),
    );
