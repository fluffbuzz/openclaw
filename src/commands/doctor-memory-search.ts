 ...
 * Runs as part of `fluffbuzz doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("fluffbuzz memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("fluffbuzz config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("fluffbuzz memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("fluffbuzz configure --section model")}`,
        `- To disable: ${formatCliCommand("fluffbuzz config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("fluffbuzz memory status --deep")}`,
      ].join("\n"),
      "Memory search",
