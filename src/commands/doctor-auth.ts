...
function pruneAuthProfiles(
  cfg: FluffBuzzConfig,
  profileIds: Set<string>,
): { next: FluffBuzzConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("fluffbuzz models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("fluffbuzz models auth setup-token")} or ${formatCliCommand(
      "fluffbuzz configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "fluffbuzz models auth login --provider openai-codex",
    )} or ${formatCliCommand("fluffbuzz configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("fluffbuzz configure")}\` or \`${formatCliCommand("fluffbuzz onboard")}\`.`;
}
