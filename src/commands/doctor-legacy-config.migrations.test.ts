...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.FLUFFBUZZ_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-oauth-"));
    process.env.FLUFFBUZZ_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.FLUFFBUZZ_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-wa-auth-"));
    try {
