...
type EnvSnapshot = {
  HOME?: string;
  FLUFFBUZZ_HOME?: string;
  FLUFFBUZZ_STATE_DIR?: string;
  FLUFFBUZZ_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    FLUFFBUZZ_HOME: process.env.FLUFFBUZZ_HOME,
    FLUFFBUZZ_STATE_DIR: process.env.FLUFFBUZZ_STATE_DIR,
    FLUFFBUZZ_OAUTH_DIR: process.env.FLUFFBUZZ_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: FluffBuzzConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: FluffBuzzConfig) {
...
async function runStateIntegrityText(cfg: FluffBuzzConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.FLUFFBUZZ_HOME = tempHome;
    process.env.FLUFFBUZZ_STATE_DIR = path.join(tempHome, ".fluffbuzz");
    delete process.env.FLUFFBUZZ_OAUTH_DIR;
    fs.mkdirSync(process.env.FLUFFBUZZ_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when FLUFFBUZZ_OAUTH_DIR is explicitly configured", async () => {
    process.env.FLUFFBUZZ_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: FluffBuzzConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints fluffbuzz-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: FluffBuzzConfig = {};
    ...
    expect(text).toMatch(/fluffbuzz sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/fluffbuzz sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /fluffbuzz sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
