...
export function requireFluffBuzzAgentDir(): string {
  const agentDir = process.env.FLUFFBUZZ_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FLUFFBUZZ_AGENT_DIR not set");
  }
  return agentDir;
}
