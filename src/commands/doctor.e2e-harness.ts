...
export const resolveFluffBuzzPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadFluffBuzzPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/fluffbuzz.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveFluffBuzzPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.FLUFFBUZZ_STATE_DIR;
  originalUpdateInProgress = process.env.FLUFFBUZZ_UPDATE_IN_PROGRESS;
  process.env.FLUFFBUZZ_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.FLUFFBUZZ_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.FLUFFBUZZ_UPDATE_IN_PROGRESS;
  } else {
    process.env.FLUFFBUZZ_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
