...
import { FluffBuzzSchema } from "../config/zod-schema.js";
...
function stripUnknownConfigKeys(config: FluffBuzzConfig): {
  config: FluffBuzzConfig;
  removed: string[];
} {
  const parsed = FluffBuzzSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: FluffBuzzConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: FluffBuzzConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: FluffBuzzConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: FluffBuzzConfig): Promise<{
  config: FluffBuzzConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: FluffBuzzConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: FluffBuzzConfig): {
  config: FluffBuzzConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: FluffBuzzConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: FluffBuzzConfig): {
  config: FluffBuzzConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: FluffBuzzConfig): Promise<{
  config: FluffBuzzConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: FluffBuzzConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    if (dmPolicy === "allowlist" && !hasAllowFromEntries(effectiveAllowFrom)) {
      warnings.push(
        `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("fluffbuzz doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
      );
    }
...
function collectExecSafeBinScopes(cfg: FluffBuzzConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: FluffBuzzConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: FluffBuzzConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: FluffBuzzConfig): {
  config: FluffBuzzConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: FluffBuzzConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: FluffBuzzConfig): {
  config: FluffBuzzConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".fluffbuzz");
  const targetPath = path.join(targetDir, "fluffbuzz.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".fluffbot", "fluffbot.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: FluffBuzzConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("fluffbuzz doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("fluffbuzz doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("fluffbuzz doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("fluffbuzz doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("fluffbuzz doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("fluffbuzz doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("fluffbuzz doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "fluffbuzz doctor --fix" to remove these keys.');
    }
  }
