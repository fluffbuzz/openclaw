...
type PromptDefaultModelResult = { model?: string; config?: FluffBuzzConfig };
...
function resolveConfiguredModelRaw(cfg: FluffBuzzConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: FluffBuzzConfig): string[] {
...
export function applyPrimaryModel(cfg: FluffBuzzConfig, model: string): FluffBuzzConfig {
...
export function applyModelAllowlist(cfg: FluffBuzzConfig, models: string[]): FluffBuzzConfig {
  const defaults = cfg.agents?.defaults;
