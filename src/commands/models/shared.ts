...
export async function loadValidConfigOrThrow(): Promise<FluffBuzzConfig> {
...
export async function updateConfig(
  mutator: (cfg: FluffBuzzConfig) => FluffBuzzConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FluffBuzzConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FluffBuzzConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("fluffbuzz agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
