...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveFluffBuzzAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("fluffbuzz models auth setup-token")}\` or \`${formatCliCommand("fluffbuzz configure")}\`.`
          : `Run \`${formatCliCommand("fluffbuzz configure")}\` or set an API key env var.`;
