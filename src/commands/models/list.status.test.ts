...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFluffBuzzAgentDir: vi.fn().mockReturnValue("/tmp/fluffbuzz-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/fluffbuzz-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/fluffbuzz-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveFluffBuzzAgentDir: mocks.resolveFluffBuzzAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/fluffbuzz-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveFluffBuzzAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/fluffbuzz-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/fluffbuzz-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/fluffbuzz-agent-custom");
