...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".fluffbuzz", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: FluffBuzzConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["FLUFFBOT_GATEWAY_TOKEN", await getenv("FLUFFBOT_GATEWAY_TOKEN")],
    ["FLUFFBOT_GATEWAY_PASSWORD", await getenv("FLUFFBOT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`FLUFFBUZZ_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["FLUFFBUZZ_GATEWAY_TOKEN", await getenv("FLUFFBUZZ_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["FLUFFBUZZ_GATEWAY_PASSWORD", await getenv("FLUFFBUZZ_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "FLUFFBUZZ_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("FLUFFBOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use FLUFFBUZZ_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> FLUFFBUZZ_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.FLUFFBUZZ_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- FLUFFBUZZ_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
