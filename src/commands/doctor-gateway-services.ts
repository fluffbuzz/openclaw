...
function resolveGatewayAuthToken(cfg: FluffBuzzConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.FLUFFBUZZ_GATEWAY_TOKEN ?? env.FLUFFBOT_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (fluffbot/fluffbot) now?",
      initialValue: true,
    });
    if (shouldRemove) {
      const removed: string[] = [];
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing FluffBuzz gateway next.");
      }
    }
  }
