...
function resolveAgentName(cfg: FluffBuzzConfig, agentId: string) {
...
function resolveAgentModel(cfg: FluffBuzzConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FluffBuzzConfig): AgentSummary[] {
