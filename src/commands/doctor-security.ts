...
import type { FluffBuzzConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: FluffBuzzConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("fluffbuzz security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.fluffbuzz/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("fluffbuzz approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.fluffbuzz.com/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("fluffbuzz configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("fluffbuzz config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("fluffbuzz doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("fluffbuzz config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('fluffbuzz config set session.dmScope "per-channel-peer"') +
