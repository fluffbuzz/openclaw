...
export function resolveTtsConfig(cfg: FluffBuzzConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.FLUFFBUZZ_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: FluffBuzzConfig): string | undefined {
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempRoot = resolvePreferredFluffBuzzTmpDir();
