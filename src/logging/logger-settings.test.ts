...
let originalFluffBuzzLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.FLUFFBUZZ_TEST_FILE_LOG;
  originalFluffBuzzLogLevel = process.env.FLUFFBUZZ_LOG_LEVEL;
  delete process.env.FLUFFBUZZ_TEST_FILE_LOG;
  delete process.env.FLUFFBUZZ_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.FLUFFBUZZ_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalFluffBuzzLogLevel === undefined) {
    delete process.env.FLUFFBUZZ_LOG_LEVEL;
  } else {
    process.env.FLUFFBUZZ_LOG_LEVEL = originalFluffBuzzLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
