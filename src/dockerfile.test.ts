...
describe("Dockerfile", () => {
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG FLUFFBUZZ_INSTALL_BROWSER");
  ...
  it("Docker GPG fingerprint awk uses correct quoting for FLUFFBUZZ_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
