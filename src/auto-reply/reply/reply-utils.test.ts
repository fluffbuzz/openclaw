...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bfluffbuzz\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: {
          identityName: "FluffBuzz",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[FluffBuzz] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
