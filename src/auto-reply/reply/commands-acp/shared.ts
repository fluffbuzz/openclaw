...
export function resolveConfiguredAcpBackendId(cfg: FluffBuzzConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: FluffBuzzConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `fluffbuzz plugins install ${localPath}`;
    }
    return "fluffbuzz plugins install @fluffbuzz/acpx";
  }
