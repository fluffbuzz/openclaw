...
function withAcpConfig(acp: FluffBuzzConfig["acp"]): FluffBuzzConfig {
  return { acp } as FluffBuzzConfig;
}
...
describe("ACP install hints", () => {
  it("prefers explicit runtime install command", () => {
    const cfg = withAcpConfig({
      runtime: { installCommand: "pnpm fluffbuzz plugins install acpx" },
    });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("pnpm fluffbuzz plugins install acpx");
  });
  ...
  it("uses local acpx extension path when present", () => {
    ...
    expect(hint).toContain("fluffbuzz plugins install ");
  ...
  it("falls back to npm install hint for acpx when local extension is absent", () => {
    ...
    const cfg = withAcpConfig({ backend: "acpx" });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("fluffbuzz plugins install acpx");
  });
