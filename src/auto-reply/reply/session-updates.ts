...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: FluffBuzzConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: FluffBuzzConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.FLUFFBUZZ_TEST_FAST === "1") {
