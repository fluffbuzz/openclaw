...
import { scheduleGatewaySigusr1Restart, triggerFluffBuzzRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting FluffBuzz in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerFluffBuzzRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting FluffBuzz via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
