...
function createDiscordParams(commandBody: string, cfg: FluffBuzzConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: FluffBuzzConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: FluffBuzzConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: FluffBuzzConfig = baseCfg) {
