...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop fluffbuzz!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@fluffbuzz_bot", { botUsername: "fluffbuzz_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@fluffbuzz_bot", { botUsername: "fluffbuzz_bot" })).toBe(true);
