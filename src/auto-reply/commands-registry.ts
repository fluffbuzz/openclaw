...
export function isCommandEnabled(cfg: FluffBuzzConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: FluffBuzzConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as FluffBuzzConfig),
...
export function getCommandDetection(_cfg?: FluffBuzzConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FluffBuzzConfig) {
