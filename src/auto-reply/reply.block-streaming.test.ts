...
function createReplyConfig(home: string, streamMode?: "block"): FluffBuzzConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("fluffbuzz-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("FLUFFBUZZ_TEST_FAST", "1");
