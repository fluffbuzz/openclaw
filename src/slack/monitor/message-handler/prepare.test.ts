...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: FluffBuzzConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  function createSenderPrefixCtx(params: {
    ...
    return {
      cfg: {
        agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/fluffbuzz" } },
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/fluffbuzz", enabled: true },
    });
