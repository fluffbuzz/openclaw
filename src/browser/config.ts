import type { BrowserConfig, BrowserProfileConfig, FluffBuzzConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_FLUFFBUZZ_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_FLUFFBUZZ_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "fluffbuzz" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_FLUFFBUZZ_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_FLUFFBUZZ_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an FluffBuzz browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for fluffbuzz/fluffbuzz CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: FluffBuzzConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_FLUFFBUZZ_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_FLUFFBUZZ_BROWSER_PROFILE_NAME]
        ? DEFAULT_FLUFFBUZZ_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "fluffbuzz";
