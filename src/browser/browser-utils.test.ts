...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-fluffbuzz-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = "test-gateway-token";
    try {
      await ensureChromeExtensionRelayServer({ cdpUrl });
      const headers = getHeadersWithAuth(`${cdpUrl}/json/version`);
      expect(headers["x-fluffbuzz-relay-token"]).toBeTruthy();
      expect(headers["x-fluffbuzz-relay-token"]).not.toBe("test-gateway-token");
    } finally {
      await stopChromeExtensionRelayServer({ cdpUrl }).catch(() => {});
      if (prev === undefined) {
        delete process.env.FLUFFBUZZ_GATEWAY_TOKEN;
      } else {
        process.env.FLUFFBUZZ_GATEWAY_TOKEN = prev;
      }
    }
  });
});
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!fluffbuzz) {
      throw new Error("expected fluffbuzz profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...fluffbuzz, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
