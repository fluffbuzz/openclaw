...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredFluffBuzzTmpDir: vi.fn(() => "/tmp/fluffbuzz"),
}));
vi.mock("../infra/tmp-fluffbuzz-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredFluffBuzzTmpDir.mockReturnValue("/tmp/fluffbuzz");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".fluffbuzz-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredFluffBuzzTmpDir.mockReturnValue("/tmp/fluffbuzz-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "fluffbuzz-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "fluffbuzz-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredFluffBuzzTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "fluffbuzz-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "fluffbuzz-preferred", "downloads")}${path.sep}`),
    );
  });
