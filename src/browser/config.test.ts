...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and puppy-orange color", () => {
    ...
    expect(profile?.name).toBe("fluffbuzz");
    expect(profile?.driver).toBe("fluffbuzz");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const fluffbuzz = resolveProfile(resolved, "fluffbuzz");
    expect(fluffbuzz?.driver).toBe("fluffbuzz");
    expect(fluffbuzz?.cdpPort).toBe(18800);
    expect(fluffbuzz?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from FLUFFBUZZ_GATEWAY_PORT when unset", () => {
    withEnv({ FLUFFBUZZ_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const fluffbuzz = resolveProfile(resolved, "fluffbuzz");
      expect(fluffbuzz?.cdpPort).toBe(19012);
      expect(fluffbuzz?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ FLUFFBUZZ_GATEWAY_PORT: undefined }, () => {
      ...
      const fluffbuzz = resolveProfile(resolved, "fluffbuzz");
      expect(fluffbuzz?.cdpPort).toBe(19022);
      expect(fluffbuzz?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(fluffbuzz?.cdpPort).toBe(19000);
    expect(fluffbuzz?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "fluffbuzz");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        fluffbuzz: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("fluffbuzz");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to fluffbuzz profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps fluffbuzz default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("fluffbuzz");
    ...
    it("keeps fluffbuzz default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("fluffbuzz");
    ...
    it("keeps fluffbuzz default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
