...
vi.mock("./chrome.js", () => ({
  resolveFluffBuzzUserDataDir: vi.fn(() => "/tmp/fluffbuzz-test/fluffbuzz/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveFluffBuzzUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "fluffbuzz-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveFluffBuzzUserDataDir).mockReturnValue(userDataDir);
