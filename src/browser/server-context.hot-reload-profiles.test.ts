...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only fluffbuzz profile
    ...
    // 2. Simulate adding a new profile to config (like user editing fluffbuzz.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.fluffbuzz = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.fluffbuzz?.cdpPort).toBe(19999);
  });
