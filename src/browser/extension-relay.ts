...
const RELAY_AUTH_HEADER = "x-fluffbuzz-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/FluffBuzz-Extension-Relay",
            revision: "0",
            userAgent: "FluffBuzz-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "FluffBuzz/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedFluffBuzzRelay({
