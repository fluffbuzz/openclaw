...
describe("browser server-context remote profile tab operations", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    ...
    state.resolved.profiles.fluffbuzz = {
      cdpPort: 18800,
      attachOnly: true,
      color: "#FF4500",
    };
    ...
    const launchMock = vi.mocked(chromeModule.launchFluffBuzzChrome);
    const ctx = createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("fluffbuzz").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote fluffbuzz profiles", async () => {
