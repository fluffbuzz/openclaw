...
const RELAY_TOKEN_CONTEXT = "fluffbuzz-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const FLUFFBUZZ_RELAY_BROWSER = "FluffBuzz/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.FLUFFBUZZ_GATEWAY_TOKEN?.trim() || process.env.FLUFFBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or FLUFFBUZZ_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedFluffBuzzRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === FLUFFBUZZ_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
