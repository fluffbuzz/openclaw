  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          fluffbuzz: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.FLUFFBUZZ_GATEWAY_PORT;
    process.env.FLUFFBUZZ_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
