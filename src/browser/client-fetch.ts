...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-fluffbuzz-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-fluffbuzz-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-fluffbuzz-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the FluffBuzz gateway (FluffBuzz.app menubar, or \`${formatCliCommand("fluffbuzz gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the FluffBuzz browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the FluffBuzz browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
