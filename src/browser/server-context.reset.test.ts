...
function localFluffBuzzProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "fluffbuzz",
...
function createLocalFluffBuzzResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localFluffBuzzProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveFluffBuzzUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-reset-"));
    const profileDir = path.join(tempRoot, "fluffbuzz");
    ...
    const ops = createLocalFluffBuzzResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveFluffBuzzUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-reset-no-own-"));
    const profileDir = path.join(tempRoot, "fluffbuzz");
    fs.mkdirSync(profileDir, { recursive: true });
