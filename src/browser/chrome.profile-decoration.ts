...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".fluffbuzz-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + puppy-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateFluffBuzzProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_FLUFFBUZZ_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FLUFFBUZZ_BROWSER_COLOR).toUpperCase();
