...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.FLUFFBUZZ_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/fluffbuzz" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          fluffbuzz: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchFluffBuzzChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.FLUFFBUZZ_GATEWAY_PORT;
  process.env.FLUFFBUZZ_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
  } else {
    process.env.FLUFFBUZZ_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
