...
function ensureNodePathEnv(): string {
  ensureFluffBuzzCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.FLUFFBUZZ_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.FLUFFBUZZ_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
