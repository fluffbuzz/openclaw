...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-exec-approvals-"));
    const previousFluffBuzzHome = process.env.FLUFFBUZZ_HOME;
    process.env.FLUFFBUZZ_HOME = tempHome;
    saveExecApprovals(params.approvals);
    try {
      return await params.run({ tempHome });
    } finally {
      if (previousFluffBuzzHome === undefined) {
        delete process.env.FLUFFBUZZ_HOME;
      } else {
        process.env.FLUFFBUZZ_HOME = previousFluffBuzzHome;
      }
  ...
  for (const testCase of approvedEnvShellWrapperCases) {
    it.runIf(process.platform !== "win32")(testCase.name, async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-approved-wrapper-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "fluffbuzz-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "fluffbuzz-allowlist-path-pin-",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-approval-cwd-link-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-approval-cwd-parent-link-"));
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-approval-cwd-real-"));
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-approval-cwd-drift-"));
    const fallback = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-approval-cwd-drift-alt-"));
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `fluffbuzz-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["fluffbuzz status; id", "fluffbuzz status; cat /etc/passwd"];
