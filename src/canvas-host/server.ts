...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.fluffbuzzCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.fluffbuzzCanvasA2UIAction &&
        typeof window.fluffbuzzCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.fluffbuzzSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("fluffbuzz:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android FluffBuzz node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.fluffbuzzSendUserAction === "function"
        ? window.fluffbuzzSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FLUFFBUZZ_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.FLUFFBUZZ_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>FluffBuzz Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
