...
describe("canvas host state dir defaults", () => {
  it("uses FLUFFBUZZ_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("fluffbuzz-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("FluffBuzz Canvas");
      } finally {
        await handler.close();
      }
    });
