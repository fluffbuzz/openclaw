...
const createTempDir = () => tempDirs.make("fluffbuzz-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets FLUFFBUZZ_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "fluffbuzz",
    });

    expect(env.FLUFFBUZZ_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("fluffbuzz");
  });

  it("overrides pre-existing FLUFFBUZZ_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      FLUFFBUZZ_SHELL: "wrong",
    });
    expect(env.FLUFFBUZZ_SHELL).toBe("acp-client");
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "fluffbuzz", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "fluffbuzz", "dist", "entry.js");
    const shimPath = path.join(dir, "fluffbuzz.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\fluffbuzz\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/fluffbuzz-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/fluffbuzz-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/fluffbuzz-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
