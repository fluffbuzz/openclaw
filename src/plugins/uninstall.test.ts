...
function createSinglePluginWithEmptySlotsConfig(): FluffBuzzConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): FluffBuzzConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "fluffbuzz-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-fluffbuzz-extensions/my-plugin",
      },
      extensionsDir,
    });
