...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      fluffbuzz: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@fluffbuzz/cognee-fluffbuzz",
      version: "0.0.1",
      fluffbuzz: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "fluffbuzz.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.fluffbuzz/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without fluffbuzz.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@fluffbuzz/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("fluffbuzz.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_FLUFFBUZZ_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when fluffbuzz.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@fluffbuzz/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing fluffbuzz.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_FLUFFBUZZ_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without fluffbuzz.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.fluffbuzz).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses fluffbuzz.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-fluffbuzz"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@fluffbuzz/voice-call@0.0.1",
              name: "@fluffbuzz/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@fluffbuzz/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@fluffbuzz/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@fluffbuzz/voice-call@0.0.1",
    });
