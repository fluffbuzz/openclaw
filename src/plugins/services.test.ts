...
import type { FluffBuzzPluginService, FluffBuzzPluginServiceContext } from "./types.js";
...
function createRegistry(services: FluffBuzzPluginService[]) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: FluffBuzzPluginServiceContext[] = [];

    const serviceA: FluffBuzzPluginService = {
      id: "service-a",
      start: (ctx) => {
        starts.push("a");
        contexts.push(ctx);
    ...
    const serviceB: FluffBuzzPluginService = {
      id: "service-b",
      start: (ctx) => {
        starts.push("b");
        contexts.push(ctx);
    ...
    const serviceC: FluffBuzzPluginService = {
      id: "service-c",
      start: (ctx) => {
        starts.push("c");
        contexts.push(ctx);
