...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing fluffbuzz.extensions; update the plugin package to include fluffbuzz.extensions (for example ["./dist/index.js"]). See https://docs.fluffbuzz.com/help/troubleshooting#plugin-install-fails-with-missing-fluffbuzz-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_FLUFFBUZZ_EXTENSIONS: "missing_fluffbuzz_extensions",
  EMPTY_FLUFFBUZZ_EXTENSIONS: "empty_fluffbuzz_extensions",
...
function ensureFluffBuzzExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_FLUFFBUZZ_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json fluffbuzz.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_FLUFFBUZZ_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureFluffBuzzExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from fluffbuzz.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-fluffbuzz"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "fluffbuzz security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "fluffbuzz security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "fluffbuzz-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "fluffbuzz-npm-pack-",
    spec,
    timeoutMs,
