...
import { discoverFluffBuzzPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `fluffbuzz-plugins-${randomUUID()}`);
  ...
  return await withEnvAsync(
    {
      ...
      FLUFFBOT_STATE_DIR: undefined,
      FLUFFBUZZ_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
describe("discoverFluffBuzzPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".fluffbuzz", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverFluffBuzzPlugins({ workspaceDir });
    });
  ...
  it("ignores backup and disabled plugin directories in scanned roots", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverFluffBuzzPlugins({});
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        fluffbuzz: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        fluffbuzz: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverFluffBuzzPlugins({ extraPaths: [packDir] });
    });
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@fluffbuzz/pack",
        fluffbuzz: { extensions: ["./linked/escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it("rejects package extension entries that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@fluffbuzz/pack",
        fluffbuzz: { extensions: ["./escape.ts"] },
      }),
      "utf-8",
    );
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverFluffBuzzPlugins({ ownershipUid: actualUid + 1 });
      });
      expect(result.candidates).toHaveLength(0);
