...
import { __testing, loadFluffBuzzPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "fluffbuzz.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadFluffBuzzPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          fluffbuzz: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadFluffBuzzPlugins>) {
...
function useNoBundledPlugins() {
  process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadFluffBuzzPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadFluffBuzzPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadFluffBuzzPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadFluffBuzzPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadFluffBuzzPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ FLUFFBUZZ_STATE_DIR: stateDir, FLUFFBOT_STATE_DIR: undefined }, () => {
