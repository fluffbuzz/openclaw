...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FluffBuzzPluginDefinition;
  register?: FluffBuzzPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FluffBuzzPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FluffBuzzPluginDefinition;
...
export function loadFluffBuzzPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverFluffBuzzPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "fluffbuzz/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "fluffbuzz/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: FluffBuzzPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as FluffBuzzPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
