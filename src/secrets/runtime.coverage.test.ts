...
function buildConfigForFluffBuzzTarget(entry: SecretRegistryEntry, envId: string): FluffBuzzConfig {
  const config = {} as FluffBuzzConfig;
  const refTargetPath =
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every fluffbuzz.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "fluffbuzz.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `FLUFFBUZZ_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForFluffBuzzTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `FLUFFBUZZ_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
