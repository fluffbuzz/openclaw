...
export function buildConfigureCandidates(config: FluffBuzzConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredFluffBuzzConfig?: FluffBuzzConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredFluffBuzzConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const fluffbuzzCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "fluffbuzz.json" as const,
  ...
  return [...fluffbuzzCandidates, ...authCandidates].toSorted((a, b) =>
...
export function collectConfigureProviderChanges(params: {
  original: FluffBuzzConfig;
  next: FluffBuzzConfig;
