...
function setByPathSegments(root: FluffBuzzConfig, segments: string[], value: unknown): boolean {
  if (segments.length === 0) {
...
function deleteByPathSegments(root: FluffBuzzConfig, segments: string[]): boolean {
  if (segments.length === 0) {
    return false;
  }
  let cursor: Record<string, unknown> = root as unknown as Record<string, unknown>;
