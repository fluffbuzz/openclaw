...
describe("secret ref resolver", () => {
  ...
  it("resolves file refs in json mode", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-file-"));
  ...
  it("resolves exec refs with protocolVersion 1 response", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-"));
    cleanupRoots.push(root);
  ...
  it("supports non-JSON single-value exec output when jsonOnly is false", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-plain-"));
    cleanupRoots.push(root);
  ...
  it("rejects symlink command paths unless allowSymlinkCommand is enabled", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-link-"));
    cleanupRoots.push(root);
  ...
  it("handles Homebrew-style symlinked exec commands with args only when explicitly allowed", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-homebrew-"));
    cleanupRoots.push(root);
  ...
  it("checks trustedDirs against resolved symlink target", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-out-"));
    cleanupRoots.push(root);
  ...
  it("rejects exec refs when protocolVersion is not 1", async () => {
    ...
    const root = await fs.mkdtemp(
      path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-protocol-"),
    );
    cleanupRoots.push(root);
  ...
  it("rejects exec refs when response omits requested id", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-id-"));
    cleanupRoots.push(root);
  ...
  it("rejects exec refs with invalid JSON when jsonOnly is true", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-exec-json-"));
    cleanupRoots.push(root);
  ...
  it("supports file singleValue mode with id=value", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-single-value-"));
    cleanupRoots.push(root);
  ...
  it("times out file provider reads when timeoutMs elapses", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-resolve-timeout-"));
    cleanupRoots.push(root);
    const filePath = path.join(root, "secrets.json");
    await writeSecureFile(
      filePath,
