...
function getSecretProviders(config: FluffBuzzConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: FluffBuzzConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: FluffBuzzConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "fluffbuzz.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
  ...
  return `fluffbuzz:${candidate.path}`;
}
...
function resolveConfigureAgentId(config: FluffBuzzConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: FluffBuzzConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredFluffBuzzConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "fluffbuzz.json",
