...
function buildCandidates(config: FluffBuzzConfig): ConfigureCandidate[] {
...
function collectProviderPlanChanges(params: { original: FluffBuzzConfig; next: FluffBuzzConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in fluffbuzz.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "fluffbuzz secrets configure",
