...
import { loadConfig, type FluffBuzzConfig } from "../config/config.js";
import {
...
function asConfig(value: unknown): FluffBuzzConfig {
  return value as FluffBuzzConfig;
}

const OPENAI_ENV_KEY_REF = { source: "env", provider: "default", id: "OPENAI_API_KEY" } as const;

function createOpenAiFileModelsConfig(): NonNullable<FluffBuzzConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("resolves env refs for config and auth profiles", async () => {
    ...
    const snapshot = await prepareSecretsRuntimeSnapshot({
      ...
      agentDirs: ["/tmp/fluffbuzz-agent-main"],
      loadAuthStore: () =>
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: FluffBuzzConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-file-provider-"));
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("activates runtime snapshots for loadConfig and ensureAuthProfileStore", async () => {
    ...
    const store = ensureAuthProfileStore("/tmp/fluffbuzz-agent-main");
  ...
  it("does not write inherited auth stores during runtime secret activation", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-runtime-"));
    ...
    const prevStateDir = process.env.FLUFFBUZZ_STATE_DIR;
    ...
    try {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.FLUFFBUZZ_STATE_DIR = prevStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });
