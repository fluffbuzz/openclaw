...
function buildFixturePaths(rootDir: string) {
  const stateDir = path.join(rootDir, ".fluffbuzz");
  return {
    rootDir,
    stateDir,
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      FLUFFBUZZ_STATE_DIR: paths.stateDir,
      FLUFFBUZZ_CONFIG_PATH: paths.configPath,
