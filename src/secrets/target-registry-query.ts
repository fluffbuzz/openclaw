...
const FLUFFBUZZ_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of FLUFFBUZZ_COMPILED_SECRET_TARGETS) {
...
const FLUFFBUZZ_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries =
    allowedTargetIds === null
      ? FLUFFBUZZ_COMPILED_SECRET_TARGETS
      : Array.from(allowedTargetIds).flatMap(
          (targetId) => FLUFFBUZZ_TARGETS_BY_ID.get(targetId) ?? [],
        );
