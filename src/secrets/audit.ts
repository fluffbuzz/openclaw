...
import { resolveStateDir, type FluffBuzzConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: FluffBuzzConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.FLUFFBUZZ_AUTH_STORE_READONLY;
  ...
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as FluffBuzzConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.FLUFFBUZZ_AUTH_STORE_READONLY;
    } else {
      process.env.FLUFFBUZZ_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
