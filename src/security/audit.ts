...
import type { ConfigFileSnapshot, FluffBuzzConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: FluffBuzzConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your FluffBuzz state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your FluffBuzz state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.FLUFFBUZZ_GATEWAY_TOKEN) || hasNonEmptyString(env.FLUFFBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.FLUFFBUZZ_GATEWAY_PASSWORD) ||
    hasNonEmptyString(env.FLUFFBOT_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("fluffbuzz security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.FLUFFBUZZ_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.FLUFFBOT_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("fluffbuzz status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("fluffbuzz security audit --deep")}".`,
    });
  }
