...
function summarizeGroupPolicy(cfg: FluffBuzzConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: FluffBuzzConfig): boolean {
...
function collectModels(cfg: FluffBuzzConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: FluffBuzzConfig): Set<string> {
  const baseCfg: FluffBuzzConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: FluffBuzzConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: FluffBuzzConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: FluffBuzzConfig): boolean {
...
function isBrowserEnabled(cfg: FluffBuzzConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: FluffBuzzConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: FluffBuzzConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: FluffBuzzConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep FLUFFBUZZ_STATE_DIR on a local-only volume and re-run "${formatCliCommand("fluffbuzz security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer FLUFFBUZZ_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const fluffbuzzGatewayToken =
    typeof env.FLUFFBUZZ_GATEWAY_TOKEN === "string" && env.FLUFFBUZZ_GATEWAY_TOKEN.trim()
      ? env.FLUFFBUZZ_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-fluffbuzz-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: fluffbuzz-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: FluffBuzzConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "FluffBuzz's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
