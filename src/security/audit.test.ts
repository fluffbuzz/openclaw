...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: FluffBuzzConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: FluffBuzzConfig) => string[];
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "fluffbuzz.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ FLUFFBUZZ_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        fluffbuzz: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
    const prevPassword = process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
    delete process.env.FLUFFBUZZ_GATEWAY_TOKEN;
    delete process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.FLUFFBUZZ_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
      } else {
        process.env.FLUFFBUZZ_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/fluffbuzz-safe-bins"];
    const cfg: FluffBuzzConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("fluffbuzz-sbx-browser-old\nfluffbuzz-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "fluffbuzz-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "fluffbuzz-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("fluffbuzz-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("fluffbuzz-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "fluffbuzz-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "fluffbuzz-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-fluffbuzz.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: FluffBuzzConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: FluffBuzzConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): FluffBuzzConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: FluffBuzzConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.fluffbuzz/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: FluffBuzzConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: FluffBuzzConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<FluffBuzzConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: FluffBuzzConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.fluffbuzz",
      configPath: "/Users/test/Dropbox/.fluffbuzz/fluffbuzz.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: FluffBuzzConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "fluffbuzz.json"),
        execDockerRawFn: execDockerRawUnavailable,
      });
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "fluffbuzz.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
    ...
    const cfg: FluffBuzzConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@fluffbuzz/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@fluffbuzz/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@fluffbuzz/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@fluffbuzz/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        fluffbuzz: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.FLUFFBUZZ_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.FLUFFBUZZ_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
