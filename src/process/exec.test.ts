...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ FLUFFBUZZ_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.FLUFFBUZZ_BASE_ENV ?? "") + "|" + (process.env.FLUFFBUZZ_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { FLUFFBUZZ_TEST_ENV: "ok" },
        },
      );
