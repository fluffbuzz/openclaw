...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        FLUFFBUZZ_BASE_ENV: "base",
        FLUFFBUZZ_TO_REMOVE: undefined,
      },
      env: {
        FLUFFBUZZ_TEST_ENV: "ok",
      },
    });

    expect(resolved.FLUFFBUZZ_BASE_ENV).toBe("base");
    expect(resolved.FLUFFBUZZ_TEST_ENV).toBe("ok");
    expect(resolved.FLUFFBUZZ_TO_REMOVE).toBeUndefined();
  });
