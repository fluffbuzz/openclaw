...
describe("resolveGatewayConnection", () => {
  ...
  it("uses FLUFFBUZZ_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ FLUFFBUZZ_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers FLUFFBUZZ_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ FLUFFBUZZ_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
