...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.FLUFFBUZZ_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service FLUFFBUZZ_GATEWAY_TOKEN does not match gateway.auth.token in fluffbuzz.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `fluffbuzz gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
