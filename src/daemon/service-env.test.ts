...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.FLUFFBUZZ_GATEWAY_PORT).toBe("18789");
    expect(env.FLUFFBUZZ_GATEWAY_TOKEN).toBe("secret");
    expect(env.FLUFFBUZZ_SERVICE_MARKER).toBe("fluffbuzz");
    expect(env.FLUFFBUZZ_SERVICE_KIND).toBe("gateway");
    expect(typeof env.FLUFFBUZZ_SERVICE_VERSION).toBe("string");
    expect(env.FLUFFBUZZ_SYSTEMD_UNIT).toBe("fluffbuzz-gateway.service");
    if (process.platform === "darwin") {
      expect(env.FLUFFBUZZ_LAUNCHD_LABEL).toBe("com.fluffbuzz.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", FLUFFBUZZ_PROFILE: "work" },
      port: 18789,
    });
    expect(env.FLUFFBUZZ_SYSTEMD_UNIT).toBe("fluffbuzz-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.FLUFFBUZZ_LAUNCHD_LABEL).toBe("com.fluffbuzz.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through FLUFFBUZZ_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", FLUFFBUZZ_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.FLUFFBUZZ_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy FLUFFBOT_GATEWAY_TOKEN to FLUFFBUZZ_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", FLUFFBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.FLUFFBUZZ_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers FLUFFBUZZ_GATEWAY_TOKEN over legacy FLUFFBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        FLUFFBUZZ_GATEWAY_TOKEN: "fluffbuzz-token",
        FLUFFBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.FLUFFBUZZ_GATEWAY_TOKEN).toBe("fluffbuzz-token");
  });

  it("omits FLUFFBUZZ_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.FLUFFBUZZ_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".fluffbuzz"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", FLUFFBUZZ_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".fluffbuzz-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", FLUFFBUZZ_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".fluffbuzz"));
  });

  it("uses FLUFFBUZZ_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", FLUFFBUZZ_STATE_DIR: "/var/lib/fluffbuzz" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/fluffbuzz"));
  });

  it("expands ~ in FLUFFBUZZ_STATE_DIR", () => {
    const env = { HOME: "/Users/test", FLUFFBUZZ_STATE_DIR: "~/fluffbuzz-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/fluffbuzz-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { FLUFFBUZZ_STATE_DIR: "C:\\State\\fluffbuzz" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\fluffbuzz");
  });
});
