...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `fluffbuzz-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      FLUFFBUZZ_LAUNCHD_LABEL: `com.fluffbuzz.launchd-int-${testId}`,
      FLUFFBUZZ_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
