...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "fluffbuzz" | "fluffbot" | "fluffbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["fluffbuzz", "fluffbot", "fluffbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["fluffbuzz_service_marker"];
  const kindKeys = ["fluffbuzz_service_kind"];
...
function isFluffBuzzGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("com.fluffbuzz.");
}

function isFluffBuzzGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("fluffbuzz-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isFluffBuzzGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("fluffbuzz gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("fluffbot") || lower.includes("fluffbot");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "fluffbot" : "fluffbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "fluffbuzz" && isFluffBuzzGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "fluffbuzz" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "fluffbuzz" && isFluffBuzzGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isFluffBuzzGatewayTaskName(name)) {
        continue;
      }
