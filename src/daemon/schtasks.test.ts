...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when FLUFFBUZZ_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".fluffbuzz", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when FLUFFBUZZ_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", FLUFFBUZZ_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".fluffbuzz-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers FLUFFBUZZ_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        FLUFFBUZZ_PROFILE: "rescue",
        FLUFFBUZZ_STATE_DIR: "C:\\State\\fluffbuzz",
      },
      expected: path.join("C:\\State\\fluffbuzz", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", FLUFFBUZZ_PROFILE: "default" },
      expected: path.join("/home/test", ".fluffbuzz", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-schtasks-test-"));
    try {
      const extraEnv = typeof options.env === "function" ? options.env(tmpDir) : options.env;
      const env = {
        USERPROFILE: tmpDir,
        FLUFFBUZZ_PROFILE: "default",
        ...extraEnv,
      };
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem FluffBuzz Gateway",
          "cd /d C:\\Projects\\fluffbuzz",
          "set NODE_ENV=production",
          "set FLUFFBUZZ_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\fluffbuzz",
          environment: {
            NODE_ENV: "production",
            FLUFFBUZZ_PORT: "18789",
          },
        });
      },
    );
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\fluffbuzz\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\fluffbuzz\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\FluffBuzz Share\\node.exe" "\\\\fileserver\\FluffBuzz Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\FluffBuzz Share\\node.exe",
            "\\\\fileserver\\FluffBuzz Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from FLUFFBUZZ_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ FLUFFBUZZ_STATE_DIR: path.join(tmpDir, "custom-state") }),
