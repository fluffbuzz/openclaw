...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("fluffbuzz-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("fluffbuzz-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("fluffbuzz-lid-a-", (first) => {
      withTempDirSync("fluffbuzz-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.fluffbuzz when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers FLUFFBUZZ_HOME over HOME", () => {
    vi.stubEnv("FLUFFBUZZ_HOME", "/srv/fluffbuzz-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/fluffbuzz-home"));
...
describe("shortenHomePath", () => {
  it("uses $FLUFFBUZZ_HOME prefix when FLUFFBUZZ_HOME is set", () => {
    vi.stubEnv("FLUFFBUZZ_HOME", "/srv/fluffbuzz-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/fluffbuzz-home")}/.fluffbuzz/fluffbuzz.json`)).toBe(
      "$FLUFFBUZZ_HOME/.fluffbuzz/fluffbuzz.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $FLUFFBUZZ_HOME replacement when FLUFFBUZZ_HOME is set", () => {
    vi.stubEnv("FLUFFBUZZ_HOME", "/srv/fluffbuzz-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/fluffbuzz-home")}/.fluffbuzz/fluffbuzz.json`),
    ).toBe("config: $FLUFFBUZZ_HOME/.fluffbuzz/fluffbuzz.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/fluffbuzz")).toBe(path.resolve(os.homedir(), "fluffbuzz"));
  });
  ...
  it("prefers FLUFFBUZZ_HOME for tilde expansion", () => {
    vi.stubEnv("FLUFFBUZZ_HOME", "/srv/fluffbuzz-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/fluffbuzz")).toBe(path.resolve("/srv/fluffbuzz-home", "fluffbuzz"));
