...
const defaultTools = createFluffBuzzCodingTools({ senderIsOwner: true });
...
describe("createFluffBuzzCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: FluffBuzzConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createFluffBuzzCodingTools({
    ...
    const anthropicTools = createFluffBuzzCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: FluffBuzzConfig = {
      tools: {
        exec: {
    ...
    const allowed = createFluffBuzzCodingTools({
    ...
    const denied = createFluffBuzzCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createFluffBuzzCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
