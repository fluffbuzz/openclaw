...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["FLUFFBUZZ_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-subagent-"));
  ...
  it("uses isolated temp state when FLUFFBUZZ_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "fluffbuzz-test-state"));
  });
});
