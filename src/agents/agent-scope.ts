...
type AgentEntry = NonNullable<NonNullable<FluffBuzzConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: FluffBuzzConfig): AgentEntry[] {
...
export function listAgentIds(cfg: FluffBuzzConfig): string[] {
...
export function resolveDefaultAgentId(cfg: FluffBuzzConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: FluffBuzzConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: FluffBuzzConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: FluffBuzzConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: FluffBuzzConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
