...
test("exec sets FLUFFBUZZ_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-fluffbuzz-shell", {
    command: "node -e \"process.stdout.write(process.env.FLUFFBUZZ_SHELL || '')\"",
    pty: true,
  });
