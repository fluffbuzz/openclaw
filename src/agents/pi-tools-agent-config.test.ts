...
import { createFluffBuzzCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: FluffBuzzConfig) {
    return createFluffBuzzCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<FluffBuzzConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<FluffBuzzConfig["agents"]>["list"]>[number]["tools"];
  }): FluffBuzzConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createFluffBuzzCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createFluffBuzzCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createFluffBuzzCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createFluffBuzzCodingTools({
      config: cfg,
    ...
    const bobTools = createFluffBuzzCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createFluffBuzzCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createFluffBuzzCodingTools({
      config: cfg,
