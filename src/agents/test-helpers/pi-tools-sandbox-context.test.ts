...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("fluffbuzz-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("fluffbuzz-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("fluffbuzz-sbx-");
