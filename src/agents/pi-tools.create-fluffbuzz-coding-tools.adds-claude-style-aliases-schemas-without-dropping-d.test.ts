...
describe("createFluffBuzzCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-read-"));
    try {
      ...
      const contents = "Hello from fluffbuzz read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "fluffbot-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "fluffbot-workspace"),
    ...
    const tools = createFluffBuzzCodingTools({ sandbox });
