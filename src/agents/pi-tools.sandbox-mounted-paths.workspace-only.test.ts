...
type CodingToolsInput = NonNullable<Parameters<typeof createFluffBuzzCodingTools>[0]>;
...
function resolveApplyPatchTool(
  params: Pick<CodingToolsInput, "sandbox" | "workspaceDir"> & { config: FluffBuzzConfig },
): ToolWithExecute {
...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const tools = createFluffBuzzCodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    await withUnsafeMountedSandboxHarness(async ({ sandboxRoot, agentRoot, sandbox }) => {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as FluffBuzzConfig;
      const tools = createFluffBuzzCodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });
