...
type CoreToolDefinition = {
  ...
  includeInFluffBuzzGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const fluffbuzzTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInFluffBuzzGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:fluffbuzz": fluffbuzzTools,
