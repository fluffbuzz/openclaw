...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureFluffBuzzModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): FluffBuzzConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): FluffBuzzConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): FluffBuzzConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-workspace-"));
    const now = Date.now();
