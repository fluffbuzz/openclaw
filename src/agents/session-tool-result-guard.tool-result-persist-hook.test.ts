...
import { loadFluffBuzzPlugins } from "../plugins/loader.js";
...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-toolpersist-"));
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-before-write-"));
    process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
