...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: FluffBuzzConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/fluffbuzz" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: FluffBuzzConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: FluffBuzzConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: FluffBuzzConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: FluffBuzzConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: FluffBuzzConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: FluffBuzzConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: FluffBuzzConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: FluffBuzzConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/fluffbuzz-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/fluffbuzz");
  });

  it("uses FLUFFBUZZ_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "fluffbuzz-home");
    vi.stubEnv("FLUFFBUZZ_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as FluffBuzzConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".fluffbuzz", "workspace"));
  });

  it("uses FLUFFBUZZ_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "fluffbuzz-home");
    vi.stubEnv("FLUFFBUZZ_HOME", home);
    // Clear state dir so it falls back to FLUFFBUZZ_HOME
    vi.stubEnv("FLUFFBUZZ_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as FluffBuzzConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".fluffbuzz", "agents", "main", "agent"));
  });
});
