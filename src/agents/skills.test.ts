...
const makeWorkspace = async () => {
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-"));
...
beforeAll(async () => {
  tempHome = await createTempHomeEnv("fluffbuzz-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".fluffbuzz", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("applySkillEnvOverrides", () => {
  it("sets and restores env vars", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"fluffbuzz":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
    });
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"fluffbuzz":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"fluffbuzz":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"fluffbuzz":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
