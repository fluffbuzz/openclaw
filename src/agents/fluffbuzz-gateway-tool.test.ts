...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createFluffBuzzTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-test-"));

    try {
      await withEnvAsync(
        { FLUFFBUZZ_STATE_DIR: stateDir, FLUFFBUZZ_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: fluffbuzz --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/fluffbuzz" } }\n}\n';
