...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("fluffbuzz-ws-", async (workspaceDir) => {
      await withTempDir("fluffbuzz-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createFluffBuzzCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("fluffbuzz-ws-", async (workspaceDir) => {
      await withTempDir("fluffbuzz-override-", async (overrideDir) => {
        const tools = createFluffBuzzCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("fluffbuzz-ws-", async (workspaceDir) => {
      const cfg: FluffBuzzConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createFluffBuzzCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-fluffbuzz.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("fluffbuzz-sandbox-", async (sandboxDir) => {
      await withTempDir("fluffbuzz-workspace-", async (workspaceDir) => {
        ...
        const tools = createFluffBuzzCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
