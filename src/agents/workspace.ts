...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.FLUFFBUZZ_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".fluffbuzz", `workspace-${profile}`);
  }
  return path.join(home, ".fluffbuzz", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".fluffbuzz";
