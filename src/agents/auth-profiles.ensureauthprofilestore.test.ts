...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-auth-merge-"));
    const previousAgentDir = process.env.FLUFFBUZZ_AGENT_DIR;
    ...
    try {
      ...
      process.env.FLUFFBUZZ_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        delete process.env.FLUFFBUZZ_AGENT_DIR;
      } else {
        process.env.FLUFFBUZZ_AGENT_DIR = previousAgentDir;
      }
  ...
  it("normalizes auth-profiles credential aliases with canonical-field precedence", () => {
    ...
    for (const testCase of cases) {
      const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-auth-alias-"));
      try {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-auth-legacy-alias-"));
    try {
      fs.writeFileSync(
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-auth-invalid-"));
