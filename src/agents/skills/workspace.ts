...
function resolveSkillsLimits(config?: FluffBuzzConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "fluffbuzz-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "fluffbuzz-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveFluffBuzzMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: FluffBuzzConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`fluffbuzz skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
