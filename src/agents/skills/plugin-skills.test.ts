    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "fluffbuzz.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "fluffbuzz.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "fluffbuzz.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("fluffbuzz-");
  const acpxRoot = await tempDirs.make("fluffbuzz-acpx-plugin-");
  const helperRoot = await tempDirs.make("fluffbuzz-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("fluffbuzz-");
  const pluginRoot = await tempDirs.make("fluffbuzz-plugin-");
  const outsideDir = await tempDirs.make("fluffbuzz-outside-");
