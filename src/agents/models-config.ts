...
type ModelsConfig = NonNullable<FluffBuzzConfig["models"]>;
...
export async function ensureFluffBuzzModelsJson(
  config?: FluffBuzzConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveFluffBuzzAgentDir();
