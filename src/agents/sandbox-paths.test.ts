...
async function withOutsideHardlinkInFluffBuzzTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside fluffbuzz tmp root",
      media: path.join(os.tmpdir(), "outside-fluffbuzz", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked FluffBuzz tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked FluffBuzz tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInFluffBuzzTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked FluffBuzz tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInFluffBuzzTmp(
