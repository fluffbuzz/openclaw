...
function resolveExecConfig(params: { cfg?: FluffBuzzConfig; agentId?: string }) {
...
export function createFluffBuzzCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createFluffBuzzReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
