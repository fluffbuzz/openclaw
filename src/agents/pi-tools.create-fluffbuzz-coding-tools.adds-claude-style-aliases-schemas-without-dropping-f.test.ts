...
describe("createFluffBuzzCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-alias-"));
    try {
      const tools = createFluffBuzzCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-structured-write-"));
    try {
      const tools = createFluffBuzzCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-structured-edit-"));
    try {
