...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses FLUFFBUZZ_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      FLUFFBUZZ_HOME: "/srv/fluffbuzz-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/fluffbuzz-home"), ".fluffbuzz", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".fluffbuzz", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("fluffbuzz-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-workspace-hardlink-"));
    try {
