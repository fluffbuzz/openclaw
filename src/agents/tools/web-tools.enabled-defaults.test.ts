...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "fluffbuzz" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "FluffBuzz", url: "https://fluffbuzz.com/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest fluffbuzz release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://fluffbuzz.com/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://fluffbuzz.com/docs"]);
