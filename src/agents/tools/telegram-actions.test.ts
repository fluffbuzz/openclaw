...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): FluffBuzzConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as FluffBuzzConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): FluffBuzzConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as FluffBuzzConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: FluffBuzzConfig, accountId = "media") {
    await handleTelegramAction(
