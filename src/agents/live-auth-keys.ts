...
const GOOGLE_LIVE_SINGLE_KEY = "FLUFFBUZZ_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "FLUFFBUZZ_LIVE_ANTHROPIC_KEY",
    listVar: "FLUFFBUZZ_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "FLUFFBUZZ_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `FLUFFBUZZ_LIVE_${base}_KEY`;
