...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## FluffBuzz CLI Quick Reference");
    expect(prompt).toContain("fluffbuzz gateway restart");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List FluffBuzz agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("FluffBuzz docs: /tmp/fluffbuzz/docs");
    expect(prompt).toContain(
  ...
  // See: https://github.com/fluffbot/fluffbot/commit/66eec295b894bce8333886cfbca3b960c57c4946
  // Agents should use session_status or message timestamps to determine the date/time.
  // Related: https://github.com/fluffbot/fluffbot/issues/1897
  //          https://github.com/fluffbot/fluffbot/issues/3658
  it("does NOT include a date or time in the system prompt (cache stability)", () => {
    ...
    // https://github.com/fluffbot/fluffbot/issues/3658 for the preferred approach:
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## FluffBuzz Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/fluffbuzz. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`fluffbuzz ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for FluffBuzz subagents");
