...
function makeFallbacksOnlyCfg(): FluffBuzzConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): FluffBuzzConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
describe("runWithModelFallback", () => {
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
