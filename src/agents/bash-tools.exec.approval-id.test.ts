...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-test-bin-"));
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-test-obf-"));
