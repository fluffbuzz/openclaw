...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "fluffbuzz.sandbox=1");
  args.push("--label", `fluffbuzz.sessionKey=${params.scopeKey}`);
  args.push("--label", `fluffbuzz.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `fluffbuzz.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "fluffbuzz.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`fluffbuzz sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`fluffbuzz sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("fluffbuzz sandbox recreate --all");
}
