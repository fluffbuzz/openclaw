 ...
 * A common mistake — caused by the similarity with the `fluffbuzz.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (fluffbuzz.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.FLUFFBUZZ_AUTH_STORE_READONLY === "1";
