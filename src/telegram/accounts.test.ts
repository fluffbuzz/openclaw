...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", FLUFFBUZZ_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
      const cfg: FluffBuzzConfig = {
        channels: {
          telegram: { accounts: { work: { botToken: "tok-work" } } },
        },
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): FluffBuzzConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): FluffBuzzConfig => ({
    channels: {
      telegram: {
