...
describe("resolveEffectiveHomeDir", () => {
  it("prefers FLUFFBUZZ_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/fluffbuzz-home"),
    );
  });
  ...
  it("expands FLUFFBUZZ_HOME when set to ~", () => {
    const env = {
      FLUFFBUZZ_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for FLUFFBUZZ_HOME", () => {
    const result = resolveRequiredHomeDir(
      { FLUFFBUZZ_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when FLUFFBUZZ_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ FLUFFBUZZ_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { FLUFFBUZZ_HOME: "/srv/fluffbuzz-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/fluffbuzz-home")}/x`);
  });
