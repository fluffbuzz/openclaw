...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["FLUFFBUZZ_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.FLUFFBUZZ_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveFluffBuzzPackageRoot } = await import("./fluffbuzz-root.js"));
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveFluffBuzzPackageRoot).mockResolvedValue("/opt/fluffbuzz");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/fluffbuzz/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
