...
import { triggerFluffBuzzRestart } from "./restart.js";
 ...
 * - FLUFFBUZZ_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.FLUFFBUZZ_NO_RESPAWN)) {
  ...
  if (isLikelySupervisedProcess(process.env)) {
    ...
    if (process.platform === "darwin" && process.env.FLUFFBUZZ_LAUNCHD_LABEL?.trim()) {
      const restart = triggerFluffBuzzRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
