...
import { resolveFluffBuzzPackageRoot, resolveFluffBuzzPackageRootSync } from "./fluffbuzz-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveFluffBuzzPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "fluffbuzz" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveFluffBuzzPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // fluffbuzz.mjs or dist/<bundle>.js
