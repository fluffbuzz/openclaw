...
vi.mock("./fluffbuzz-root.js", () => ({
  resolveFluffBuzzPackageRoot: vi.fn(async () => null),
  resolveFluffBuzzPackageRootSync: vi.fn(() => null),
}));
...
let fluffbuzzRoot: typeof import("./fluffbuzz-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    fluffbuzzRoot = await import("./fluffbuzz-root.js");
  });
  ...
  it("uses resolveFluffBuzzPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/fluffbuzz");
    (
      fluffbuzzRoot.resolveFluffBuzzPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/fluffbuzz"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "fluffbuzz" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "fluffbuzz.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-fluffbuzz");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/fluffbuzz-bundle");
    (
      fluffbuzzRoot.resolveFluffBuzzPackageRootSync as unknown as ReturnType<typeof vi.fn>
