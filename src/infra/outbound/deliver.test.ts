...
import { resolvePreferredFluffBuzzTmpDir } from "../tmp-fluffbuzz-dir.js";
...
const telegramChunkConfig: FluffBuzzConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: FluffBuzzConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes FluffBuzz tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredFluffBuzzTmpDir()]),
      }),
    );
  });

  it("includes FluffBuzz tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes FluffBuzz tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes FluffBuzz tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: FluffBuzzConfig = { channels: { signal: { mediaMaxMb: 2 } } };
