...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: FluffBuzzConfig | null = null;
  ...
  get(key: string, cfg: FluffBuzzConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: FluffBuzzConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: FluffBuzzConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: FluffBuzzConfig): void {
