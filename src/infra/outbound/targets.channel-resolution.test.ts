...
const mocks = vi.hoisted(() => ({
  ...
  loadFluffBuzzPlugins: vi.fn(),
}));

const TEST_WORKSPACE_ROOT = "/tmp/fluffbuzz-test-workspace";
...
vi.mock("../../plugins/loader.js", () => ({
  loadFluffBuzzPlugins: mocks.loadFluffBuzzPlugins,
}));
...
describe("resolveOutboundTarget channel resolution", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadFluffBuzzPlugins.mockReset();
  });
  ...
  it("recovers telegram plugin resolution so announce delivery does not fail with Unsupported channel: telegram", () => {
    ...
    expect(mocks.loadFluffBuzzPlugins).toHaveBeenCalledTimes(1);
  });
  ...
  it("retries bootstrap on subsequent resolve when the first bootstrap attempt fails", () => {
    ...
    expect(mocks.loadFluffBuzzPlugins).toHaveBeenCalledTimes(2);
  });
});
