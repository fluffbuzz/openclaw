...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred FluffBuzz tmp root", async () => {
    ...
    try {
      ...
      const hostTmpOutsideFluffBuzz = path.join(os.tmpdir(), "outside-fluffbuzz", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
