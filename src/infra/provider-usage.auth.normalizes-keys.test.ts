...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-provider-auth-suite-"));
  });
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".fluffbuzz", "agents", "main", "agent");
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    const stateDir = path.join(home, ".fluffbuzz");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(
      path.join(stateDir, "fluffbuzz.json"),
