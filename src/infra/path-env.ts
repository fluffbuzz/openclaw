...
function candidateBinDirs(opts: EnsureFluffBuzzPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `fluffbuzz` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "fluffbuzz");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.FLUFFBUZZ_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "fluffbuzz"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `fluffbuzz` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFluffBuzzCliOnPath(opts: EnsureFluffBuzzPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FLUFFBUZZ_PATH_BOOTSTRAPPED)) {
    return;
  }
