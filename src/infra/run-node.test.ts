...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-run-node-"));
  try {
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const exitCode = await runNodeMain({
          ...
          env: {
            ...process.env,
            FLUFFBUZZ_FORCE_BUILD: "1",
            FLUFFBUZZ_RUNNER_LOG: "0",
          },
          spawn,
        ...
        expect(nodeCalls).toEqual([[process.execPath, "fluffbuzz.mjs", "--version"]]);
      });
    },
  );
});
