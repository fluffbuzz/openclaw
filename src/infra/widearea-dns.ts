...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FLUFFBUZZ_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*fluffbuzz-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "fluffbuzz";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "fluffbuzz");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "fluffbuzz-gw");
  ...
  records.push(`_fluffbuzz-gw._tcp IN PTR ${instanceLabel}._fluffbuzz-gw._tcp`);
  records.push(`${instanceLabel}._fluffbuzz-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._fluffbuzz-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; fluffbuzz-content-hash: ${contentHash}\n${contentBody}`;
}
