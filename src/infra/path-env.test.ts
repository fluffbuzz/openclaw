...
let ensureFluffBuzzCliOnPath: typeof import("./path-env.js").ensureFluffBuzzCliOnPath;

describe("ensureFluffBuzzCliOnPath", () => {
  const envKeys = [
    "PATH",
    "FLUFFBUZZ_PATH_BOOTSTRAPPED",
    "FLUFFBUZZ_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureFluffBuzzCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling fluffbuzz exists", () => {
    const tmp = abs("/tmp/fluffbuzz-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "fluffbuzz");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.FLUFFBUZZ_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.FLUFFBUZZ_PATH_BOOTSTRAPPED = "1";
    ensureFluffBuzzCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/fluffbuzz-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "fluffbuzz");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/fluffbuzz-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "fluffbuzz");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "fluffbuzz");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/fluffbuzz-path/case-linuxbrew");
