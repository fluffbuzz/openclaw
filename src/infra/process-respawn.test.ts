...
const triggerFluffBuzzRestartMock = vi.hoisted(() => vi.fn());

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./restart.js", () => ({
  triggerFluffBuzzRestart: (...args: unknown[]) => triggerFluffBuzzRestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerFluffBuzzRestartMock.mockClear();
  if (originalPlatformDescriptor) {
...
function expectLaunchdKickstartSupervised(params?: { launchJobLabel?: string }) {
  ...
  triggerFluffBuzzRestartMock.mockReturnValue({ ok: true, method: "launchctl" });
  const result = restartGatewayProcessWithFreshPid();
  expect(result.mode).toBe("supervised");
  expect(triggerFluffBuzzRestartMock).toHaveBeenCalledOnce();
...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("runs launchd kickstart helper on macOS when launchd label is set", () => {
    expectLaunchdKickstartSupervised({ launchJobLabel: "com.fluffbuzz.gateway" });
  });
  ...
  it("returns failed when launchd kickstart helper fails", () => {
    ...
    process.env.LAUNCH_JOB_LABEL = "com.fluffbuzz.gateway";
    process.env.FLUFFBUZZ_LAUNCHD_LABEL = "com.fluffbuzz.gateway";
    triggerFluffBuzzRestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
      detail: "spawn failed",
    });
  ...
  it("does not schedule kickstart on non-darwin platforms", () => {
    ...
    expect(result.mode).toBe("supervised");
    expect(triggerFluffBuzzRestartMock).not.toHaveBeenCalled();
    expect(spawnMock).not.toHaveBeenCalled();
  });
