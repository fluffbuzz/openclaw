...
const TEMP_DIR_PREFIX = "fluffbuzz-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: { stdout: string }) {
  const cwd = await createTempDir("fluffbuzz-install-source-utils-");
  const archivePath = path.join(cwd, "fluffbuzz-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("fluffbuzz-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-fluffbuzz-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "fluffbuzz-plugin@1.2.3",
          name: "fluffbuzz-plugin",
          version: "1.2.3",
          filename: "fluffbuzz-plugin-1.2.3.tgz",
    ...
    const result = await runPack("fluffbuzz-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "fluffbuzz-plugin",
        version: "1.2.3",
        resolvedSpec: "fluffbuzz-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "fluffbuzz-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "fluffbuzz-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nfluffbuzz-plugin-1.2.3.tgz\n",
    });
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@fluffbuzz/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@fluffbuzz/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@fluffbuzz/whatsapp");
      expect(result.error).toContain("docs.fluffbuzz.com/tools/plugin");
    }
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("fluffbuzz-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "fluffbuzz-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@fluffbuzz/plugin-demo@2.0.0",
            filename: "fluffbuzz-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@fluffbuzz/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "fluffbuzz-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@fluffbuzz/plugin-demo@2.0.0",
      },
    });
  });
