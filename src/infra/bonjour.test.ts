...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.FLUFFBUZZ_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("fluffbuzz-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("fluffbuzz (FluffBuzz)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("fluffbuzz");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("fluffbuzz.local");
