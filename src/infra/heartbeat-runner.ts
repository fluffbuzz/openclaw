...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FluffBuzzConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: FluffBuzzConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: FluffBuzzConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: FluffBuzzConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: FluffBuzzConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: FluffBuzzConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FluffBuzzConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
