...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __fluffbuzzDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__fluffbuzzDiagnosticEventsState) {
    globalStore.__fluffbuzzDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__fluffbuzzDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: FluffBuzzConfig): boolean {
