...
const DEFAULT_PACKAGE_NAME = "fluffbuzz";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["fluffbuzz", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "fluffbuzz.mjs");
    ...
    const doctorStep = await runStep(
      step("fluffbuzz doctor", doctorArgv, gitRoot, { FLUFFBUZZ_UPDATE_IN_PROGRESS: "1" }),
    );
