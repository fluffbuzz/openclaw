...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__fluffbuzz_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "fluffbuzz-root");
...
describe("resolveFluffBuzzPackageRoot", () => {
  let resolveFluffBuzzPackageRoot: typeof import("./fluffbuzz-root.js").resolveFluffBuzzPackageRoot;
  let resolveFluffBuzzPackageRootSync: typeof import("./fluffbuzz-root.js").resolveFluffBuzzPackageRootSync;

  beforeAll(async () => {
    ({ resolveFluffBuzzPackageRoot, resolveFluffBuzzPackageRootSync } =
      await import("./fluffbuzz-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "fluffbuzz");
    const pkgRoot = path.join(project, "node_modules", "fluffbuzz");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "fluffbuzz" }));

    expect(resolveFluffBuzzPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "fluffbuzz");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "fluffbuzz.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "fluffbuzz" }));

    expect(resolveFluffBuzzPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveFluffBuzzPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-fluffbuzz package roots", async () => {
    const pkgRoot = fx("not-fluffbuzz");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-fluffbuzz" }));

    expect(resolveFluffBuzzPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveFluffBuzzPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveFluffBuzzPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
