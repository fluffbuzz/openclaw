...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-install-safe-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-install-safe-outside-"));
      try {
