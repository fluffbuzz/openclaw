...
import { POSIX_FLUFFBUZZ_TMP_DIR, resolvePreferredFluffBuzzTmpDir } from "./tmp-fluffbuzz-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredFluffBuzzTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `fluffbuzz-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredFluffBuzzTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_FLUFFBUZZ_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredFluffBuzzTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredFluffBuzzTmpDir", () => {
  it("prefers /tmp/fluffbuzz when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_FLUFFBUZZ_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/fluffbuzz when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_FLUFFBUZZ_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/fluffbuzz when /tmp/fluffbuzz is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/fluffbuzz when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/fluffbuzz is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/fluffbuzz is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/fluffbuzz is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback FluffBuzz temp dir/);
  });
