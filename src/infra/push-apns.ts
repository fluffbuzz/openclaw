...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.FLUFFBUZZ_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.FLUFFBUZZ_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set FLUFFBUZZ_APNS_TEAM_ID and FLUFFBUZZ_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.FLUFFBUZZ_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.FLUFFBUZZ_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.FLUFFBUZZ_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set FLUFFBUZZ_APNS_PRIVATE_KEY_P8 or FLUFFBUZZ_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading FLUFFBUZZ_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createFluffBuzzPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    fluffbuzz: createFluffBuzzPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
