...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FLUFFBUZZ_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "FluffBuzz";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(FluffBuzz\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.FLUFFBUZZ_MDNS_HOSTNAME?.trim() ||
    process.env.FLUFFBOT_MDNS_HOSTNAME?.trim() ||
    "fluffbuzz";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
