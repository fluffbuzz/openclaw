...
export function triggerFluffBuzzRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.FLUFFBUZZ_SYSTEMD_UNIT,
        process.env.FLUFFBUZZ_PROFILE,
      );
  ...
  const label =
    process.env.FLUFFBUZZ_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.FLUFFBUZZ_PROFILE);
