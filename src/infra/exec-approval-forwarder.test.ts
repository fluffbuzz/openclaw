...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): FluffBuzzConfig {
  return {
...
describe("exec approval forwarder", () => {
  ...
  it("prefers turn-source routing over stale session last route", async () => {
    vi.useFakeTimers();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-exec-approval-forwarder-test-"));
    try {
