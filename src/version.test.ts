...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "fluffbuzz", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "fluffbuzz", version: "2.3.4" });
  ...
  it("ignores non-fluffbuzz package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers FLUFFBUZZ_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        FLUFFBUZZ_VERSION: "9.9.9",
        FLUFFBUZZ_SERVICE_VERSION: "2.2.2",
  ...
  it("uses service and package fallbacks and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        FLUFFBUZZ_VERSION: "   ",
        FLUFFBUZZ_SERVICE_VERSION: "  2.0.0  ",
