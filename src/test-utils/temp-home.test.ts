...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("fluffbuzz-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.FLUFFBUZZ_STATE_DIR).toBe(path.join(tempHome.home, ".fluffbuzz"));
    ...
    expect(process.env.FLUFFBUZZ_STATE_DIR).toBe(previousStateDir);
