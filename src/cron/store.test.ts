...
const { makeStorePath } = createCronStoreHarness({ prefix: "fluffbuzz-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses FLUFFBUZZ_HOME for tilde expansion", () => {
    vi.stubEnv("FLUFFBUZZ_HOME", "/srv/fluffbuzz-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/fluffbuzz-home", "cron", "jobs.json"));
  });
});
