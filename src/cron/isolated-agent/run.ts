...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/fluffbuzz/fluffbuzz/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.FLUFFBUZZ_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.fluffbuzz/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: FluffBuzzConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/fluffbuzz/fluffbuzz/issues/29774
