...
type HomeEnvSnapshot = {
  ...
  FLUFFBUZZ_HOME: string | undefined;
  FLUFFBUZZ_STATE_DIR: string | undefined;
};
...
function restoreHomeEnv(snapshot: HomeEnvSnapshot) {
  ...
  restoreValue("FLUFFBUZZ_HOME");
  restoreValue("FLUFFBUZZ_STATE_DIR");
}

async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  const home = path.join(suiteTempHomeRoot, `case-${suiteTempHomeCaseId++}`);
  await fs.mkdir(path.join(home, ".fluffbuzz", "agents", "main", "sessions"), { recursive: true });
  const snapshot = snapshotHomeEnv();
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  delete process.env.FLUFFBUZZ_HOME;
  process.env.FLUFFBUZZ_STATE_DIR = path.join(home, ".fluffbuzz");
...
describe("runCronIsolatedAgentTurn", () => {
  beforeAll(async () => {
    suiteTempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-cron-turn-suite-"));
  });
  ...
  it("passes resolved agentDir to runEmbeddedPiAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.agentDir).toBe(path.join(home, ".fluffbuzz", "agents", "main", "agent"));
    });
  });
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".fluffbuzz", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
