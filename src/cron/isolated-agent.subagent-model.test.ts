...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "fluffbuzz-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".fluffbuzz", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<FluffBuzzConfig> = {},
): FluffBuzzConfig {
  const base: FluffBuzzConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<FluffBuzzConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<FluffBuzzConfig>,
