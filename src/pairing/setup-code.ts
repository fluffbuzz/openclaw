...
function resolveGatewayPort(cfg: FluffBuzzConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: FluffBuzzConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.FLUFFBUZZ_GATEWAY_TOKEN?.trim() ||
    env.FLUFFBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.FLUFFBUZZ_GATEWAY_PASSWORD?.trim() ||
    env.FLUFFBOT_GATEWAY_PASSWORD?.trim() ||
