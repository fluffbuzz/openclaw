...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredFluffBuzzTmpDir(), "fluffbuzz-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub FLUFFBUZZ_STATE_DIR.
    // Also keep it outside the FluffBuzz temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["FLUFFBUZZ_STATE_DIR"]);
    process.env.FLUFFBUZZ_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "fluffbuzz-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredFluffBuzzTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default FluffBuzz state workspace and sandbox roots", async () => {
  ...
  it("rejects default FluffBuzz state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
