...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "fluffbuzz-bundled-skills-test-"));
    process.env.FLUFFBUZZ_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
