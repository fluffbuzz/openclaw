...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.FLUFFBUZZ_DISABLE_ROUTE_FIRST;
    delete process.env.FLUFFBUZZ_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.FLUFFBUZZ_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("passes suppressDoctorStdout=true for routed --json commands", async () => {
    await expect(tryRouteCli(["node", "fluffbuzz", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "fluffbuzz", "status"])).resolves.toBe(true);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "fluffbuzz", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
