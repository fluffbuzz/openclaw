...
import type { FluffBuzzConfig, ConfigFileSnapshot } from "../config/types.fluffbuzz.js";
...
vi.mock("../infra/fluffbuzz-root.js", () => ({
  resolveFluffBuzzPackageRoot: vi.fn(),
}));
...
const { resolveFluffBuzzPackageRoot } = await import("../infra/fluffbuzz-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/fluffbuzz-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as FluffBuzzConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/fluffbuzz-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveFluffBuzzPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("fluffbuzz-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveFluffBuzzPackageRoot).mockClear();
    ...
    vi.mocked(resolveFluffBuzzPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("fluffbuzz");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/fluffbuzz-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "fluffbuzz-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("FluffBuzz update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as FluffBuzzConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveFluffBuzzPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("fluffbuzz-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ FLUFFBUZZ_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.FLUFFBUZZ_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("fluffbuzz-update-wizard");
    await withEnvAsync({ FLUFFBUZZ_GIT_DIR: tempDir }, async () => {
      setTty(true);
