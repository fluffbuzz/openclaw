...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "fluffbuzz", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "fluffbuzz", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "fluffbuzz", "status"]);
  });
