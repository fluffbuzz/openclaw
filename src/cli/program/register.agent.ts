...
${formatHelpExamples([
  ['fluffbuzz agent --to +15555550123 --message "status update"', "Start a new session."],
  ['fluffbuzz agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'fluffbuzz agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'fluffbuzz agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['fluffbuzz agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'fluffbuzz agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.fluffbuzz.com/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.fluffbuzz.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['fluffbuzz agents set-identity --agent main --name "FluffBuzz" --emoji "🐾"', "Set name + emoji."],
  ["fluffbuzz agents set-identity --agent main --avatar avatars/fluffbuzz.png", "Set avatar path."],
  [
    "fluffbuzz agents set-identity --workspace ~/.fluffbuzz/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "fluffbuzz agents set-identity --identity-file ~/.fluffbuzz/workspace/IDENTITY.md --agent main",
