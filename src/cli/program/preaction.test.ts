...
beforeEach(() => {
  ...
  originalHideBanner = process.env.FLUFFBUZZ_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.FLUFFBUZZ_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.FLUFFBUZZ_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("fluffbuzz");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "fluffbuzz", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("fluffbuzz-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "fluffbuzz", "message", "send"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "fluffbuzz", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "fluffbuzz", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "fluffbuzz", "agents"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "fluffbuzz", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "fluffbuzz", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "fluffbuzz", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "fluffbuzz", "--version"],
    });
  ...
  it("hides banner when FLUFFBUZZ_HIDE_BANNER is truthy", async () => {
    process.env.FLUFFBUZZ_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "fluffbuzz", "status"],
    });
