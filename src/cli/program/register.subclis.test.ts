...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.FLUFFBUZZ_DISABLE_LAZY_SUBCOMMANDS;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.FLUFFBUZZ_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.FLUFFBUZZ_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "fluffbuzz", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "fluffbuzz"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "fluffbuzz", "nodes", "list"], "fluffbuzz");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "fluffbuzz", "acp", "--help"], "fluffbuzz");
