...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update FluffBuzz and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["fluffbuzz update", "Update a source checkout (git)"],
        ["fluffbuzz update --channel beta", "Switch to beta channel (git + npm)"],
        ["fluffbuzz update --channel dev", "Switch to dev channel (git + npm)"],
        ["fluffbuzz update --tag beta", "One-off update to a dist-tag or version"],
        ["fluffbuzz update --dry-run", "Preview actions without changing anything"],
        ["fluffbuzz update --no-restart", "Update without restarting the service"],
        ["fluffbuzz update --json", "Output result as JSON"],
        ["fluffbuzz update --yes", "Non-interactive (accept downgrade prompts)"],
        ["fluffbuzz update wizard", "Interactive update wizard"],
        ["fluffbuzz --update", "Shorthand for fluffbuzz update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run fluffbuzz update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.fluffbuzz.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fluffbuzz update status", "Show channel + version status."],
          ["fluffbuzz update status --json", "JSON output."],
          ["fluffbuzz update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.fluffbuzz.com/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
