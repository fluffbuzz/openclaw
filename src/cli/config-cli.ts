...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("fluffbuzz doctor")}\` ${message}`;
}
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "fluffbuzz.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.fluffbuzz.com/cli/config")}\n`,
    )
    .option(
      "--section <section>",
