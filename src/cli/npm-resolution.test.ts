...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@fluffbuzz/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@fluffbuzz/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@fluffbuzz/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@fluffbuzz/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @fluffbuzz/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@fluffbuzz/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@fluffbuzz/plugin-alpha@1.2.3",
        installPath: "/tmp/fluffbuzz/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@fluffbuzz/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @fluffbuzz/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@fluffbuzz/plugin-alpha@latest",
      true,
      "/tmp/fluffbuzz/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@fluffbuzz/plugin-alpha@latest",
      installPath: "/tmp/fluffbuzz/extensions/alpha",
      version: "1.2.3",
