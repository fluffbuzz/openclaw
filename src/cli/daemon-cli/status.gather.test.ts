...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    FLUFFBUZZ_STATE_DIR: "/tmp/fluffbuzz-daemon",
    FLUFFBUZZ_CONFIG_PATH: "/tmp/fluffbuzz-daemon/fluffbuzz.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.FLUFFBUZZ_STATE_DIR ?? "/tmp/fluffbuzz-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.FLUFFBUZZ_CONFIG_PATH ?? `${stateDir}/fluffbuzz.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/fluffbuzz-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.FLUFFBUZZ_STATE_DIR = "/tmp/fluffbuzz-cli";
    process.env.FLUFFBUZZ_CONFIG_PATH = "/tmp/fluffbuzz-cli/fluffbuzz.json";
