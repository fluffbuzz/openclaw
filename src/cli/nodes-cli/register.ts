    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fluffbuzz nodes status", "List known nodes with live status."],
          ["fluffbuzz nodes pairing pending", "Show pending node pairing requests."],
          ['fluffbuzz nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["fluffbuzz nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.fluffbuzz.com/cli/nodes")}\n`,
    );
