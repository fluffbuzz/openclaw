...
import { resolveFluffBuzzPackageRoot } from "../../infra/fluffbuzz-root.js";
...
const FLUFFBUZZ_REPO_URL = "https://github.com/fluffbuzz/fluffbuzz.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "fluffbuzz";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("fluffbuzz@")) {
    return trimmed.slice("fluffbuzz@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.FLUFFBUZZ_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveFluffBuzzPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", FLUFFBUZZ_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `FLUFFBUZZ_GIT_DIR points at a non-git directory: ${params.dir}. Set FLUFFBUZZ_GIT_DIR to an empty folder or an fluffbuzz checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`FLUFFBUZZ_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "fluffbuzz.mjs");
