...
const STEP_LABELS: Record<string, string> = {
  ...
  "fluffbuzz doctor entry": "Checking doctor entrypoint",
  "fluffbuzz doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g fluffbuzz@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g fluffbuzz@latest --omit=optional");
  }
