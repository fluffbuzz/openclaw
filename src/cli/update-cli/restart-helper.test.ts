...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'fluffbuzz-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses FLUFFBUZZ_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        FLUFFBUZZ_PROFILE: "default",
        FLUFFBUZZ_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.fluffbuzz.gateway'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses FLUFFBUZZ_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        FLUFFBUZZ_PROFILE: "default",
        FLUFFBUZZ_LAUNCHD_LABEL: "com.custom.fluffbuzz",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.fluffbuzz'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "FluffBuzz Gateway"');
      expect(content).toContain('schtasks /Run /TN "FluffBuzz Gateway"');
    ...
    it("uses FLUFFBUZZ_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        FLUFFBUZZ_PROFILE: "default",
        FLUFFBUZZ_WINDOWS_TASK_NAME: "FluffBuzz Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "FluffBuzz Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "FluffBuzz Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        FLUFFBUZZ_PROFILE: "production",
      });
      expect(content).toContain("fluffbuzz-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        FLUFFBUZZ_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/com.fluffbuzz.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "FluffBuzz Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
