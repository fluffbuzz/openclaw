...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary = typeof env.FLUFFBUZZ_GATEWAY_TOKEN === "string" ? env.FLUFFBUZZ_GATEWAY_TOKEN : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy = typeof env.FLUFFBOT_GATEWAY_TOKEN === "string" ? env.FLUFFBOT_GATEWAY_TOKEN : "";
...
function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary =
    typeof env.FLUFFBUZZ_GATEWAY_PASSWORD === "string" ? env.FLUFFBUZZ_GATEWAY_PASSWORD : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy =
    typeof env.FLUFFBOT_GATEWAY_PASSWORD === "string" ? env.FLUFFBOT_GATEWAY_PASSWORD : "";
  if (legacy.trim().length > 0) {
    return legacy.trim();
  }
  return undefined;
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.fluffbuzz.com/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the FluffBuzz iOS app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("fluffbuzz devices list")}`,
          `  ${theme.command("fluffbuzz devices approve <requestId>")}`,
        );
