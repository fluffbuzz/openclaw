...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# FluffBuzz Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(fluffbuzz completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# FluffBuzz Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(fluffbuzz completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.fluffbuzz.com/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $FLUFFBUZZ_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "fluffbuzz") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c fluffbuzz -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'fluffbuzz gateway' and not 'fluffbuzz other gateway', we need to check the sequence.
