    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fluffbuzz memory status", "Show index and provider status."],
          ["fluffbuzz memory index --force", "Force a full reindex."],
          ['fluffbuzz memory search --query "deployment notes"', "Search indexed memory entries."],
          ["fluffbuzz memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.fluffbuzz.com/cli/memory")}\n`,
    );
