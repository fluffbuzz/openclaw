...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("FLUFFBUZZ_GATEWAY_TOKEN", "");
    vi.stubEnv("FLUFFBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("FLUFFBUZZ_GATEWAY_PASSWORD", "");
    vi.stubEnv("FLUFFBOT_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("fluffbuzz devices approve <requestId>");
  });
  ...
  it("uses FLUFFBUZZ_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("FLUFFBUZZ_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue({
      secrets: {
        providers: {
          default: { source: "env" },
