...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.fluffbuzz.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FluffBuzz hooks")
    ...
    .option("--hook-url <url>", "FluffBuzz hook URL")
    .option("--hook-token <token>", "FluffBuzz hook token")
