...
const SANDBOX_EXAMPLES = {
  main: [
    ["fluffbuzz sandbox list", "List all sandbox containers."],
    ["fluffbuzz sandbox list --browser", "List only browser containers."],
    ["fluffbuzz sandbox recreate --all", "Recreate all containers."],
    ["fluffbuzz sandbox recreate --session main", "Recreate a specific session."],
    ["fluffbuzz sandbox recreate --agent mybot", "Recreate agent containers."],
    ["fluffbuzz sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["fluffbuzz sandbox list", "List all sandbox containers."],
    ["fluffbuzz sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["fluffbuzz sandbox recreate --all", "Recreate all containers."],
    ["fluffbuzz sandbox recreate --session main", "Recreate a specific session."],
    ["fluffbuzz sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["fluffbuzz sandbox recreate --browser --all", "Recreate only browser containers."],
    ["fluffbuzz sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["fluffbuzz sandbox explain", "Show effective sandbox config."],
    ["fluffbuzz sandbox explain --session agent:main:main", "Explain a specific session."],
    ["fluffbuzz sandbox explain --agent work", "Explain an agent sandbox."],
    ["fluffbuzz sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.fluffbuzz.com/cli/sandbox")}\n`,
    )
    .action(() => {
