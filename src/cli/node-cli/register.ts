    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "fluffbuzz node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["fluffbuzz node status", "Check node host service status."],
          ["fluffbuzz node install", "Install the node host service."],
          ["fluffbuzz node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.fluffbuzz.com/cli/node")}\n`,
    );
