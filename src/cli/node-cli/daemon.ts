...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("fluffbuzz node install"), formatCliCommand("fluffbuzz node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("fluffbuzz node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    FLUFFBUZZ_LOG_PREFIX: baseEnv.FLUFFBUZZ_LOG_PREFIX ?? "node",
