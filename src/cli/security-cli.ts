...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fluffbuzz security audit", "Run a local security audit."],
          ["fluffbuzz security audit --deep", "Include best-effort live Gateway probe checks."],
          ["fluffbuzz security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["fluffbuzz security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.fluffbuzz.com/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("FluffBuzz security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("fluffbuzz security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("fluffbuzz security audit --fix")}`));
        if (!fixResult) {
