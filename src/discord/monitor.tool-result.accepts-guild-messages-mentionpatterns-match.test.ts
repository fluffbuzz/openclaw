  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/fluffbuzz",
      },
    },
    session: { store: "/tmp/fluffbuzz-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bfluffbuzz\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "fluffbuzz: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "FluffBuzz" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/fluffbuzz" },
      session: { store: "/tmp/fluffbuzz-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
