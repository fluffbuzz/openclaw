...
function resolveVoiceTtsConfig(params: { cfg: FluffBuzzConfig; override?: TtsConfig }): {
  cfg: FluffBuzzConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredFluffBuzzTmpDir(), "discord-voice-"));
