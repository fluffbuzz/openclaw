...
describe("agent components", () => {
  const createCfg = (): FluffBuzzConfig => ({}) as FluffBuzzConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): FluffBuzzConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/fluffbuzz-sessions-test.json");
  });
