...
homepage: https://docs.fluffbuzz.com/automation/hooks#command-logger
metadata:
  {
    "fluffbuzz":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.fluffbuzz/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.fluffbuzz/logs/commands.log ~/.fluffbuzz/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/fluffbuzz`:
   ```
   /home/username/.fluffbuzz/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
