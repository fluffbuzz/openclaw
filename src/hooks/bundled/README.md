...
This directory contains hooks that ship with FluffBuzz. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.fluffbuzz/fluffbuzz.json`:
...
2. Restart gateway: `pkill -9 -f 'fluffbuzz.*gateway' && pnpm fluffbuzz gateway`
3. Enable the hook: `fluffbuzz hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.fluffbuzz.com/automation/hooks
