...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => FluffBuzzConfig;
  action?: "new" | "reset";
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): FluffBuzzConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
