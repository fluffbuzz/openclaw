...
function telegramCfg(): FluffBuzzConfig {
  return { channels: { telegram: { botToken: "tok" } } } as FluffBuzzConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: FluffBuzzConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as FluffBuzzConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as FluffBuzzConfig;
...
function createSignalAccountOverrideCfg(): FluffBuzzConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as FluffBuzzConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as FluffBuzzConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as FluffBuzzConfig,
        accountId: undefined,
        params: {
