...
describe("message action capability checks", () => {
  ...
  it("aggregates buttons/card support across plugins", () => {
    ...
    expect(supportsChannelMessageButtons({} as FluffBuzzConfig)).toBe(true);
    expect(supportsChannelMessageCards({} as FluffBuzzConfig)).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      supportsChannelMessageButtonsForChannel({ cfg: {} as FluffBuzzConfig, channel: "discord" }),
    ).toBe(true);
    expect(
      supportsChannelMessageButtonsForChannel({ cfg: {} as FluffBuzzConfig, channel: "telegram" }),
    ).toBe(false);
    expect(
      supportsChannelMessageCardsForChannel({ cfg: {} as FluffBuzzConfig, channel: "telegram" }),
    ).toBe(true);
    expect(supportsChannelMessageCardsForChannel({ cfg: {} as FluffBuzzConfig })).toBe(false);
  });
});
