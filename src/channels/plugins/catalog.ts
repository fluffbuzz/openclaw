...
import { discoverFluffBuzzPlugins } from "../../plugins/discovery.js";
import type { FluffBuzzPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, FluffBuzzPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["FLUFFBUZZ_PLUGIN_CATALOG_PATHS", "FLUFFBUZZ_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<FluffBuzzPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: FluffBuzzPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: FluffBuzzPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverFluffBuzzPlugins({ workspaceDir: options.workspaceDir });
