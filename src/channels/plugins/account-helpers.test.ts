...
function cfg(accounts?: Record<string, unknown> | null): FluffBuzzConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as FluffBuzzConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as FluffBuzzConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as FluffBuzzConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as FluffBuzzConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as FluffBuzzConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as FluffBuzzConfig)).toBe("default");
    });
  });
});
