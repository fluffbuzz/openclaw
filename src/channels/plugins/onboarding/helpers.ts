...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in FluffBuzz config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "FLUFFBUZZ_SECRET",
      envVarFormatError:
        'Use an env var name like "FLUFFBUZZ_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
