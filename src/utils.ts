...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".fluffbuzz");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.FLUFFBUZZ_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$FLUFFBUZZ_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via FLUFFBUZZ_STATE_DIR.
