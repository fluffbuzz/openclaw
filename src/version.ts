...
declare const __FLUFFBUZZ_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "fluffbuzz";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["FLUFFBUZZ_VERSION"],
      env["FLUFFBUZZ_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current FluffBuzz version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __FLUFFBUZZ_VERSION__ === "string" ? __FLUFFBUZZ_VERSION__ : undefined,
  bundledVersion: process.env.FLUFFBUZZ_BUNDLED_VERSION,
});
