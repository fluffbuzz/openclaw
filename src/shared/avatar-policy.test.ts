...
describe("avatar policy", () => {
  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    expect(isWorkspaceRelativeAvatarPath("avatars/fluffbuzz.png")).toBe(true);
    expect(isWorkspaceRelativeAvatarPath("C:\\\\avatars\\\\fluffbuzz.png")).toBe(true);
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/fluffbuzz.svg")).toBe(true);
    expect(looksLikeAvatarPath("fluffbuzz.webp")).toBe(true);
