...
describe("shared/frontmatter", () => {
  ...
  test("resolveFluffBuzzManifestBlock parses JSON5 metadata and picks fluffbuzz block", () => {
    const frontmatter = {
      metadata: "{ fluffbuzz: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveFluffBuzzManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveFluffBuzzManifestBlock returns undefined for invalid input", () => {
    expect(resolveFluffBuzzManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveFluffBuzzManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveFluffBuzzManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
