...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "FLUFFBUZZ_SKIP_BROWSER_CONTROL_SERVER",
  "FLUFFBUZZ_SKIP_GMAIL_WATCHER",
  "FLUFFBUZZ_SKIP_CANVAS_HOST",
  "FLUFFBUZZ_BUNDLED_PLUGINS_DIR",
  "FLUFFBUZZ_SKIP_CHANNELS",
  "FLUFFBUZZ_SKIP_PROVIDERS",
  "FLUFFBUZZ_SKIP_CRON",
  "FLUFFBUZZ_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.FLUFFBUZZ_STATE_DIR = path.join(tempHome, ".fluffbuzz");
  delete process.env.FLUFFBUZZ_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.FLUFFBUZZ_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER = "1";
  process.env.FLUFFBUZZ_SKIP_CANVAS_HOST = "1";
  process.env.FLUFFBUZZ_SKIP_CHANNELS = "1";
  process.env.FLUFFBUZZ_SKIP_PROVIDERS = "1";
  process.env.FLUFFBUZZ_SKIP_CRON = "1";
  process.env.FLUFFBUZZ_TEST_MINIMAL_GATEWAY = "1";
  process.env.FLUFFBUZZ_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "fluffbuzz-test-no-bundled-extensions")
    : "fluffbuzz-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "fluffbuzz-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".fluffbuzz-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__fluffbuzzTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__fluffbuzzTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["FLUFFBUZZ_GATEWAY_TOKEN"]);
  const prev = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.FLUFFBUZZ_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
