...
import type { FluffBuzzConfig } from "../../config/types.fluffbuzz.js";
import {
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: FluffBuzzConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadFluffBuzzPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadFluffBuzzPlugins() already cache
