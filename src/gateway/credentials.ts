...
function throwUnresolvedGatewaySecretInput(path: string): never {
  throw new Error(
    [
      ...
      "Fix: set FLUFFBUZZ_GATEWAY_TOKEN/FLUFFBUZZ_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.FLUFFBUZZ_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.FLUFFBOT_GATEWAY_TOKEN);
}
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.FLUFFBUZZ_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.FLUFFBOT_GATEWAY_PASSWORD);
}
