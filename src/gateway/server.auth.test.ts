...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  const deviceIdentityPath = path.join(
    os.tmpdir(),
    `fluffbuzz-auth-rate-limit-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
  try {
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const deviceIdentityPath = path.join(
    os.tmpdir(),
    `fluffbuzz-auth-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.FLUFFBUZZ_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.FLUFFBUZZ_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.FLUFFBUZZ_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.FLUFFBUZZ_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            FLUFFBUZZ_VERSION: " ",
            FLUFFBUZZ_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `fluffbuzz-test-device-${randomUUID()}.json`),
        nonce,
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
      process.env.FLUFFBUZZ_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
    ...
    try {
      await withGatewayServer(async ({ port }) => {
        ...
        const { device } = await createSignedDevice({
          ...
          clientId: GATEWAY_CLIENT_NAMES.CONTROL_UI,
          clientMode: GATEWAY_CLIENT_MODES.WEBCHAT,
          identityPath: path.join(os.tmpdir(), `fluffbuzz-controlui-device-${randomUUID()}.json`),
          nonce: String(nonce),
        });
        const res = await connectReq(ws, {
          token: "secret",
  ...
  test("requires pairing for remote operator device identity with shared token auth", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "fluffbuzz-device-scope-"));
  ...
  test("auto-approves loopback scope upgrades for control ui clients", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "fluffbuzz-device-token-scope-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
  ...
  test("allows operator shared auth with legacy paired metadata", async () => {
    ...
    const { approveDevicePairing, getPairedDevice, listDevicePairing, requestDevicePairing } =
      await import("../infra/device-pairing.js");
    const identityDir = await mkdtemp(join(tmpdir(), "fluffbuzz-device-legacy-meta-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
  ...
  test("auto-approves local scope upgrades even when paired metadata is legacy-shaped", async () => {
    ...
    const identityDir = await mkdtemp(join(tmpdir(), "fluffbuzz-device-legacy-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
    const devicePublicKey = publicKeyRawBase64UrlFromPem(identity.publicKeyPem);
    const seeded = await requestDevicePairing({
      deviceId: identity.deviceId,
