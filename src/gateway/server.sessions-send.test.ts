...
const { createFluffBuzzTools } = await import("../agents/fluffbuzz-tools.js");
...
type SessionSendTool = ReturnType<typeof createFluffBuzzTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createFluffBuzzTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["FLUFFBUZZ_GATEWAY_PORT", "FLUFFBUZZ_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.FLUFFBUZZ_GATEWAY_PORT = String(gatewayPort);
  process.env.FLUFFBUZZ_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("FLUFFBUZZ_CONFIG_PATH missing in gateway test environment");
      }
