...
export function resolveHooksConfig(cfg: FluffBuzzConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: FluffBuzzConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-fluffbuzz-token"] === "string"
      ? req.headers["x-fluffbuzz-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
