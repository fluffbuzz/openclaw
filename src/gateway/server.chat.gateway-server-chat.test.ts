...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-gw-"));
    try {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-gw-"));
