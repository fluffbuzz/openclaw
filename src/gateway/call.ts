...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.FLUFFBUZZ_GATEWAY_URL) ??
      trimToUndefined(process.env.FLUFFBOT_GATEWAY_URL));
  ...
  const allowPrivateWs = process.env.FLUFFBUZZ_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set FLUFFBUZZ_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: fluffbuzz doctor --fix",
...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  return trimToUndefined(env.FLUFFBUZZ_GATEWAY_TOKEN) ?? trimToUndefined(env.FLUFFBOT_GATEWAY_TOKEN);
}

function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    trimToUndefined(env.FLUFFBUZZ_GATEWAY_PASSWORD) ?? trimToUndefined(env.FLUFFBOT_GATEWAY_PASSWORD)
  );
}
