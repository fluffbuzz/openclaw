...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.FLUFFBUZZ_SKIP_CHANNELS;
    prevSkipGmail = process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.FLUFFBUZZ_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.FLUFFBUZZ_SKIP_CHANNELS = "0";
    delete process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER;
    delete process.env.FLUFFBUZZ_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.FLUFFBUZZ_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER;
    } else {
      process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.FLUFFBUZZ_SKIP_PROVIDERS;
    } else {
      process.env.FLUFFBUZZ_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeEnvRefConfig() {
    const configPath = process.env.FLUFFBUZZ_CONFIG_PATH;
    if (!configPath) {
      throw new Error("FLUFFBUZZ_CONFIG_PATH is not set");
    }
    await fs.writeFile(
      configPath,
      `${JSON.stringify(
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.FLUFFBUZZ_STATE_DIR;
    if (!stateDir) {
      throw new Error("FLUFFBUZZ_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_FLUFFBUZZ_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_FLUFFBUZZ_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_FLUFFBUZZ_AUTH_REF" is missing or empty.',
      );
    } finally {
