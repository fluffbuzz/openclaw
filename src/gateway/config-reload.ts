...
import type { FluffBuzzConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: FluffBuzzConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: FluffBuzzConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: FluffBuzzConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: FluffBuzzConfig) => void | Promise<void>;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: FluffBuzzConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: FluffBuzzConfig) => {
