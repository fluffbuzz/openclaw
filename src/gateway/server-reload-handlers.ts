  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (FLUFFBUZZ_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (FLUFFBUZZ_SKIP_CHANNELS=1 or FLUFFBUZZ_SKIP_PROVIDERS=1)",
        );
      } else {
