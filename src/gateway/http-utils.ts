...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-fluffbuzz-agent-id")?.trim() ||
    getHeader(req, "x-fluffbuzz-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(model: string | undefined): string | undefined {
  ...
  const m =
    raw.match(/^fluffbuzz[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-fluffbuzz-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-fluffbuzz-message-channel")) ??
