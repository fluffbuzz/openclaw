...
import { createFluffBuzzTools } from "../agents/fluffbuzz-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-fluffbuzz-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-fluffbuzz-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-fluffbuzz-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-fluffbuzz-thread-id")?.trim() || undefined;
  ...
  const allTools = createFluffBuzzTools({
