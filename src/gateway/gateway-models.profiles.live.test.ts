...
import { resolveFluffBuzzAgentDir } from "../agents/agent-paths.js";
...
import { ensureFluffBuzzModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, FluffBuzzConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.FLUFFBUZZ_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.FLUFFBUZZ_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.FLUFFBUZZ_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.FLUFFBUZZ_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.FLUFFBUZZ_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.FLUFFBUZZ_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: FluffBuzzConfig;
  agentDir: string;
}): FluffBuzzConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<FluffBuzzConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.FLUFFBUZZ_CONFIG_PATH,
    token: process.env.FLUFFBUZZ_GATEWAY_TOKEN,
    skipChannels: process.env.FLUFFBUZZ_SKIP_CHANNELS,
    skipGmail: process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER,
    skipCron: process.env.FLUFFBUZZ_SKIP_CRON,
    skipCanvas: process.env.FLUFFBUZZ_SKIP_CANVAS_HOST,
    agentDir: process.env.FLUFFBUZZ_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.FLUFFBUZZ_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.FLUFFBUZZ_SKIP_CHANNELS = "1";
  process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER = "1";
  process.env.FLUFFBUZZ_SKIP_CRON = "1";
  process.env.FLUFFBUZZ_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.FLUFFBUZZ_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveFluffBuzzAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-live-state-"));
  process.env.FLUFFBUZZ_STATE_DIR = tempStateDir;
  ...
  process.env.FLUFFBUZZ_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.fluffbuzz-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveFluffBuzzAgentDir();
  const sanitizedCfg: FluffBuzzConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-live-"));
  const tempConfigPath = path.join(tempDir, "fluffbuzz.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.FLUFFBUZZ_CONFIG_PATH = tempConfigPath;

  await ensureFluffBuzzModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "FluffBuzz live tool probe (local, safe): " +
    ...
    process.env.FLUFFBUZZ_CONFIG_PATH = previous.configPath;
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = previous.token;
    process.env.FLUFFBUZZ_SKIP_CHANNELS = previous.skipChannels;
    process.env.FLUFFBUZZ_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.FLUFFBUZZ_SKIP_CRON = previous.skipCron;
    process.env.FLUFFBUZZ_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.FLUFFBUZZ_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.FLUFFBUZZ_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureFluffBuzzModelsJson(cfg);
      ...
      const rawModels = process.env.FLUFFBUZZ_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via FLUFFBUZZ_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.fluffbuzz-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
