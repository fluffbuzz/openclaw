...
import { ensureFluffBuzzCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.FLUFFBUZZ_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.FLUFFBUZZ_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "FLUFFBUZZ_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "FLUFFBUZZ_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("fluffbuzz doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("fluffbuzz doctor")}" to repair, then retry.`,
    );
  }
  ...
  let cfgAtStart: FluffBuzzConfig;
  {
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `fluffbuzz config set gateway.auth.mode token` and `fluffbuzz config set gateway.auth.token <token>`.",
      );
    }
  }
  cfgAtStart = (
