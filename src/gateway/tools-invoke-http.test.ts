...
let lastCreateFluffBuzzToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/fluffbuzz-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateFluffBuzzToolsContext?.agentTo,
          agentThreadId: lastCreateFluffBuzzToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createFluffBuzzTools: (ctx: Record<string, unknown>) => {
      lastCreateFluffBuzzToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateFluffBuzzToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-fluffbuzz-message-to": "channel:24514",
        "x-fluffbuzz-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
