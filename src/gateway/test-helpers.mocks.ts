...
type GetReplyFromConfigFn = (
  ...
  configOverride?: FluffBuzzConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `fluffbuzz-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.FLUFFBUZZ_CONFIG_PATH = path.join(root, "fluffbuzz.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "fluffbuzz.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "fluffbuzz-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadFluffBuzzPlugins: () => pluginRegistryState.registry,
  };
});
