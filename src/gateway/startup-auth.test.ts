...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: FluffBuzzConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: FluffBuzzConfig) {
  ...
  async function expectNoTokenGeneration(cfg: FluffBuzzConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("uses FLUFFBUZZ_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
      ...
      env: {
        FLUFFBUZZ_GATEWAY_PASSWORD: "password-from-env",
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          FLUFFBUZZ_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        } as NodeJS.ProcessEnv,
      }),
