...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: FluffBuzzConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: FluffBuzzConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/fluffbuzz-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<FluffBuzzConfig["bindings"]>[number];
