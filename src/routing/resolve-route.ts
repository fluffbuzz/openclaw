...
function listAgents(cfg: FluffBuzzConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: FluffBuzzConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<FluffBuzzConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: FluffBuzzConfig): AgentLookupCache {
...
function pickFirstExistingAgentId(cfg: FluffBuzzConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: FluffBuzzConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<FluffBuzzConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  FluffBuzzConfig,
  {
    bindingsRef: FluffBuzzConfig["bindings"];
    agentsRef: FluffBuzzConfig["agents"];
    sessionRef: FluffBuzzConfig["session"];
...
function resolveRouteCacheForConfig(cfg: FluffBuzzConfig): Map<string, ResolvedAgentRoute> {
