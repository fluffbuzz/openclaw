...
import { buildWebSearchProviderConfig, withTempHome, writeFluffBuzzConfig } from "./test-helpers.js";
import { FluffBuzzSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = FluffBuzzSchema.safeParse({
      $schema: "https://fluffbuzz.com/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://fluffbuzz.com/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = FluffBuzzSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  ...
  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "puppy" } });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = FluffBuzzSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeFluffBuzzConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeFluffBuzzConfig(home, {
        gateway: { bind: "${FLUFFBUZZ_BIND}" },
      });

      const prev = process.env.FLUFFBUZZ_BIND;
      process.env.FLUFFBUZZ_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.FLUFFBUZZ_BIND;
        } else {
          process.env.FLUFFBUZZ_BIND = prev;
        }
      }
    });
  });
