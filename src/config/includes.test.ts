...
const ETC_FLUFFBUZZ_DIR = path.join(ROOT_DIR, "etc", "fluffbuzz");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "fluffbuzz.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcFluffBuzzPath(...parts: string[]) {
  return path.join(ETC_FLUFFBUZZ_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcFluffBuzzPath("agents.json");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-includes-symlink-"));
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "fluffbuzz.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-includes-hardlink-"));
      try {
    ...
    it("rejects oversized include files", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-includes-big-"));
      try {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "fluffbuzz.json")),
