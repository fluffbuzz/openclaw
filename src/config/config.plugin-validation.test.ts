...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "fluffbuzz.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS: process.env.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-config-plugin-validation-"));
    ...
    process.env.FLUFFBUZZ_STATE_DIR = path.join(suiteHome, ".fluffbuzz");
    process.env.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.FLUFFBUZZ_STATE_DIR === undefined) {
      delete process.env.FLUFFBUZZ_STATE_DIR;
    } else {
      process.env.FLUFFBUZZ_STATE_DIR = envSnapshot.FLUFFBUZZ_STATE_DIR;
    }
    if (envSnapshot.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.FLUFFBUZZ_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
