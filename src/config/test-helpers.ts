...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "fluffbuzz-config-" });
}

export async function writeFluffBuzzConfig(home: string, config: unknown): Promise<string> {
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeFluffBuzzConfig(home, config);
