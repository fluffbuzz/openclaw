...
import type { ConfigFileSnapshot } from "./types.fluffbuzz.js";
import { FluffBuzzSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(FluffBuzzSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.fluffbuzz/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/fluffbuzz/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/fluffbuzz/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/fluffbuzz/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/fluffbuzz/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = FluffBuzzSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "FluffBuzzConfig";
    const hints = mainSchemaHints;
