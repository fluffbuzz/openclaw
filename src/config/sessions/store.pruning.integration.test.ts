...
// Keep integration tests deterministic: never read a real fluffbuzz.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.FLUFFBUZZ_SESSION_CACHE_TTL_MS;
    process.env.FLUFFBUZZ_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.FLUFFBUZZ_SESSION_CACHE_TTL_MS;
    } else {
      process.env.FLUFFBUZZ_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("does not archive external transcript paths when capping entries", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-external-cap-"));
  ...
  it("never deletes transcripts outside the agent sessions directory during budget cleanup", async () => {
    ...
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-external-session-"));
    const externalTranscript = path.join(externalDir, "outside.jsonl");
