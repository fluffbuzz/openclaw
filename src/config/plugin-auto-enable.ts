...
function isWhatsAppConfigured(cfg: FluffBuzzConfig): boolean {
...
function isGenericChannelConfigured(cfg: FluffBuzzConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: FluffBuzzConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: FluffBuzzConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: FluffBuzzConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: FluffBuzzConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: FluffBuzzConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: FluffBuzzConfig, pluginId: string): FluffBuzzConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
