...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".fluffbuzz", "fluffbuzz.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".fluffbuzz", "logs", "config-audit.jsonl");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: FluffBuzzConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      } satisfies FluffBuzzConfig;

      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "fluffbuzz config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'fluffbuzz config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          FLUFFBUZZ_WATCH_MODE: "1",
          FLUFFBUZZ_WATCH_SESSION: "watch-session-1",
          FLUFFBUZZ_WATCH_COMMAND: "gateway --force",
