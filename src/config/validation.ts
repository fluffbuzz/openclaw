...
import type { FluffBuzzConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: FluffBuzzConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: FluffBuzzConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: FluffBuzzConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FluffBuzzSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FluffBuzzConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FluffBuzzConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as FluffBuzzConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as FluffBuzzConfig,
  };
}
