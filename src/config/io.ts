...
import type { FluffBuzzConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareFluffBuzzVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  fluffbuzz config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  fluffbuzz config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  ...
  pathSegments: string[],
): { changed: boolean; next: FluffBuzzConfig } {
...
function coerceConfig(value: unknown): FluffBuzzConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as FluffBuzzConfig;
}
...
function stampConfigVersion(cfg: FluffBuzzConfig): FluffBuzzConfig {
...
function warnIfConfigFromFuture(cfg: FluffBuzzConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareFluffBuzzVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FluffBuzz (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as FluffBuzzConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FluffBuzzConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as FluffBuzzConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: FluffBuzzConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as FluffBuzzConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.FLUFFBUZZ_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.FLUFFBUZZ_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.FLUFFBUZZ_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.FLUFFBUZZ_WATCH_SESSION === "string" &&
        deps.env.FLUFFBUZZ_WATCH_SESSION.trim().length > 0
          ? deps.env.FLUFFBUZZ_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.FLUFFBUZZ_WATCH_COMMAND === "string" &&
        deps.env.FLUFFBUZZ_WATCH_COMMAND.trim().length > 0
          ? deps.env.FLUFFBUZZ_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `FLUFFBUZZ_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: FluffBuzzConfig | null = null;
let runtimeConfigSourceSnapshot: FluffBuzzConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FLUFFBUZZ_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FLUFFBUZZ_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): FluffBuzzConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): FluffBuzzConfig {
