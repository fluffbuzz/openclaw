...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ FLUFFBUZZ_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-fluffbuzz",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-fluffbuzz#general");
  });
  ...
  it("derives session transcripts dir from FLUFFBUZZ_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FLUFFBUZZ_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.fluffbuzz");
  ...
  it("resolves cross-agent paths when FLUFFBUZZ_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.fluffbuzz"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
      ...
      expect(sessionFile).toBe(
        path.join(
          path.resolve("/home/user/.fluffbuzz"),
          "agents",
          "bot1",
          "sessions",
          "sess-1.jsonl",
