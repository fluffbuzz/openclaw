...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("fluffbuzz-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "fluffbuzz-sandbox-browser-global" },
      agentBrowser: { network: "fluffbuzz-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("fluffbuzz-sandbox-browser-agent");
  });
