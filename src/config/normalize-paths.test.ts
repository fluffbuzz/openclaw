  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.fluffbuzz/logs/fluffbuzz.log" },
        hooks: {
          path: "~/.fluffbuzz/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.fluffbuzz/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".fluffbuzz", "logs", "fluffbuzz.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".fluffbuzz", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".fluffbuzz", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".fluffbuzz", "agents", "main"));
