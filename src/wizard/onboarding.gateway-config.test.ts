...
describe("configureGatewayForOnboarding", () => {
  ...
  it("prefers FLUFFBUZZ_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.FLUFFBUZZ_GATEWAY_TOKEN;
    process.env.FLUFFBUZZ_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.FLUFFBUZZ_GATEWAY_PASSWORD;
    process.env.FLUFFBUZZ_GATEWAY_PASSWORD = "gateway-secret";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "FLUFFBUZZ_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.FLUFFBUZZ_GATEWAY_PASSWORD = previous;
      }
    }
  });
});
