...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.fluffbuzz/fluffbuzz.json",
    exists: false,
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/fluffbuzz-workspace",
...
describe("runOnboardingWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local onboarding probe", async () => {
    ...
    process.env.FLUFFBUZZ_GATEWAY_PASSWORD = "gateway-ref-password";
