...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "fluffbuzz.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "fluffbuzz.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.FLUFFBUZZ_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.FLUFFBUZZ_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.FLUFFBUZZ_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.FLUFFBUZZ_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[fluffbuzz] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[fluffbuzz] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[fluffbuzz] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[fluffbuzz] ${parsed.error}`);
      process.exit(2);
    }
