...
private const val NOTIFICATION_CHANNEL_BASE_ID = "fluffbuzz.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "FluffBuzz Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "FluffBuzz Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "FluffBuzz Active")
      }
