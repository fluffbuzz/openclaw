...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: FluffBuzzProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: FluffBuzzProtocol.AnyCodable] = [
            "presence": .init([FluffBuzzProtocol.AnyCodable(entry)]),
        ]
        let payload = FluffBuzzProtocol.AnyCodable(payloadMap)
