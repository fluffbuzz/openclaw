...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: FluffBuzzProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let evt = voiceWakeChangedEvent(payload: FluffBuzzProtocol.AnyCodable(["triggers": ["fluffbuzz", "computer"]]))
        ...
        #expect(updated == ["fluffbuzz", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = voiceWakeChangedEvent(payload: FluffBuzzProtocol.AnyCodable(["unexpected": 123]))
