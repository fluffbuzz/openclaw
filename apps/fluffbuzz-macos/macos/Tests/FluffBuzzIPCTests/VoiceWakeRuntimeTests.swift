...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["fluffbuzz"]
        let text = "hey fluffbuzz"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey fluffbuzz do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["fluffbuzz"], minPostTriggerGap: 0.3)
