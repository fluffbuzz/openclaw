...
enum DebugActions {
    private static let verboseDefaultsKey = "fluffbuzz.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "fluffbuzz.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = FluffBuzzPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "FluffBuzz", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FluffBuzzPaths.configURL
        guard
