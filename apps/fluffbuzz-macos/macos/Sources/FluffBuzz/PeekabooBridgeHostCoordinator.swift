...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "com.fluffbuzz", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: FluffBuzzPeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["fluffbot", "clawdis", "fluffbot"]

    private static var fluffbuzzSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "FluffBuzz", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = FluffBuzzPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.fluffbuzzSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.fluffbuzzSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.fluffbuzzSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.fluffbuzzSocketPath)
        } catch {
...
@MainActor
private final class FluffBuzzPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "com.fluffbuzz.peekaboo")
