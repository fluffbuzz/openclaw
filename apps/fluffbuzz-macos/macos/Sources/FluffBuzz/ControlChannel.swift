...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FluffBuzzProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.fluffbuzz", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FluffBuzzKit.AnyCodable]()) {
                $0[$1.key] = FluffBuzzKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: FluffBuzzProtocol.AnyCodable?) -> [String: FluffBuzzProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FluffBuzzProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FluffBuzzKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FluffBuzzProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("fluffbuzz.control.heartbeat")
    static let controlAgentEvent = Notification.Name("fluffbuzz.control.agent")
}
