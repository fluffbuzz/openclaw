...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "com.fluffbuzz", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = FluffBuzzConfigFile.hostKey(sshHost)
        let urlKey = FluffBuzzConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "com.fluffbuzz.remote.tunnel.port", qos: .utility)
            do {
