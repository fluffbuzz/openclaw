...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.fluffbuzz", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FluffBuzzSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FluffBuzzSessionPreviewEntry,
    ...
    private static func cache(payload: FluffBuzzSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FluffBuzzChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FluffBuzzKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FluffBuzzKit.AnyCodable]) -> [FluffBuzzChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FluffBuzzChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FluffBuzzChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FluffBuzzChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FluffBuzzChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FluffBuzzChatMessage) -> String? {
