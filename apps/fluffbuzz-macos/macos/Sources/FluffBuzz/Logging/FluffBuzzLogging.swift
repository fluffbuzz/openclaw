...
enum FluffBuzzLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = FluffBuzzOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = FluffBuzzFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        FluffBuzzLogging.bootstrapIfNeeded()
        let label = FluffBuzzLogging.makeLabel(subsystem: subsystem, category: category)
...
struct FluffBuzzOSLogHandler: AppLogLevelBackedHandler {
...
struct FluffBuzzFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = FluffBuzzLogging.parseLabel(self.label)
