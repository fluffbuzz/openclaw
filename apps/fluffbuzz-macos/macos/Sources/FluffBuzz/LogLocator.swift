...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FLUFFBUZZ_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/fluffbuzz")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("fluffbuzz-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("fluffbuzz-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/fluffbuzz/ (rolling or stdout), or nil if none exist.
