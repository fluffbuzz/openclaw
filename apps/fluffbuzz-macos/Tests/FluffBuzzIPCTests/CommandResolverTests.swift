...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersFluffBuzzBinary() async throws {
        ...
        let fluffbuzzPath = tmp.appendingPathComponent("node_modules/.bin/fluffbuzz")
        try self.makeExec(at: fluffbuzzPath)

        let cmd = CommandResolver.fluffbuzzCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([fluffbuzzPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/fluffbuzz.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.fluffbuzzCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func prefersFluffBuzzBinaryOverPnpm() async throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
        ...
        let fluffbuzzPath = binDir.appendingPathComponent("fluffbuzz")
        ...
        #expect(cmd.prefix(2).elementsEqual([fluffbuzzPath.path, "rpc"]))
    }

    @Test func usesFluffBuzzBinaryWithoutNodeRuntime() async throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "fluffbuzz", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "fluffbuzz", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("fluffbuzz@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/fluffbuzz", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "fluffbuzz@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/fluffbuzz'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("fluffbuzz"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == fluffbuzzPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
