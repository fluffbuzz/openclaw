...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: FluffBuzzProtocol.AnyCodable(["ok": FluffBuzzProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: FluffBuzzProtocol.AnyCodable(["ok": FluffBuzzProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = FluffBuzzProtocol.AnyCodable([
            "runId": FluffBuzzProtocol.AnyCodable("run-1"),
            "sessionKey": FluffBuzzProtocol.AnyCodable("main"),
            "state": FluffBuzzProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: FluffBuzzProtocol.AnyCodable(["a": FluffBuzzProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
