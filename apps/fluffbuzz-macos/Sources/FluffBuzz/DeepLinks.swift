...
private let deepLinkLogger = Logger(subsystem: "com.fluffbuzz", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "FluffBuzz is paused", message: "Unpause FluffBuzz to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run FluffBuzz agent?", message: body) else { return }
        }
