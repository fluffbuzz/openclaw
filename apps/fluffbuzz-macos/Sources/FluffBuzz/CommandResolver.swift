...
enum CommandResolver {
    private static let projectRootDefaultsKey = "fluffbuzz.gatewayProjectRootPath"
    private static let helperName = "fluffbuzz"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let fluffbuzzEntry = root.appendingPathComponent("fluffbuzz.mjs").path
        if FileManager().isReadableFile(atPath: fluffbuzzEntry) { return fluffbuzzEntry }
        let binEntry = root.appendingPathComponent("bin/fluffbuzz.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/fluffbuzz")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let fluffbuzzPaths = self.fluffbuzzManagedPaths(home: home)
        if !fluffbuzzPaths.isEmpty {
            extras.insert(contentsOf: fluffbuzzPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + fluffbuzzPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func fluffbuzzManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".fluffbuzz"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func fluffbuzzExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectFluffBuzzExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("fluffbuzz.mjs").path,
            root.appendingPathComponent("bin/fluffbuzz.js").path,
        ]
    ...
    static func hasAnyFluffBuzzInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.fluffbuzzExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func fluffbuzzNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let fluffbuzzPath = self.projectFluffBuzzExecutable(projectRoot: root) {
            return [fluffbuzzPath, subcommand] + extraArgs
        }
        if let fluffbuzzPath = self.fluffbuzzExecutable(searchPaths: searchPaths) {
            return [fluffbuzzPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "fluffbuzz", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            fluffbuzz entrypoint missing (looked for dist/index.js or fluffbuzz.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real fluffbuzz CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/fluffbuzz"
        ...
        \(cliSection)
        \(projectSection)
        if command -v fluffbuzz >/dev/null 2>&1; then
          CLI="$(command -v fluffbuzz)"
          fluffbuzz \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/fluffbuzz.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/fluffbuzz.mjs"
            node "$PRJ/fluffbuzz.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/fluffbuzz.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/fluffbuzz.js"
            node "$PRJ/bin/fluffbuzz.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent fluffbuzz"
          pnpm --silent fluffbuzz \(quotedArgs);
        else
          echo "fluffbuzz CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? FluffBuzzConfigFile.loadDict()
