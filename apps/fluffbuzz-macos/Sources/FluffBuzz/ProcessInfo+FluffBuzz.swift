...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`com.fluffbuzz.mac`) even if the shipped
    ...
    {
        if environment["FLUFFBUZZ_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "fluffbuzz.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "fluffbuzz.nixMode") { return true }
