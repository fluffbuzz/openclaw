...
private let webChatSwiftLogger = Logger(subsystem: "com.fluffbuzz", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: FluffBuzzChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> FluffBuzzChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(FluffBuzzChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> FluffBuzzChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                FluffBuzzGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    FluffBuzzChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    FluffBuzzAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<FluffBuzzChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any FluffBuzzChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = FluffBuzzChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: FluffBuzzChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<FluffBuzzChatView>) -> NSViewController
    {
