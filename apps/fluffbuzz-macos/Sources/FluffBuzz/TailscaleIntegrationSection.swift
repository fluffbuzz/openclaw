...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.fluffbuzz/fluffbuzz.json. Prefer FLUFFBUZZ_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.fluffbuzz/fluffbuzz.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.fluffbuzz/fluffbuzz.json. Restart the gateway to apply."
        }
