...
struct ChatMessageBubble: View {
    let message: FluffBuzzChatMessage
    let style: FluffBuzzChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? FluffBuzzChatTheme.userText : FluffBuzzChatTheme.assistantText
    ...
    private var inlineAttachments: [FluffBuzzChatMessageContent] {
    ...
    private var toolCalls: [FluffBuzzChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [FluffBuzzChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? FluffBuzzChatTheme.userBubble
        }
        if self.style == .onboarding {
            return FluffBuzzChatTheme.onboardingAssistantBubble
        }
        return FluffBuzzChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return FluffBuzzChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: FluffBuzzChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? FluffBuzzChatTheme.userText : FluffBuzzChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: FluffBuzzChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(FluffBuzzChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(FluffBuzzChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [FluffBuzzChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: FluffBuzzChatTheme.assistantText)
            }
        }
    }
}
