...
@MainActor
struct FluffBuzzChatComposer: View {
    @Bindable var viewModel: FluffBuzzChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(FluffBuzzChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(FluffBuzzChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \FluffBuzzPendingAttachment.id)
                { (att: FluffBuzzPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            FluffBuzzPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(FluffBuzzChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(FluffBuzzChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(FluffBuzzChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(FluffBuzzChatTheme.subtleCard)
        .clipShape(Capsule())
    }
