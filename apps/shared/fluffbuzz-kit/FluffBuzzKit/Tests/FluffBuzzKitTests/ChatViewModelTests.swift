...
import Foundation
import Testing
@testable import FluffBuzzChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> FluffBuzzChatHistoryPayload
{
    FluffBuzzChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> FluffBuzzChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [FluffBuzzChatHistoryPayload],
    sessionsResponses: [FluffBuzzChatSessionsListResponse] = []) async -> (TestChatTransport, FluffBuzzChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { FluffBuzzChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: FluffBuzzChatViewModel,
...
private func sendUserMessage(_ vm: FluffBuzzChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, FluffBuzzChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FluffBuzzChatHistoryPayload]
    private let sessionsResponses: [FluffBuzzChatSessionsListResponse]

    private let stream: AsyncStream<FluffBuzzChatTransportEvent>
    private let continuation: AsyncStream<FluffBuzzChatTransportEvent>.Continuation

    init(
        historyResponses: [FluffBuzzChatHistoryPayload],
        sessionsResponses: [FluffBuzzChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<FluffBuzzChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> FluffBuzzChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FluffBuzzChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [FluffBuzzChatAttachmentPayload]) async throws -> FluffBuzzChatSendResponse
    {
        ...
        return FluffBuzzChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FluffBuzzChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FluffBuzzChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: FluffBuzzChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = FluffBuzzChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = FluffBuzzChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { FluffBuzzChatViewModel(sessionKey: "main", transport: transport) }
