...
private let chatUILogger = Logger(subsystem: "com.fluffbuzz", category: "FluffBuzzChatUI")

@MainActor
@Observable
public final class FluffBuzzChatViewModel {
    public private(set) var messages: [FluffBuzzChatMessage] = []
    ...
    public var attachments: [FluffBuzzPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FluffBuzzChatPendingToolCall] = []
    public private(set) var sessions: [FluffBuzzChatSessionEntry] = []
    private let transport: any FluffBuzzChatTransport
    ...
    private var pendingToolCallsById: [String: FluffBuzzChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any FluffBuzzChatTransport) {
    ...
    public var sessionChoices: [FluffBuzzChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [FluffBuzzChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FluffBuzzPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FluffBuzzChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FluffBuzzChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: FluffBuzzChatMessage) -> FluffBuzzChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> FluffBuzzChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return FluffBuzzChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: FluffBuzzChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [FluffBuzzChatMessage],
        incoming: [FluffBuzzChatMessage]) -> [FluffBuzzChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [FluffBuzzChatMessage]) -> [FluffBuzzChatMessage] {
        var result: [FluffBuzzChatMessage] = []
    ...
    private static func dedupeKey(for message: FluffBuzzChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FluffBuzzChatMessageContent] = [
            FluffBuzzChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> FluffBuzzChatAttachmentPayload in
            FluffBuzzChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FluffBuzzChatSessionEntry {
        FluffBuzzChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: FluffBuzzChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FluffBuzzChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FluffBuzzAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FluffBuzzChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FluffBuzzPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
