...
enum FluffBuzzChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FluffBuzzChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FluffBuzzChatTheme.onboardingAssistantBubble"),
...
enum FluffBuzzPlatformImageFactory {
    static func image(_ image: FluffBuzzPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
