...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "com.fluffbuzz.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to FluffBuzz", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to FluffBuzz gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to FluffBuzz.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "FluffBuzzShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "FluffBuzz is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "FluffBuzz Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("fluffbuzz-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
            ...
            try await gateway.connect(
                ...
                token: config.token,
                password: config.password,
                connectOptions: makeOptions("fluffbot-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
