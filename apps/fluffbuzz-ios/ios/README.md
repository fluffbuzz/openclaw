# FluffBuzz iOS (Super Alpha)

NO TEST FLIGHT AVAILABLE AT THIS POINT

This iPhone app is super-alpha and internal-use only. It connects to an FluffBuzz Gateway as a `role: node`.
...
- `apps/ios/Sources/FluffBuzz.entitlements` sets `aps-environment` to `development`.
