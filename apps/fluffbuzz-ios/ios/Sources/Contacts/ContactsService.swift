...
final class ContactsService: ContactsServicing {
    ...
    func search(params: FluffBuzzContactsSearchParams) async throws -> FluffBuzzContactsSearchPayload {
        ...
        return FluffBuzzContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: FluffBuzzContactsAddParams) async throws -> FluffBuzzContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return FluffBuzzContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return FluffBuzzContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> FluffBuzzContactPayload {
        FluffBuzzContactPayload(
