...
@MainActor
final class FluffBuzzAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "com.fluffbuzz.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "com.fluffbuzz.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "com.fluffbuzz.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "fluffbuzz.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "fluffbuzz.watch.promptId"
    static let sessionKeyKey = "fluffbuzz.watch.sessionKey"
    static let actionPrimaryIDKey = "fluffbuzz.watch.action.primary.id"
    static let actionPrimaryLabelKey = "fluffbuzz.watch.action.primary.label"
    static let actionSecondaryIDKey = "fluffbuzz.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "fluffbuzz.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "fluffbuzz.watch.action.primary"
    static let actionSecondaryIdentifier = "fluffbuzz.watch.action.secondary"
    static let actionIdentifierPrefix = "fluffbuzz.watch.action."
    static let actionIDKeyPrefix = "fluffbuzz.watch.action.id."
    static let actionLabelKeyPrefix = "fluffbuzz.watch.action.label."
    static let categoryPrefix = "fluffbuzz.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: FluffBuzzWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> FluffBuzzWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FluffBuzzWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "FluffBuzz" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [FluffBuzzWatchAction]) -> [UNNotificationAction] {
...
@main
struct FluffBuzzApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(FluffBuzzAppDelegate.self) private var appDelegate
...
extension FluffBuzzApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("FluffBuzz: installing uncaught exception handler")
