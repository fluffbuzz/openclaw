...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "com.fluffbuzz.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "com.fluffbuzz.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "com.fluffbuzz.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "com.fluffbuzz.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = FluffBuzzCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = FluffBuzzCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = FluffBuzzCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = FluffBuzzCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = FluffBuzzCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: FluffBuzzLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(FluffBuzzGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: FluffBuzzNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: FluffBuzzNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FluffBuzzNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(FluffBuzzLocationGetParams.self, from: req.paramsJSON)) ??
            FluffBuzzLocationGetParams()
        ...
        let payload = FluffBuzzLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(FluffBuzzCanvasPresentParams.self, from: req.paramsJSON)) ??
                FluffBuzzCanvasPresentParams()
        ...
        case FluffBuzzCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case FluffBuzzCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(FluffBuzzCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case FluffBuzzCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(FluffBuzzCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FluffBuzzCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(FluffBuzzCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case FluffBuzzCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.fluffbuzzA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing fluffbuzzA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzCanvasA2UICommand.push.rawValue, FluffBuzzCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [FluffBuzzKit.AnyCodable]
            if command == FluffBuzzCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(FluffBuzzCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try FluffBuzzCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(FluffBuzzCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try FluffBuzzCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.fluffbuzzA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FluffBuzzCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(FluffBuzzCameraSnapParams.self, from: req.paramsJSON)) ??
                FluffBuzzCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FluffBuzzCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(FluffBuzzCameraClipParams.self, from: req.paramsJSON)) ??
                FluffBuzzCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(FluffBuzzScreenRecordParams.self, from: req.paramsJSON)) ??
            FluffBuzzScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(FluffBuzzSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FluffBuzzNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FluffBuzzNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FluffBuzzNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(FluffBuzzChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FluffBuzzNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = FluffBuzzChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(FluffBuzzPhotosLatestParams.self, from: req.paramsJSON)) ??
            FluffBuzzPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(FluffBuzzContactsSearchParams.self, from: req.paramsJSON)) ??
                FluffBuzzContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzContactsCommand.add.rawValue:
            let params = try Self.decodeParams(FluffBuzzContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(FluffBuzzCalendarEventsParams.self, from: req.paramsJSON)) ??
                FluffBuzzCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(FluffBuzzCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(FluffBuzzRemindersListParams.self, from: req.paramsJSON)) ??
                FluffBuzzRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(FluffBuzzRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(FluffBuzzMotionActivityParams.self, from: req.paramsJSON)) ??
                FluffBuzzMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(FluffBuzzPedometerParams.self, from: req.paramsJSON)) ??
                FluffBuzzPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([FluffBuzzLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            FluffBuzzCanvasCommand.present.rawValue,
            FluffBuzzCanvasCommand.hide.rawValue,
            FluffBuzzCanvasCommand.navigate.rawValue,
            FluffBuzzCanvasCommand.evalJS.rawValue,
            FluffBuzzCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FluffBuzzCanvasA2UICommand.reset.rawValue,
            FluffBuzzCanvasA2UICommand.push.rawValue,
            FluffBuzzCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([FluffBuzzScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([FluffBuzzSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([FluffBuzzChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FluffBuzzWatchCommand.status.rawValue,
            FluffBuzzWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([FluffBuzzPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FluffBuzzRemindersCommand.list.rawValue,
            FluffBuzzRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FluffBuzzTalkCommand.pttStart.rawValue,
            FluffBuzzTalkCommand.pttStop.rawValue,
            FluffBuzzTalkCommand.pttCancel.rawValue,
            FluffBuzzTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FluffBuzzWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = FluffBuzzWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FluffBuzzWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(FluffBuzzWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = FluffBuzzWatchNotifyPayload(
    ...
    func locationMode() -> FluffBuzzLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return FluffBuzzLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to FluffBuzz."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to FluffBuzz."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "fluffbuzz-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "FluffBuzz Share Self-Test",
            url: URL(string: "https://fluffbuzz.com/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.fluffbuzzPushKind(userInfo)
        let receivedMessage =
    ...
    private static func fluffbuzzPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["fluffbuzz"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["fluffbuzz"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
