...
struct OnboardingWizardView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Connect to your FluffBuzz gateway")
                .font(.subheadline)
                .foregroundStyle(.secondary)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `fluffbuzz devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `fluffbuzz devices approve` (or `fluffbuzz devices approve <requestId>`)\n"
                            + "2) `/pair approve` in Telegram\n"
                            + "\(requestLine)\n"
                            + "FluffBuzz will also retry automatically when you return to this app.")
                }
            }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "fluffbuzz.local"
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "fluffbuzz.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "fluffbuzz.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "fluffbuzz.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "fluffbuzz.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
