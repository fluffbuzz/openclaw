...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [FluffBuzzCapability.canvas.rawValue, FluffBuzzCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(FluffBuzzCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(FluffBuzzCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = FluffBuzzLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(FluffBuzzCapability.location.rawValue) }

        caps.append(FluffBuzzCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(FluffBuzzCapability.watch.rawValue)
        }
        caps.append(FluffBuzzCapability.photos.rawValue)
        caps.append(FluffBuzzCapability.contacts.rawValue)
        caps.append(FluffBuzzCapability.calendar.rawValue)
        caps.append(FluffBuzzCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(FluffBuzzCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            FluffBuzzScreenCommand.record.rawValue,
            FluffBuzzSystemCommand.notify.rawValue,
            FluffBuzzChatCommand.push.rawValue,
        ...
        if caps.contains(FluffBuzzCapability.camera.rawValue) {
            commands.append(FluffBuzzCameraCommand.list.rawValue)
            commands.append(FluffBuzzCameraCommand.snap.rawValue)
            commands.append(FluffBuzzCameraCommand.clip.rawValue)
        }
        if caps.contains(FluffBuzzCapability.location.rawValue) {
            commands.append(FluffBuzzLocationCommand.get.rawValue)
        }
        if caps.contains(FluffBuzzCapability.device.rawValue) {
            commands.append(FluffBuzzDeviceCommand.status.rawValue)
            commands.append(FluffBuzzDeviceCommand.info.rawValue)
        }
        if caps.contains(FluffBuzzCapability.watch.rawValue) {
            commands.append(FluffBuzzWatchCommand.status.rawValue)
            commands.append(FluffBuzzWatchCommand.notify.rawValue)
        }
        if caps.contains(FluffBuzzCapability.photos.rawValue) {
            commands.append(FluffBuzzPhotosCommand.latest.rawValue)
        }
        if caps.contains(FluffBuzzCapability.contacts.rawValue) {
            commands.append(FluffBuzzContactsCommand.search.rawValue)
            commands.append(FluffBuzzContactsCommand.add.rawValue)
        }
        if caps.contains(FluffBuzzCapability.calendar.rawValue) {
            commands.append(FluffBuzzCalendarCommand.events.rawValue)
            commands.append(FluffBuzzCalendarCommand.add.rawValue)
        }
        if caps.contains(FluffBuzzCapability.reminders.rawValue) {
            commands.append(FluffBuzzRemindersCommand.list.rawValue)
            commands.append(FluffBuzzRemindersCommand.add.rawValue)
        }
        if caps.contains(FluffBuzzCapability.motion.rawValue) {
            commands.append(FluffBuzzMotionCommand.activity.rawValue)
            commands.append(FluffBuzzMotionCommand.pedometer.rawValue)
        }
