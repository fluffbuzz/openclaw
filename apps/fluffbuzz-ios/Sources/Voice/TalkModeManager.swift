...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__FLUFFBUZZ_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<FluffBuzzTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "com.fluffbuzz", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = FluffBuzzTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> FluffBuzzTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return FluffBuzzTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> FluffBuzzTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> FluffBuzzTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return FluffBuzzTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> FluffBuzzTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = FluffBuzzTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: FluffBuzzTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: FluffBuzzAgentEventPayload.self
            ) else {
                continue
            }
