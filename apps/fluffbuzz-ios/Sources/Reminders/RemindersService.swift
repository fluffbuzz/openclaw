...
final class RemindersService: RemindersServicing {
    func list(params: FluffBuzzRemindersListParams) async throws -> FluffBuzzRemindersListPayload {
        ...
        let payload: [FluffBuzzReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return FluffBuzzReminderPayload(
        ...
        return FluffBuzzRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: FluffBuzzRemindersAddParams) async throws -> FluffBuzzRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = FluffBuzzReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return FluffBuzzRemindersAddPayload(reminder: payload)
    }
