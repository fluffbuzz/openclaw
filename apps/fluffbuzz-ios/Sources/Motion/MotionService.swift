...
final class MotionService: MotionServicing {
    func activities(params: FluffBuzzMotionActivityParams) async throws -> FluffBuzzMotionActivityPayload {
        ...
        let mapped: [FluffBuzzMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return FluffBuzzMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: FluffBuzzPedometerParams) async throws -> FluffBuzzPedometerPayload {
        ...
        let payload: FluffBuzzPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = FluffBuzzPedometerPayload(
