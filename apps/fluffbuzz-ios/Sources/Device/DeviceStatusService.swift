...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> FluffBuzzDeviceStatusPayload {
        ...
        return FluffBuzzDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> FluffBuzzDeviceInfoPayload {
        ...
        return FluffBuzzDeviceInfoPayload(
    ...
    private func batteryStatus() -> FluffBuzzBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: FluffBuzzBatteryState = switch device.batteryState {
        ...
        return FluffBuzzBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> FluffBuzzThermalStatusPayload {
        let state: FluffBuzzThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return FluffBuzzThermalStatusPayload(state: state)
    }

    private func storageStatus() -> FluffBuzzStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return FluffBuzzStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
