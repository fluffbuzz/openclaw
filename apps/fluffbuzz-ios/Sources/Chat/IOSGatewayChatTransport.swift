...
struct IOSGatewayChatTransport: FluffBuzzChatTransport, Sendable {
    private static let logger = Logger(subsystem: "com.fluffbuzz", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> FluffBuzzChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(FluffBuzzChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> FluffBuzzChatHistoryPayload {
        ...
        return try JSONDecoder().decode(FluffBuzzChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [FluffBuzzChatAttachmentPayload]) async throws -> FluffBuzzChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [FluffBuzzChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(FluffBuzzChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(FluffBuzzGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FluffBuzzGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FluffBuzzChatEventPayload.self)
                        {
