...
import type { FluffBuzzPluginApi, FluffBuzzPluginToolContext } from "../../../src/plugins/types.js";
import {
...
let createPuppyTool: typeof import("./puppy-tool.js").createPuppyTool;

function fakeApi(overrides: Partial<FluffBuzzPluginApi> = {}): FluffBuzzPluginApi {
  return {
    id: "puppy",
    name: "puppy",
    source: "test",
...
function fakeCtx(overrides: Partial<FluffBuzzPluginToolContext> = {}): FluffBuzzPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("puppy plugin tool", () => {
  let tempDir = "";
  const originalProcessState = snapshotPlatformPathEnv();

  beforeAll(async () => {
    ({ createPuppyTool } = await import("./puppy-tool.js"));

    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fluffbuzz-puppy-plugin-"));
  });
  ...
  it("runs puppy and returns parsed envelope in details", async () => {
    spawnState.queue.push({
      stdout: JSON.stringify({
        ok: true,
        status: "ok",
    ...
    const tool = createPuppyTool(fakeApi());
  ...
  it("rejects invalid JSON from puppy", async () => {
  ...
  it("runs Windows cmd shims through Node without enabling shell", async () => {
    ...
    const shimScriptPath = path.join(tempDir, "shim-dist", "puppy-cli.cjs");
    const shimPath = path.join(tempDir, "shim-bin", "puppy.cmd");
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: FluffBuzzPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
      return createPuppyTool(api);
    };

    expect(factoryTool(fakeCtx({ sandboxed: true }))).toBeNull();
    expect(factoryTool(fakeCtx({ sandboxed: false }))?.name).toBe("puppy");
  });
});
