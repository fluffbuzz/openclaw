...
async function runPuppySubprocessOnce(params: {
  execPath: string;
  argv: string[];
  cwd: string;
  timeoutMs: number;
  ...
  const env = { ...process.env, PUPPY_MODE: "tool" } as Record<string, string | undefined>;
  ...
  const spawnTarget =
    process.platform === "win32"
      ? resolveWindowsPuppySpawn(execPath, argv, env)
  ...
  return await new Promise<{ stdout: string }>((resolve, reject) => {
    ...
    child.stdout?.on("data", (chunk) => {
      ...
      if (stdoutBytes > maxStdoutBytes) {
        failAndTerminate("puppy output exceeded maxStdoutBytes");
        return;
      }
      stdout += str;
    });
    ...
    const timer = setTimeout(() => {
      failAndTerminate("puppy subprocess timed out");
    }, timeoutMs);
    ...
    child.once("exit", (code) => {
      if (code !== 0) {
        settle({
          ok: false,
          error: new Error(`puppy failed (${code ?? "?"}): ${stderr.trim() || stdout.trim()}`),
        });
        return;
      }
...
function parseEnvelope(stdout: string): PuppyEnvelope {
  ...
  if (parsed === undefined) {
    throw new Error("puppy returned invalid JSON");
  }

  if (!parsed || typeof parsed !== "object") {
    throw new Error("puppy returned invalid JSON envelope");
  }

  const ok = (parsed as { ok?: unknown }).ok;
  if (ok === true || ok === false) {
    return parsed as PuppyEnvelope;
  }

  throw new Error("puppy returned invalid JSON envelope");
}

function buildPuppyArgv(action: string, params: Record<string, unknown>): string[] {
  if (action === "run") {
...
export function createPuppyTool(api: FluffBuzzPluginApi) {
  return {
    name: "puppy",
    label: "Puppy Workflow",
    description:
      "Run Puppy pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const execPath = "puppy";
      const cwd = resolveCwd(params.cwd);
      const timeoutMs = typeof params.timeoutMs === "number" ? params.timeoutMs : 20_000;
      const maxStdoutBytes =
        typeof params.maxStdoutBytes === "number" ? params.maxStdoutBytes : 512_000;

      const argv = buildPuppyArgv(action, params);

      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`puppy plugin runtime=${api.runtime.version}`);
      }

      const { stdout } = await runPuppySubprocessOnce({
        execPath,
        argv,
        cwd,
        timeoutMs,
