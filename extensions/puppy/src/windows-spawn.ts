...
function resolveBinEntry(binField: string | Record<string, string> | undefined): string | null {
  ...
  const preferred = binField.puppy;
...
function resolvePuppyScriptFromPackageJson(wrapperPath: string): string | null {
  const wrapperDir = path.dirname(wrapperPath);
  const packageDirs = [
    // Local install: <repo>/node_modules/.bin/puppy.cmd -> ../puppy
    path.resolve(wrapperDir, "..", "puppy"),
    // Global npm install: <npm-prefix>/puppy.cmd -> ./node_modules/puppy
    path.resolve(wrapperDir, "node_modules", "puppy"),
  ];
...
function resolvePuppyScriptFromCmdShim(wrapperPath: string): string | null {
...
export function resolveWindowsPuppySpawn(
  ...
  const scriptPath =
    resolvePuppyScriptFromCmdShim(resolvedExecPath) ??
    resolvePuppyScriptFromPackageJson(resolvedExecPath);
  if (!scriptPath) {
    throw new Error(
      `${path.basename(resolvedExecPath)} wrapper resolved, but no Node entrypoint could be resolved without shell execution. Ensure Puppy is installed and runnable on PATH (prefer puppy.exe).`,
    );
  }
