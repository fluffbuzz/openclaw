...
function resolveGatewayPort(cfg: FluffBuzzPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.FLUFFBUZZ_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.FLUFFBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: FluffBuzzPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.FLUFFBUZZ_GATEWAY_TOKEN,
      process.env.FLUFFBOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.FLUFFBUZZ_GATEWAY_PASSWORD,
      process.env.FLUFFBOT_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: FluffBuzzPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the FluffBuzz iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the FluffBuzz iOS app:",
            "",
            "```",
            qrAscii,
            "```",
