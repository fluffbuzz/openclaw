...
import type { ReplyPayload, FluffBuzzConfig } from "fluffbuzz/plugin-sdk";
import { createReplyPrefixOptions } from "fluffbuzz/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // FluffBuzzConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as FluffBuzzConfig;
