/**
 * Twitch channel plugin for FluffBuzz.
 *
...
import { buildChannelConfigSchema } from "fluffbuzz/plugin-sdk";
 ...
 * for FluffBuzz. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: FluffBuzzConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: FluffBuzzConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: FluffBuzzConfig): boolean => {
