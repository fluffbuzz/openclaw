...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: FLUFFBUZZ_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: FluffBuzzConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var FLUFFBUZZ_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.FLUFFBUZZ_TWITCH_ACCESS_TOKEN?.trim();
