...
vi.mock("fluffbuzz/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("fluffbuzz/plugin-sdk")>("fluffbuzz/plugin-sdk");
  return {
    ...actual,
...
import type { FluffBuzzPluginServiceContext } from "fluffbuzz/plugin-sdk";
import { emitDiagnosticEvent } from "fluffbuzz/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/fluffbuzz-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): FluffBuzzPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("fluffbuzz.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("fluffbuzz.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("fluffbuzz.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("fluffbuzz.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("fluffbuzz.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("fluffbuzz.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("fluffbuzz.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("fluffbuzz.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("fluffbuzz.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("fluffbuzz.webhook.processed");
    expect(spanNames).toContain("fluffbuzz.message.processed");
    expect(spanNames).toContain("fluffbuzz.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["fluffbuzz.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("fluffbuzz.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "fluffbuzz.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["fluffbuzz.reason"]).toBe("string");
    expect(String(attrs?.["fluffbuzz.reason"])).not.toContain(
