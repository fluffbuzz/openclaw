...
Designed to be called from workflow engines (for example, Puppy via
`fluffbuzz.invoke --each`) without adding new FluffBuzz code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Puppy) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on FluffBuzz internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FluffBuzz extension (like `puppy`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.fluffbuzz/extensions` as a standalone plugin directory.
