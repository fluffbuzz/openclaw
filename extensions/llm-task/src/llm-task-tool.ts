...
import Ajv from "ajv";
import { resolvePreferredFluffBuzzTmpDir } from "fluffbuzz/plugin-sdk";
// NOTE: This extension is intended to be bundled with FluffBuzz.
// When running from source (tests/dev), FluffBuzz internals live under src/.
...
export function createLlmTaskTool(api: FluffBuzzPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Puppy workflows via fluffbuzz.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredFluffBuzzTmpDir(), "fluffbuzz-llm-task-"),
        );
