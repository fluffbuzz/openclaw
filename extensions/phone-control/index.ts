...
import type { FluffBuzzPluginApi, FluffBuzzPluginService } from "fluffbuzz/plugin-sdk";
...
function normalizeDenyList(cfg: FluffBuzzPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: FluffBuzzPluginApi["config"]): string[] {
...
export default function register(api: FluffBuzzPluginApi) {
  ...
  const timerService: FluffBuzzPluginService = {
