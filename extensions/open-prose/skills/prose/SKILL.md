...
metadata: { "fluffbuzz": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == FluffBuzz `sessions_spawn`
- **File I/O** == FluffBuzz `read`/`write`
- **Remote fetch** == FluffBuzz `web_fetch` (or `exec` with curl when POST is required)
