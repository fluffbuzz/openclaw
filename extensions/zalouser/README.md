...
FluffBuzz extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
fluffbuzz channels login --channel zalouser              # Login via QR
...
```bash
# Text
fluffbuzz message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
fluffbuzz message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `fluffbuzz channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
fluffbuzz channels login --channel zalouser --account work
fluffbuzz message send --channel zalouser --account work --target <id> --message "Hello"
