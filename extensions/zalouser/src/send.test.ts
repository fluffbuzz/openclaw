...
describe("zalouser send helpers", () => {
  ...
  it("uses env profile fallback and builds link command", async () => {
    ...
    const result = await sendLinkZalouser("thread-5", " https://fluffbuzz.com ", { isGroup: true });

    expect(mockRunZca).toHaveBeenCalledWith(
      ["msg", "link", "thread-5", "https://fluffbuzz.com", "-g"],
      { profile: "env-profile" },
    );
  ...
  it("returns caught command errors", async () => {
    ...
    await expect(sendLinkZalouser("thread-6", "https://fluffbuzz.com")).resolves.toEqual({
      ok: false,
