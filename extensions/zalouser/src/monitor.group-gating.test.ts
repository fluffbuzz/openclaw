import type { FluffBuzzConfig, PluginRuntime, RuntimeEnv } from "fluffbuzz/plugin-sdk";
...
function createConfig(): FluffBuzzConfig {
  return {
    channels: {
      zalouser: {
        enabled: true,
    ...
    channel: {
      ...
      groups: {
        resolveRequireMention: vi.fn((input) => {
          const cfg = input.cfg as FluffBuzzConfig;
