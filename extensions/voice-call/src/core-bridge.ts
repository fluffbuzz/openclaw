...
function resolveFluffBuzzRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.FLUFFBUZZ_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set FLUFFBUZZ_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveFluffBuzzRoot(), "dist", "extensionAPI.js");
