...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["fluffbuzzPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["fluffbuzz", "pollId"]) ??
    readNestedString(value, ["fluffbuzz", "poll", "id"]) ??
    readNestedString(value, ["data", "fluffbuzzPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "fluffbuzz", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          fluffbuzzPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "fluffbuzz poll vote",
          displayText: "Vote recorded",
          value: { fluffbuzzPollId: pollId, pollId },
        },
      },
    ],
  };
