...
describe("loginGeminiCliOAuth", () => {
  ...
  beforeEach(() => {
    ...
    process.env.FLUFFBUZZ_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.FLUFFBUZZ_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret";
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "puppy@fluffbuzz.com" });
      }
      if (url === LOAD_PROD) {
