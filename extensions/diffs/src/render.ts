...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-fluffbuzz-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-fluffbuzz-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
function buildHtmlDocument(params: {
  ...
  return `<!doctype html>
<html lang="en"${params.runtimeMode === "image" ? ' data-fluffbuzz-diffs-ready="true"' : ""}>
  <head>
    <meta charset="utf-8" />
