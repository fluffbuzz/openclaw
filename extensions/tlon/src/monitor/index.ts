import type { RuntimeEnv, ReplyPayload, FluffBuzzConfig } from "fluffbuzz/plugin-sdk";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "fluffbuzz/plugin-sdk";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as FluffBuzzConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in FluffBuzz config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your FluffBuzz config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.fluffbuzz.com/concepts/session#secure-dm-mode`;
