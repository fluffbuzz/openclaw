...
import type { FluffBuzzConfig, PluginRuntime } from "fluffbuzz/plugin-sdk";
import type { DynamicAgentCreationConfig } from "./types.js";

export type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: FluffBuzzConfig;
  agentId?: string;
};
...
export async function maybeCreateDynamicAgent(params: {
  ...
  if (existingAgent) {
    ...
    const updatedCfg: FluffBuzzConfig = {
      ...cfg,
      bindings: [
        ...existingBindings,
        {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.fluffbuzz/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.fluffbuzz/agents/{agentId}/agent";
