import type { AnyAgentTool, FluffBuzzPluginApi } from "fluffbuzz/plugin-sdk";
...
export function createToolFactoryHarness(cfg: FluffBuzzPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<FluffBuzzPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
