...
import { createDedupeCache, createPersistentDedupe } from "fluffbuzz/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.FLUFFBUZZ_STATE_DIR?.trim() || env.FLUFFBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["fluffbuzz-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".fluffbuzz");
}
