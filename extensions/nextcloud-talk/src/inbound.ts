...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as FluffBuzzConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as FluffBuzzConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as FluffBuzzConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as FluffBuzzConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as FluffBuzzConfig);
