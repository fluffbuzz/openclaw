---
description: Update Fluffbot from upstream when branch has diverged (ahead/behind)
---

# Fluffbot Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Fluffbot" || true

# Move old version
mv /Applications/Fluffbot.app /tmp/Fluffbot-backup.app

# Install new build
cp -R dist/Fluffbot.app /Applications/

# Launch
open /Applications/Fluffbot.app
```
...
pnpm fluffbot health

# Verify no zombie processes
ps aux | grep -E "(fluffbot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm fluffbot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update fluffbot.json with fallback chains
...
pnpm fluffbot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
