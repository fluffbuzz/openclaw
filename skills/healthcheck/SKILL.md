---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for FluffBuzz deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, FluffBuzz cron scheduling for periodic checks, or version status checks on a machine running FluffBuzz (laptop, workstation, Pi, VPS).
---

# FluffBuzz Host Hardening

## Overview

Assess and harden the host running FluffBuzz, then align it to a user-defined risk tolerance without breaking access. Use FluffBuzz security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FluffBuzz changes the host firewall, SSH, or OS updates; it does not.
...
5. FluffBuzz gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FluffBuzz can access sensitive data.
...
### 2) Run FluffBuzz security audits (read-only)

As part of the default read-only checks, run `fluffbuzz security audit --deep`. Only offer alternatives if the user requests them:

1. `fluffbuzz security audit` (faster, non-probing)
2. `fluffbuzz security audit --json` (structured output)

Offer to apply FluffBuzz safe defaults (numbered):

1. `fluffbuzz security audit --fix`

Be explicit that `--fix` only tightens FluffBuzz defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FluffBuzz version/update status (read-only)

As part of the default read-only checks, run `fluffbuzz update status`.
...
- Credential hygiene notes (location of FluffBuzz creds, prefer disk encryption)
...
- FluffBuzz security audit (re-run)
...
After FluffBuzz install or first hardening pass, run at least one baseline audit and version check:

- `fluffbuzz security audit`
- `fluffbuzz security audit --deep`
- `fluffbuzz update status`

Ongoing monitoring is recommended. Use the FluffBuzz cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `fluffbuzz cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `fluffbuzz update status`
...
Before creating, `fluffbuzz cron list` and match on exact `name`. If found, `fluffbuzz cron edit <id> ...`.
If not found, `fluffbuzz cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `fluffbuzz update status` (preferred for source checkouts and channels)
2. `npm view fluffbuzz version` (published npm version)

## FluffBuzz command accuracy

Use only supported commands and flags:

- `fluffbuzz security audit [--deep] [--fix] [--json]`
- `fluffbuzz status` / `fluffbuzz status --deep`
- `fluffbuzz health --json`
- `fluffbuzz update status`
- `fluffbuzz cron add|list|runs|run`

Do not invent CLI flags or imply FluffBuzz enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FluffBuzz compaction:
